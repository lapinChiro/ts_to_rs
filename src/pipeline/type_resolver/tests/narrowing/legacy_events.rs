//! Legacy narrowing-event detection tests (typeof / instanceof / null
//! check / truthy / compound guards / complement narrowing).
//!
//! These predate the `NarrowTrigger` split and the T4 migration; they
//! verify that `TypeResolver::detect_narrowing_guard` /
//! `detect_early_return_narrowing` still emit the right narrow events
...
fn test_narrowing_typeof_string() {
    ...
    let has_string_narrowing = narrow_views(&res)
...
fn test_narrowing_instanceof() {
    ...
    let has_error_narrowing = narrow_views(&res).any(|e| {
        e.var_name == "x"
...
fn test_narrowing_null_check() {
    ...
    let has_non_null_narrowing = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    assert!(
...
fn test_narrowing_eq_null_generates_event_in_alternate() {
    ...
    let has_else_narrowing = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    assert!(
        has_else_narrowing,
...
fn test_narrowing_eq_null_without_alternate_generates_no_event() {
    ...
    let has_narrowing = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    assert!(
        !has_narrowing,
...
fn test_narrowing_compound_eq_null_and_typeof_generates_both_events() {
    ...
    let has_x_narrowing = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    let has_y_narrowing = narrow_views(&res)
...
fn test_narrowing_typeof_object_on_any_enum() {
    ...
    let has_object_narrowing =
        narrow_views(&res).any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::Any));
    assert!(
...
fn test_narrowing_complement_typeof_eq_in_alternate_2variant() {
    ...
    let has_f64_in_alt =
        narrow_views(&res).any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::F64));
    assert!(
        has_f64_in_alt,
...
fn test_narrowing_complement_3variant_sub_union() {
    ...
    let complement_event = narrow_views(&res)
...
fn test_narrowing_early_return_null_check() {
    ...
    let events_after_if: Vec<_> = narrow_views(&res)
        .filter(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String))
        .collect();
    assert!(
...
fn test_narrowing_early_return_typeof_neq() {
    ...
    let has_string_after_if = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    assert!(
        has_string_after_if,
...
fn test_narrowing_early_return_instanceof() {
    ...
    let has_instanceof_event = narrow_views(&res).any(|e| {
        e.var_name == "x"
...
fn test_narrowing_early_return_scope_boundary() {
    ...
    let complement_event =
        narrow_views(&res).find(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::F64));
    ...
    let positive_event = narrow_views(&res)
        .find(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
...
fn test_narrowing_typeof_boolean() {
    ...
    let has_bool_narrowing =
        narrow_views(&res).any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::Bool));
    assert!(
        has_bool_narrowing,
...
fn test_narrowing_truthy_check() {
    ...
    let has_truthy_narrowing = narrow_views(&res)
        .any(|e| e.var_name == "x" && matches!(e.narrowed_type, RustType::String));
    assert!(

...
pub fn convert_stmt(
    ...
    match stmt {
        ast::Stmt::Return(ret) => {
            ...
            if let Some(stmts) = try_expand_spread_return(ret, tctx, type_env, synthetic)? {
                return Ok(stmts);
            }
            let expr = ret
                .arg
        ...
        ast::Stmt::Decl(ast::Decl::Var(var_decl)) => {
            // Spread array detection at SWC AST level
            if let Some(stmts) = try_expand_spread_var_decl(var_decl, tctx, type_env, synthetic)? {
                return Ok(stmts);
            }
            if let Some(expanded) =
                try_convert_object_destructuring(var_decl, tctx, type_env, synthetic)?
            {
                Ok(expanded)
            } else if let Some(expanded) =
                try_convert_array_destructuring(var_decl, tctx, type_env, synthetic)?
            {
                Ok(expanded)
            } else {
                convert_var_decl(var_decl, tctx, type_env, synthetic)
            }
        }
        ast::Stmt::If(if_stmt) => convert_if_stmt(if_stmt, tctx, return_type, type_env, synthetic),
        ast::Stmt::Expr(expr_stmt) => {
            // Spread array detection at SWC AST level
            if let Some(stmts) = try_expand_spread_expr_stmt(expr_stmt, tctx, type_env, synthetic)?
            {
                return Ok(stmts);
            }
            // Cat A: standalone expression
            let expr = convert_expr(&expr_stmt.expr, tctx, type_env, synthetic)?;
            Ok(vec![Stmt::Expr(expr)])
        }
        ast::Stmt::Throw(throw_stmt) => Ok(vec![convert_throw_stmt(
            throw_stmt, tctx, type_env, synthetic,
        )?]),
        ast::Stmt::While(while_stmt) => {
            convert_while_stmt(while_stmt, tctx, return_type, type_env, synthetic)
        }
        ...
        ast::Stmt::For(for_stmt) => {
            match convert_for_stmt(for_stmt, tctx, return_type, type_env, synthetic) {
                Ok(s) => Ok(vec![s]),
                Err(_) => {
                    convert_for_stmt_as_loop(for_stmt, tctx, return_type, type_env, synthetic)
                }
            }
        }
        ...
        ast::Stmt::Try(try_stmt) => {
            convert_try_stmt(try_stmt, tctx, return_type, type_env, synthetic)
        }
        ast::Stmt::Decl(ast::Decl::Fn(fn_decl)) => {
            Ok(vec![convert_nested_fn_decl(fn_decl, tctx, synthetic)?])
        }
        ast::Stmt::Switch(switch_stmt) => {
            convert_switch_stmt(switch_stmt, tctx, return_type, type_env, synthetic)
        }
...
fn convert_if_with_conditional_assignment(
    ...
    let rhs_ir = convert_expr(ca.rhs, tctx, type_env, synthetic)?;
    ...
    if let Some(outer) = &ca.outer_comparison {
        ...
        let other = convert_expr(outer.other_operand, tctx, type_env, synthetic)?;
...
fn convert_if_stmt(
    ...
    if let Some(ca) = extract_conditional_assignment(&if_stmt.test) {
        let then_body =
            convert_block_or_stmt(&if_stmt.cons, tctx, return_type, type_env, synthetic)?;
        let else_body = if_stmt
            .alt
            .as_ref()
            .map(|alt| convert_block_or_stmt(alt, tctx, return_type, type_env, synthetic))
            .transpose()?;
        return convert_if_with_conditional_assignment(
            &ca, tctx, type_env, then_body, else_body, synthetic,
        );
    }
    ...
    let (if_let_guards, non_if_let_ast): (Vec<_>, Vec<_>) = {
        ...
        for (guard, ast_expr) in &compound.guards {
            if can_generate_if_let(guard, type_env, tctx) {
                if_let.push(guard);
            } else {
    ...
    if !if_let_guards.is_empty() {
        ...
        let remaining_condition =
            convert_and_combine_conditions(&all_remaining, tctx, type_env, synthetic)?;
        ...
        let stmt = build_nested_if_let(&if_let_guards, type_env, tctx, inner_body, else_body);
        return Ok(vec![stmt]);
    }
    ...
    // Convert then branch
    let then_body = convert_block_or_stmt(&if_stmt.cons, tctx, return_type, type_env, synthetic)?;
    ...
    if let Some(guard) = guard {
        if can_generate_if_let(guard, type_env, tctx) {
            return Ok(vec![generate_if_let(
                guard, type_env, tctx, then_body, else_body,
            )]);
        }
    }

    // Fallback: regular if statement
    let condition = convert_expr(&if_stmt.test, tctx, type_env, synthetic)?;
    Ok(vec![Stmt::If {
        condition,
        then_body,
        else_body,
...
fn convert_and_combine_conditions(
    ...
    for ast_expr in exprs {
        parts.push(convert_expr(ast_expr, tctx, type_env, synthetic)?);
    }
...
fn build_nested_if_let(
    ...
    for guard in guards.iter().rev() {
        let stmt = generate_if_let(guard, type_env, tctx, current_body, else_body.clone());
...
fn can_generate_if_let(
    ...
    guard.if_let_pattern(type_env, tctx).is_some()
}
...
fn generate_if_let(
    ...
    let (pattern, is_swap) = guard.if_let_pattern(type_env, tctx).unwrap();
...
fn convert_for_stmt(
    ...
    let (var, start) = match &for_stmt.init {
        Some(ast::VarDeclOrExpr::VarDecl(var_decl)) => {
            ...
            let start_expr = convert_expr(init, tctx, type_env, synthetic)?;
            (name, start_expr)
        }
        _ => {
            return Err(anyhow!(
    ...
    let end = match &for_stmt.test {
        Some(test) => match test.as_ref() {
            ast::Expr::Bin(bin) if bin.op == ast::BinaryOp::Lt => {
                ...
                convert_expr(&bin.right, tctx, type_env, synthetic)?
            }
    ...
    let mut body = convert_block_or_stmt(&for_stmt.body, tctx, return_type, type_env, synthetic)?;
...
fn convert_for_of_stmt(
    ...
    let iterable = convert_expr(&for_of.right, tctx, type_env, synthetic)?;
    let body = convert_block_or_stmt(&for_of.body, tctx, return_type, type_env, synthetic)?;
    Ok(Stmt::ForIn {
        label: None,
        var,
        iterable,
...
fn convert_for_in_stmt(
    ...
    // Cat A: iterator source
    let obj = convert_expr(&for_in.right, tctx, type_env, synthetic)?;
    let iterable = Expr::MethodCall {
        object: Box::new(obj),
        method: "keys".to_string(),
        args: vec![],
    };
    let body = convert_block_or_stmt(&for_in.body, tctx, return_type, type_env, synthetic)?;
    Ok(Stmt::ForIn {
        label: None,
        var,
        iterable,
...
fn convert_labeled_stmt(
    ...
    match labeled.body.as_ref() {
        ast::Stmt::While(while_stmt) => {
            ...
            let condition = convert_expr(&while_stmt.test, tctx, type_env, synthetic)?;
            let body =
                convert_block_or_stmt(&while_stmt.body, tctx, return_type, type_env, synthetic)?;
            Ok(Stmt::While {
        ...
        ast::Stmt::ForOf(for_of) => {
            let mut stmt = convert_for_of_stmt(for_of, tctx, return_type, type_env, synthetic)?;
        ...
        ast::Stmt::For(for_stmt) => {
            let mut stmt = convert_for_stmt(for_stmt, tctx, return_type, type_env, synthetic)?;
            if let Stmt::ForIn { ref mut label, .. } = stmt {
                *label = Some(label_name);
            }
            Ok(stmt)
...
fn convert_while_stmt(
    ...
    let body = convert_block_or_stmt(&while_stmt.body, tctx, return_type, type_env, synthetic)?;

    if let Some(ca) = extract_conditional_assignment(&while_stmt.test) {
        return convert_while_with_conditional_assignment(&ca, tctx, type_env, body, synthetic);
    }
...
fn convert_try_stmt(
    ...
    if let Some(finalizer) = &try_stmt.finalizer {
        let finally_body =
            convert_stmt_list(&finalizer.stmts, tctx, return_type, type_env, synthetic)?;
    ...
    if let Some(handler) = &try_stmt.handler {
        ...
        let catch_body =
            convert_stmt_list(&handler.body.stmts, tctx, return_type, type_env, synthetic)?;
...
fn convert_throw_stmt(
    ...
    let err_arg = extract_error_message(&throw_stmt.arg, tctx, type_env, synthetic);
...
fn extract_error_message(
    ...
    match expr {
        ast::Expr::New(new_expr) => {
            ...
            if let Some(args) = &new_expr.args {
                if let Some(first) = args.first() {
                    ...
                    if let Ok(e) = convert_expr(&first.expr, tctx, type_env, synthetic) {
                        return e;
                    }
                }
            }
        ...
        other => convert_expr(other, tctx, type_env, synthetic)
...
pub fn convert_stmt_list(
    ...
    let mut result = Vec::new();
    for stmt in stmts {
        let converted = convert_stmt(stmt, tctx, return_type, type_env, synthetic)?;
        for s in &converted {
            match s {
                Stmt::Let {
                    name, ty: Some(ty), ..
...
fn convert_spread_segments(
    ...
    array_lit
        ...
        .filter_map(|e| e.as_ref())
        .map(|elem| {
            let expr = convert_expr(&elem.expr, tctx, type_env, synthetic)?;
...
fn try_expand_spread_var_decl(
    ...
    let segments = convert_spread_segments(array_lit, tctx, type_env, synthetic)?;
...
fn convert_switch_stmt(
    ...
    if let Some(result) =
        try_convert_discriminated_union_switch(switch, tctx, return_type, type_env, synthetic)?
    {
        return Ok(result);
    }

    // Check if this is a switch (typeof x) on a union/any enum type
    if let Some(result) = try_convert_typeof_switch(switch, tctx, return_type, type_env, synthetic)?
    {
        return Ok(result);
    }
    ...
    if let Some(result) =
        try_convert_string_enum_switch(switch, tctx, return_type, type_env, synthetic)?
    {
        return Ok(result);
    }

    // Cat A: switch discriminant
    let mut discriminant = convert_expr(&switch.discriminant, tctx, type_env, synthetic)?;
    ...
    if has_code_fallthrough {
        ...
        convert_switch_clean_match(switch, discriminant, tctx, return_type, type_env, synthetic)
    }
}
...
fn try_convert_typeof_switch(
    ...
    for case in &switch.cases {
        if let Some(test) = &case.test {
            ...
            let variant = resolve_typeof_to_enum_variant(&var_type, &typeof_str, tctx);
...
fn convert_block_or_stmt(
    ...
    match stmt {
        ...
        other => convert_stmt(other, tctx, return_type, type_env, synthetic),
    }
}

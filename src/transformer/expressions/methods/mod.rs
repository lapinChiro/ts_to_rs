...
//! ## Module layout
//!
//! - [`self`] — constants (`PATTERN_ARG_METHODS`, `REGEX_PATTERN_ARG_METHODS`,
//!   `REMAPPED_METHODS`), [`is_remapped_method`] predicate,
//!   [`build_iter_method_call`] helper, and the [`map_method_call`] main
//!   dispatcher plus [`build_regex_new`] for `Regex` literal emission
//! - [`closures`] — closure-argument rewrites used by iterator method
//!   arms ([`closures::deref_closure_params`],
//!   [`closures::strip_closure_type_annotations`],
//!   [`closures::wrap_sort_comparator_body`])
...
use self::closures::{
    deref_closure_params, strip_closure_type_annotations, wrap_sort_comparator_body,
};

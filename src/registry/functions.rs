...
fn extract_rest_param(rest: &ast::RestPat) -> Option<(String, TsTypeInfo)> {
    ...
    let ty = type_ann.and_then(|ann| convert_to_ts_type_info(&ann.type_ann).ok())?;
    Some((name, ty))
}
...
fn extract_ident_param(ident: &ast::BindingIdent) -> Option<(String, TsTypeInfo)> {
...
/// - その他: `None`
pub(super) fn extract_ts_fn_param(param: &ast::TsFnParam) -> Option<ParamDef<TsTypeInfo>> {
    match param {
        ast::TsFnParam::Ident(ident) => {
            let (name, ty) = extract_ident_param(ident)?;
            Some(ParamDef {
                name,
                ty,
        ...
        ast::TsFnParam::Rest(rest) => {
            let (name, ty) = extract_rest_param(rest)?;
            Some(ParamDef {
                name,
                ty,
                optional: false,
...
/// - `Pat::Assign`: デフォルトパラメータ。左辺の Ident から型を取得し `has_default: true` を設定
/// - `Pat::Rest`: arg から名前、type_ann から型を取得（fallback: arg の type_ann）
/// - その他: `None`
pub(super) fn extract_pat_param(pat: &ast::Pat) -> Option<ParamDef<TsTypeInfo>> {
    match pat {
        ast::Pat::Ident(ident) => {
            let (name, ty) = extract_ident_param(ident)?;
            Some(ParamDef {
...
/// 関数型エイリアス (`type F = (x: T) => U`) を `TypeDef::Function<TsTypeInfo>` として収集する。
pub(super) fn try_collect_fn_type_alias(
    ...
    match alias.type_ann.as_ref() {
        ast::TsType::TsFnOrConstructorType(ast::TsFnOrConstructorType::TsFnType(fn_type)) => {
            let params: Vec<ParamDef<TsTypeInfo>> = fn_type
                .params
                .iter()
                .filter_map(extract_ts_fn_param)
                .collect();
            let has_rest = fn_type
                .params
                .iter()
                .any(|p| matches!(p, ast::TsFnParam::Rest(_)));
            let return_type = convert_to_ts_type_info(&fn_type.type_ann.type_ann).ok();
        ...
        ast::TsType::TsTypeLit(lit) => try_collect_call_signature_fn(lit),
        _ => None,
    }
}

/// Extracts a `TypeDef::Function<TsTypeInfo>` from a type literal that contains only call signatures.
///
/// `type Handler = { (c: string): number }` → `TypeDef::Function { params: [(c, TsTypeInfo::String)], return_type: Some(TsTypeInfo::Number) }`
///
...
fn try_collect_call_signature_fn(lit: &ast::TsTypeLit) -> Option<TypeDef<TsTypeInfo>> {
    ...
    let params: Vec<ParamDef<TsTypeInfo>> =
        sig.params.iter().filter_map(extract_ts_fn_param).collect();
    let has_rest = sig
        .params
        .iter()
        .any(|p| matches!(p, ast::TsFnParam::Rest(_)));
    ...
    let type_params: Vec<TypeParam<TsTypeInfo>> = collect_type_params(sig.type_params.as_deref());
...
pub(super) fn collect_fn_def_with_extras(func: &ast::Function) -> Result<TypeDef<TsTypeInfo>> {
    let params: Vec<ParamDef<TsTypeInfo>> = func
        .params
        .iter()
        .filter_map(|param| extract_pat_param(&param.pat))
        .collect();
    let has_rest = func
        .params
        .iter()
    ...
    let type_params = collect_type_params(func.type_params.as_deref());
...
pub(super) fn collect_arrow_def_with_extras(arrow: &ast::ArrowExpr) -> Result<TypeDef<TsTypeInfo>> {
    let params: Vec<ParamDef<TsTypeInfo>> =
        arrow.params.iter().filter_map(extract_pat_param).collect();
    let has_rest = arrow
        .params
        .iter()
    ...
    let type_params = collect_type_params(arrow.type_params.as_deref());

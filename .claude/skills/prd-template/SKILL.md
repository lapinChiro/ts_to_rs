...
- **Spec stage 完了 verification**: `.claude/rules/spec-stage-adversarial-checklist.md` の **13-rule checklist を全項目 verify** (1 = Matrix completeness + abbreviation prohibition (sub-rule 1-1/1-2/1-3/1-4) / 2 = Oracle grounding + PRD doc embed mandatory (sub-rule 2-1/2-2/2-3) / 3 = NA justification + SWC parser empirical observation (sub-rule 3-1/3-2/3-3) / 4 = Grammar consistency + doc-first dependency order (sub-rule 4-1/4-2/4-3) / 5 = E2E readiness + Stage tasks separation (sub-rule 5-1/5-2/5-3/5-4) / 6 = Matrix/Design integrity + Scope 3-tier consistency (sub-rule 6-1/6-2/6-3/6-4) / 7 = Control-flow exit sub-case completeness (sub-rule 7-1/7-2/7-3/7-4) / 8 = Cross-cutting invariant enumeration (sub-rule 8-1/8-2) / 9 = Dispatch-arm sub-case alignment (sub-rule 9-1/9-2/9-3/9-4) / 10 = Cross-axis matrix completeness (9 default axis、(i) AST dispatch hierarchy 含む) / 11 = AST node enumerate completeness check (sub-rule 11-1/11-2/11-3/11-4/11-5/11-6、`_` arm 全面禁止 + phase 別 mechanism + ast-variants.md single source of truth + pre-draft ast-variant audit + Architectural concern relevance scope) / 12 = Rule 10/11 Mandatory application + structural enforcement (sub-rule 12-1〜12-8) / 13 = Spec Stage Self-Review (skill workflow Step 4.5 hard-code、sub-rule 13-1〜13-6 including Cell numbering convention audit symmetry))。1 つでも未達なら Implementation stage 移行不可。
...
- (matrix-driven の場合) `spec-stage-adversarial-checklist.md` 13-rule 全項目を本 PRD 内で verification 済 (Rule 1-10 + Rule 11 AST node enumerate completeness check + Rule 12 Mandatory application + structural enforcement + Rule 13 Spec Stage Self-Review skill workflow integrated)
...
| Rule | [spec-stage-adversarial-checklist.md](../../rules/spec-stage-adversarial-checklist.md) | Spec stage 完了 verification (13-rule checklist、本 skill が参照) |

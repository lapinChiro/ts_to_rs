...
4. **Factual accuracy semantic check** (= I-D-pre Cell 4 / v11-7、framework v1.8 source、確定 2026-05-11):
   ...
   - **単純 grep + 存在 check ではなく**、reference が claim する **意味論的 context との一致** を verify (= 例: "Iteration v11 で expressions/mod.rs 同居 cohesion 化" が claim する → v11 の actual change scope と expressions/mod.rs の cohesion 化が同 PRD の同 iteration で行われた事実と一致するか確認)
   - **Verification mechanism (structural enforcement)**:
     - **(4-1) Line-ref factual accuracy** = `scripts/verify_line_refs.py` (Method A utility) で PRD doc 内 heading-based line-ref drift detection (= heading 行 number と PRD doc 内 claimed line ref の semantic sync 自動 verify)。Method A utility は **regression-tested formal lock-in** (= `tests/i_d_pre_method_a_test.rs` で auto-verify mechanism、I-D-pre Cell 4 / T1-pre-5 formal lock-in DONE)
     - **(4-2) Handoff doc line-ref factual accuracy** = `scripts/audit-handoff-doc-line-refs.py` で handoff doc 内 `<path>:<line>` cross-ref drift detection (= 4 categories: INVALID_RANGE / MISSING_FILE / OUT_OF_BOUNDS / AMBIGUOUS)、CI step integrated (= PR merge gate、I-D-pre Cell 3 / T1-pre-3a/b DONE)
     - **(4-3) Cross-reference semantic accuracy** = `scripts/verify_prd_self_audits.py` (Path E utility) で PRD doc 内 cross-reference (= Scope / Invariants / Spec→Impl Mapping / Test Plan 等 sections) の cell # appearance consistency + status pending verdict + label namespace collision + external file drift の 4 axes auto-verify
   - **Failure mode**: reference が文法的に正しい (= grep で hit する) が **意味論的 context が claim と矛盾** している (= factual conflate)。例: I-224 T6a 1st-round で "Iteration v11 で expressions/mod.rs 同居 cohesion 化" を catch せず通過、2nd-round adversarial で発覚 (= v11 と expressions/mod.rs が異 PRD の異 iteration の事実、conflate detection 必須)
   - **Lesson source**: PRD I-224 T6a 1st-round factual conflate miss → 2nd-round adversarial で発覚、framework rule level での Layer 1 semantic check sub-step 必須化。I-D-pre Cell 4 / v11-7 で Method A utility (`scripts/verify_line_refs.py`) formal lock-in、本 sub-step (4) を v1.8 で wording 追加 (= self-applied integration)
...
- **v1.8** (2026-05-11): I-D-pre Cell 4 / v11-7 self-applied integration として Layer 1 (Mechanical) sub-step (4) "Factual accuracy semantic check" 新規追加。
  - **Sub-step (4)**: 修正 doc / comment / commit message 内 固有名詞 reference (PRD ID / Iteration v# / task ID / file path / line ref / function/struct 名) が claim する **意味と一致** することを semantic check (= 単純 grep + 存在 check ではなく **意味論的 context との一致** を verify)
  - **Structural enforcement (3 mechanisms hard-coded)**:
    - (4-1) `scripts/verify_line_refs.py` (Method A、PRD doc heading-based line-ref drift detection、`tests/i_d_pre_method_a_test.rs` で formal lock-in)
    - (4-2) `scripts/audit-handoff-doc-line-refs.py` (handoff doc `<path>:<line>` cross-ref drift detection、4 categories、CI merge gate integrated)
    - (4-3) `scripts/verify_prd_self_audits.py` (Path E、PRD doc 内 4 axes cross-reference auto-verify、strict byte-exact comparison per Rule 13 (13-6-c))
  - **Lesson source**: PRD I-224 T6a 1st-round で `Iteration v11 で expressions/mod.rs 同居 cohesion 化` factual conflate を catch せず通過、2nd-round adversarial で発覚 → framework rule level での Layer 1 semantic check sub-step 必須化。I-D-pre Cell 4 / v11-7 で Method A utility 早期実装 (Iteration v12) + 本 PRD Phase 5 (T2-pre-1) で formal lock-in
  - **Related Rule 9 sub-rule (9-4) + Rule 13 sub-rule (13-6) v1.8 integration**: `spec-stage-adversarial-checklist.md` Rule 9 sub-rule (9-4) "Cell numbering convention single-source-of-truth" + Rule 13 sub-rule (13-6) "Cell numbering convention audit symmetry" と同 framework v1.8 self-applied integration set (= I-D-pre Phase 5 = T2-pre-1 + T2-pre-2 coordinated rule wording strengthening)。本 sub-step (4) の (4-3) mechanism Path E utility の strict byte-exact comparison は Rule 13 (13-6-c) audit ↔ Rule symmetry principle の本 Layer 1 sub-step (4) factual accuracy 領域への適用結果 (= /check_job deep deep review で発見、`scripts/verify_prd_self_audits.py:567 verify_external_file_drift` 100-byte tolerance 排除で structural fix DONE)。

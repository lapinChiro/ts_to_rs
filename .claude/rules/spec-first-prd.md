...
**由来**: I-144 T1 (2026-04-19) で R4 (`&&=` narrow preserve) / F6 (try body narrow
preserve) が observation ✓ だったが Rust emission で E0308 / try body 崩壊 が発覚。
両 cell を I-161 / I-149 scope の pre-existing defect として別起票、本 PRD regression
fixture から削除。本条項はこの empirical lesson を structural に framework 化する。
...
## Lessons Learned (I-144 T1 — Dual verdict)

Phase 4 Rollout 適用中 (I-144) で得た重大な framework 学び:

4. **observation ✓ は Rust emission ✓ を保証しない**: I-144 Spec stage v2 で
   R4 (`&&=` on narrowed) / F6 (try body narrow preserve) は tsc observation で
   TS runtime ✓ preserved と確認されていた。しかし T1 per-cell E2E fixture 作成で
   empirical probe した結果、Rust emission は R4=E0308 (`&&` が f64 に非適用) /
   F6=try/catch 崩壊 (throw の return Err + catch body 欠落) で RED 発覚。
   **原因**: observation は TS 側の ground truth、Rust emission は独立の concern。
   matrix cell に bare「✓ preserved」と書くと Rust 側を暗黙に ✓ と誤認する。
   **framework 対応**: Stage 1 artifact #3 に「T1 E2E probe = Rust emission
   empirical verify」を明記、matrix cell verdict を **Dual verdict (TS / Rust)**
   に structural 強制。T1 probe 結果で TS ✓ / Rust ✗ が判明した cell は
   pre-existing defect として別 PRD scope に empirical 再分類する責務を T1 に持たせる。

## Versioning

本ルールは SDCDF **Beta** (Phase 4 Rollout 昇格: 2026-04-17)。
Phase 3 Pilot で **Spec gap = 0** を達成し、正式 rule として昇格。
I-144 T1 で **Dual verdict (TS / Rust)** 条項を追加 (2026-04-19)。

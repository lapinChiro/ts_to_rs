...
## Worked Example (I-224 T5-2 Iteration v11)

**契機**: T5-2 で `UserMainSubstitution` enum を追加 → `transformer/mod.rs` が
1005 行 (= 5 行超過)。

**Step 1 (周辺調査)**:
- `transformer/mod.rs`: `Transformer` struct 定義 + `transform_module` /
  `transform_module_collecting` の dispatch
- `transformer/main_synthesis/mod.rs`: `UserMainKind` enum 定義 + 996 行 (= 既に
  near-limit)
- `transformer/main_synthesis/user_main.rs`: `detect_user_main` 関数 (= UserMainKind
  を生成)

**Step 2 (問題点)**:
1. **DRY 違反**: `transform_module` と `transform_module_collecting` で `(exec_mode,
   user_main_kind) → UserMainSubstitution` の dispatch table が **同じ match block で
   2 箇所重複** (各 ~10 行)
2. **凝集度低下 (pre-existing)**: `UserMainKind` (型) は `main_synthesis/mod.rs` に、
   それを生成する `detect_user_main` (関数) は `main_synthesis/user_main.rs` に
   別居 = 関連 knowledge の認知コスト増
3. **新追加の `UserMainSubstitution` をどこに置くか**: 単純切り出しなら
   `transformer/user_main_substitution.rs` (機械的末尾切り出し) だが、これは
   `UserMainKind` から派生する型を別 module に置くことになり凝集度をさらに下げる

**Step 3 (再構成 plan = 選択 A + B の組合せ)**:
- `UserMainKind` enum を `main_synthesis/mod.rs` → `main_synthesis/user_main.rs` に移動
  (= 型と detector を同居 = 凝集度向上)
- `UserMainSubstitution` enum + impl も `main_synthesis/user_main.rs` に追加 (= 入力
  dimension 型と派生 dispatch state 型を同居)
- `UserMainSubstitution::from_dispatch(exec_mode, user_main_kind)` constructor を
  新設 (= DRY 解消の single source of truth)
- `transform_module` / `transform_module_collecting` の重複 match block を
  `UserMainSubstitution::from_dispatch(...)` 1 行に置換

**Step 4 (検証)**: `transformer/mod.rs` 931 行 / `main_synthesis/mod.rs` 974 行 /
`main_synthesis/user_main.rs` 389 行、全 file < 1000、clippy 0 warnings、3542 lib tests
pass、7 i224 invariants tests pass。

**結果**: 行数超過の解消 + 凝集度向上 (UserMainKind + detect_user_main +
UserMainSubstitution が同居) + DRY 違反解消 (dispatch table 単一 source of truth) を
**同時に達成**。
...
- **v1.0** (2026-05-08): I-224 T5-2 Iteration v11 で `transformer/mod.rs` が 1005 行
  超過した契機で本ルール新設。Worked Example として `UserMainKind` + `detect_user_main`
  + `UserMainSubstitution` の `main_synthesis/user_main.rs` 同居 + `from_dispatch`
  constructor による DRY 解消を記録。

...
- **I-319+I-343 配列インデックス安全化**: Vec インデックス読み取りを `.get(idx).cloned().unwrap()` に変換。代入ターゲットは直接インデックスを維持。配列デストラクチャリングも安全化。I-343（Vec<Struct> move エラー）も副次解消。I-346 で downstream unwrap を追加しテストインフラ復旧

...
13. [I-224: top-level fn main mechanism + framework v12-2 candidate empirical 補強 chain](#i-224-top-level-fn-main-mechanism--framework-v12-2-candidate-empirical-補強-chainclosed-2026-05-09)
14. [I-D-pre: Audit mechanism bootstrap](#i-d-pre-audit-mechanism-bootstrap-path-b-split-adoption--bootstrapping-circularity-構造的解消closed-2026-05-11)
15. [I-D-c11: Path E utility `expand_cell_list` filter upper-bound uniform consistency](#i-d-c11-path-e-utility-expand_cell_list-pattern-全-filter-upper-bound-uniform-consistencyclosed-2026-05-18)
16. [I-205: Class member access dispatch with getter/setter methodology framework](#i-205-class-member-access-dispatch-with-getter-setter-methodology-frameworkclosed-2026-05-18follow-up-t14-t16-deferred-to-todo)
17. [Lock-in テスト (削除禁止)](#lock-in-テスト-削除禁止)
18. [残存 broken window](#残存-broken-window)
...
- **Module-level identifier axis** (`check_ts_internal_fn_name_namespace` @ `src/transformer/statements/mod.rs:88` 経由 `scan_for_ts_namespace_collisions` @ `src/transformer/namespace_lint.rs`、I-224 T1 で追加):
  ...
  - SWC AST `_` arm 不在 (Rule 11 (11-1) 自己適用準拠、新 variant 追加時は compile error で全 dispatch 強制更新)
...
| Rule 11 (11-1)〜(d-4) | AST node enumerate completeness check: `_ => ` arm 全面禁止 + phase 別 mechanism (Transformer = `UnsupportedSyntaxError`、TypeResolver = no-op + reason comment、NA = `unreachable!()`) + `doc/grammar/ast-variants.md` single source of truth + `audit-ast-variant-coverage.py` CI 化 |
| Rule 12 (12-1)〜(12-8) | Rule 10/11 Mandatory application + structural enforcement (matrix-driven / non-matrix-driven 区別なし)、Permitted/Prohibited reasons 列挙、machine-parseable yaml block format hard-code、`prd-template` skill Step 0c で必須 section 化 |
...
- **I-203** (TODO、user 承認 2026-04-27): Codebase-wide AST match exhaustiveness compliance (Rule 11 (11-1) 既存 codebase application、本 PRD で確立した `_` arm 全面禁止 + `unsupported_arm!()` macro pattern を codebase 全体に適用)
...
- **Third-party review H-2 finding**: `Rule 12 (12-3) Permitted reasons` (= "infra で AST input dimension irrelevant" / "refactor で機能 emission decision なし" / "pure doc 改修") に該当しない **gray zone violation**、**ideal-implementation-primacy 観点で「実装範囲が広い (test harness 跨ぎ)」を defer 理由とする compromise を排除**
...
**Lesson (cross-PRD applicable)**: 「test harness / lib API / external dependency limitation を理由に PRD scope を分離する」decision は `feedback_no_dev_cost_judgment.md` 違反 risk = `Rule 12 (12-3)` Permitted reasons に該当するか **explicit verify 必須**。該当しない場合は cohesive batch で本 PRD scope に integrate (= "1 PRD = 1 architectural concern" boundary を architectural concern relevance で再定義、scope 規模ではなく concern boundary で判断)。
...
### 3-tuple dispatch tree + per-item runtime decision split pattern (= Rule 9 (9-1) 1-to-1 mapping compliance)
...
- **Medium 1 (1-to-1 mapping violation)**: `has_lit_top_level_const` 次元は A6 (mixed) cells で複数 arm に partition (cells #71/#72 等が 2 arms に double-claim)、Rule 9 (9-1) 1-to-1 違反
...
4. **Substitute/rewrite logic dispatch arm symmetric coverage** (v11-1): substitute logic にも Rule 9 (9-1) Dispatch-arm sub-case alignment を symmetric 適用
...
- `tests/i224_helper_test.rs` (Rule 9 (9-1) 1-to-1 mapping lock-in)
...
| 5 | v13-5 | `spec-stage-adversarial-checklist.md` Rule 9 (9-4) + Rule 13 (13-6) cell numbering convention + `verify_cell_numbering_drift_detection` + Path E Axis 3 `CELL_SLOT_AS_IDENTIFIER_RE` | Phase 3 + Phase 5 |
...
2. **2nd `/check_job` deep deep review**: 新 Rule 9 (9-4) "single-source-of-truth = matrix #" author
...
- Rule 9 sub-rule (9-4): Cell numbering convention single-source-of-truth (Written form "Cell N",
...
- **全 future PRDs**: bootstrap utilities (verify_line_refs.py / verify_prd_self_audits.py /
  audit-handoff-doc-line-refs.py) + Layer 1 sub-step (4) factual accuracy semantic check rule + Rule 9 (9-4) /
...
## I-205: Class member access dispatch with getter/setter methodology framework (closed 2026-05-18、follow-up T14-T16 deferred to TODO)

**TL;DR**: Class field の **getter / setter method dispatch** を AST レベルで Read / Write / Update / Compound assign / Logical compound 全 context に対し structural symmetric に確立、Decision Table A/B/C (3-axis 81 cells matrix) で全 dispatch arm の Tier 1 化 + Tier 2 honest error reclassify を T1-T13 で empirical lock-in。**T1-T13 implementation 完了 2026-05-01** (= architectural concern 達成 + Hono Tier-transition compliance Improvement / Preservation + 3296 lib tests + 159 e2e tests + 122 integration tests PASS)、T14 (E2E fixture green-ify) は **I-162 (= class without explicit constructor → `Self::new()` 自動合成、案 γ Phase 1) prerequisite で block**、T15 (/check_job 4-layer review) + T16 (semantic naming refactor + I-205 範囲内 `unwrap()` cleanup) は T14 完了後の follow-up activities。**2026-05-18 PRD doc close 判定**: T1-T13 で I-205 architectural concern fully achieved + matrix 全セル unit test level lock-in 達成、T14-T16 = post-Implementation verification + cleanup 性質 = closed PRD precedent (I-D-pre / I-224 / I-399 / I-180) と同 pattern で doc/handoff/ archive + TODO restore + backlog/ delete に integrate、backlog/ clean state 確立 + 案 γ Phase 2 着手 timing で TODO entry + 本 archive section reference で restore。

### Architectural concern + 解決軸

I-205 architectural concern = **"Class member access dispatch with getter/setter methodology framework"**。Pre-PRD (= PRD 2.8 I-201-A AutoAccessor 単体 Tier 1 化 Spec stage 中) で empirical 発見 = **framework 全体が broken** (= `class Foo { get name(): string {...}; set name(v: string) {...}; }` に対し `obj.name` Read / `obj.name = v` Write の Rust emission が全 context で incorrect)。解決軸 = **AST level structural symmetric dispatch establishment** (= Read context `resolve_member_access` ↔ Write context `dispatch_member_write` ↔ Update context `dispatch_*_member_update` ↔ Compound assign `dispatch_*_member_compound` ↔ Logical compound `dispatch_*_member_logical_compound` の **5 dispatch path 全 structural symmetric**)。

### Decision Table A/B/C matrix (3-axis 81 cells)

Matrix dimensions:
- **Axis A (operation kind)**: Read / Write / Update (`++/--`) / Compound assign (`+= -= *= ...`) / Logical compound (`??= &&= ||=`) = 5 variants
- **Axis B (member kind)**: B1 field / B2 getter only / B3 setter only / B4 getter+setter / B5 protected / B6 method / B7 inherited / B8 static / B9 unknown = 9 variants
- **Axis C (LHS type or getter return type)**: D1 f64 (Copy) / D2 String (non-Copy primitive) / D3 bool / D4 Vec / D5 HashMap / D6 Option<T> + Option<Copy> + Option<non-Copy> / D7 Named / D8 various = ~8 variants

Cells 1-81 with documented gaps {29, 34, 35, 41, 45, 48-49, 51-59, 65-69} = NA gaps (= Static defensive cells で別 PRD I-A/I-B migrate + cells 65-69 = AutoAccessor 系列で PRD 2.8 I-201-A scope) を total ~60 cells active matrix。

### T1-T13 implementation summary (= I-205 implementation 主要部、2026-04-28〜2026-05-01)

| Task | 完了日 | Scope | Test count |
|------|--------|-------|------------|
| T1 | 2026-04-28 | `doc/grammar/ast-variants.md` ClassMember section update + MethodKind tracking 言及 (doc-first) | - |
| T2 | 2026-04-28 | `MethodSignature.kind` + `TsMethodInfo.kind` field 追加 + `MethodKind` enum + 51 site bulk script kind preserve | 5 unit tests |
| T3 | 2026-04-28 | `collect_class_info` で method.kind propagate + `_ => {}` arm 排除 (Rule 11 (11-1) fix) + latent bug fix (`resolve_method_sig` kind hardcode) | 4 unit tests |
| T4 | 2026-04-28 | TsTypeLit / `convert_method_info_to_sig` で kind propagate (T3 batch 内 Fix 2 で前倒し完了) | 3 unit tests |
| T5 | 2026-04-28 | `resolve_member_access` に Read context dispatch 拡張 (cells 2-9) + `dispatch_instance_member_read` / `dispatch_static_member_read` helpers | 19 unit tests |
| T6 | 2026-04-28 | `convert_assign_expr` Member target arm に setter dispatch (`dispatch_member_write` helper) + Read/Write symmetric DRY refactor (`classify_member_receiver` shared helper) | 17 unit tests |
| T7 | 2026-04-29 | UpdateExpr (`++/--`) Member target setter desugar + TypeResolver `Update.arg` 未再帰 Spec gap fix | 15 unit tests |
| T8 | 2026-04-29 | Compound assign (`+= -= *= ...`) setter desugar + INV-3 1-evaluate compliance (non-Ident receiver IIFE form) + DRY refactor + `member_dispatch.rs` 6-file split | 30 unit tests |
| T9 | 2026-04-29 | Logical compound (`??= &&= \|\|=`) setter desugar + `logical_compound.rs` (783 行) + INV-3 SE-having IIFE form + LHS type orthogonality | 22 unit tests |
| T10 | 2026-05-01 | Inside-class `this.x` dispatch + `body_requires_mut_self_borrow` recursive walker (T6 setter MethodCall detection、silent E0596 compile error structural fix) | 22 unit tests |
| T12 | 2026-05-01 | Class Method Getter body C1 limited pattern (`return self.field;`) で non-Copy return type `.clone()` 自動挿入 | 20 unit tests |
| T13 | 2026-05-01 | B6/B7 corner cells Tier 2 reclassify lock-in + INV-5 Option B audit + 5 NEW integration tests | 5 integration tests |
| **Total** | | | **3296 lib tests + 5 integration + Hono Improvement (clean 110→111 / errors 64→63) + Preservation** |

### Framework lessons (= 本 PRD self-applied integration evidence、framework v1.3 → v1.4 → v1.5 → v1.6 → v1.7 連続 revision source)

1. **Rule 9 (9-3) Field-addition symmetric conversion site audit** (= framework v1.7 source、I-205 T2 + I-383 T8' (`type_params` field 追加) で 2 度連続 latent field drop pattern 発生、3 度目 structural prevention 要請): 新 IR struct field 追加 PRD で全 construction site (= constructor / builder / converter / external_types registration / serializer) を **proactive enumerate** + 各 site の責務 (`hardcode default` / `propagate from source` / `propagate from m.X`) を spec-traceable 記録、`## Field Addition Symmetric Audit` section embed mandatory。本 PRD で **first-class adopter として self-applied verify** (= T2 `MethodSignature.kind` 追加 で全 site 責務を audit + record)。`spec-stage-adversarial-checklist.md` Rule 9 (9-3) sub-rule + audit script `verify_field_addition_symmetric_audit` candidate に integrate
2. **TypeResolver visit coverage of operand-context expressions** (= T7 Iteration v11 Spec gap fix + T8 Spec gap fix で 2 度連続 silent semantic loss = framework 失敗 signal): `pipeline/type_resolver/expressions/mod.rs::resolve_expr` の `ast::Expr::Update(_)` / Compound `ast::Expr::Assign(_)` arm が `arg` / `value` を recursive resolve せず → Member target receiver expr_type 未登録 → Transformer dispatch silent Fallback、本 PRD で 2 度の Spec stage 逆戻り発動で structural fix。framework rule level での **TypeResolver visit coverage of operand-context expressions** Mandatory enforcement candidate
3. **Decision Table direct unit test coverage Layer 1 sub-rule** (= 本 archive integration 由来 framework v1.8+ candidate、PRD I-D-main T2-7 で hard-code): `UserMainSubstitution` enum + `from_dispatch` constructor (10-cell decision table) 等の **dispatch table constructor** に対する direct unit test missing が **indirect coverage** (= integration test 経由) のみで放置される pattern が 1st review iteration で発覚せず 2nd review で finding 化 = traceability cost 累積 → I-D-main T2-7 で check-job-review-layers.md Layer 1 sub-rule 追加で structural prevent
4. **Constructor body conversion bypass dispatch (`try_extract_this_assignment` で B3/B4 dispatch bypass で invalid struct field emit、I-222 別 PRD として TODO 起票)** = T10 Iteration v16 で発見、I-205 architectural concern と orthogonal な constructor body emission concern (= 別 PRD で independent treatment)
5. **`body_has_self_assignment` → `body_requires_mut_self_borrow` recursive walker structural fix** (= T6 setter dispatch 導入 で IR shape 変化 → caller helper update audit 不足、T10 で structural fix): IR shape evolution → caller helper update audit axis を Rule 9 (9-3) の symmetric audit に extend する empirical evidence

### Iteration v1〜v19 chain key learnings (= 13 T-tasks across 19 Iterations、4 weeks dev period、Spec stage v7 final + Implementation stage 全 T1-T13 完了)

- **Iteration v1-v7**: Spec stage establishment、4 Spec stage Tasks (TS-0 matrix construction + TS-1 oracle observations + TS-2 SWC parser empirical + TS-3 E2E fixture red 状態 + TS-4 Impact Area audit findings + TS-5 13-rule self-applied verify pass) + framework v1.3 → v1.6 連続 revision (= audit symmetry restoration + Rule 9 sub-rule (9-3) field-addition symmetric audit 確立)
- **Iteration v8-v13**: Implementation stage T1-T9、各 T-task 単独 commit + `/check_job` 4-layer review、Spec gap fixes 累積 (= TypeResolver visit coverage / Spec→Impl Mapping table asymmetric / matrix Block form mismatch 等)、INV-1〜INV-5 lock-in、Hono Tier-transition compliance Improvement 達成
- **Iteration v14-v18**: T8 deep-deep `/check_job` 4-layer review + T9 logical compound dispatch + T10 internal `this.x` dispatch + T12 Getter body C1 `.clone()` 自動挿入 + T13 B6/B7 corner reclassify、cells 70/72/74 unit test green + I-162 prerequisite gate で T14 E2E green-ify defer 確定
- **Iteration v19** (2026-05-01): T13 完了 + T12 architectural concern fully achieved declaration + T14-T16 scope re-partition (= I-162 prerequisite で T14 defer、T15-T16 = post-Implementation review + cleanup activities) + **subsequent 新 PRD I-D (Framework rule integration cohesive batch) migration confirm** (= I-205 lesson source 全統合 cohesive batch、別 architectural concern boundary で切り出し) = 本 PRD doc が後続 PRD I-D-main の v18 Path B split 由来 framework rule integration scope の **lesson source 主要供給** 確立

### T14-T16 deferred scope (= TODO `[I-205-T14-T16-followup]` entry で restore、案 γ Phase 2 着手 timing)

- **T14 (E2E fixture green-ify)**: cells 70/71/72/74 (= Getter body C1 `.clone()` insertion fixtures) + cells 1-81 active matrix の全 E2E fixture を red → green、Tier-transition compliance verify (`prd-completion.md` broken-fix PRD pattern: existing class Method Getter/Setter related Tier 2 errors transition Tier 1 = improvement、no new compile errors introduced for 本 PRD scope 外 features)。**Prerequisite**: I-162 (class without explicit constructor → `Self::new()` 自動合成) 完了 (= 現状 generated Rust が `pub fn init() { println!("{}", p.name()); }` で emit、I-162 完了後 `let p = Profile::new(); println!(...);` 形式に変換可能)
- **T15 (/check_job 4-layer review + 13-rule self-applied verify)**: T14 完了後、`/check_job` 起動 + Layer 1-4 全実施 + Defect classification 5 category trace、Spec gap 0 + Implementation gap 0 + 全 review findings fix 達成
- **T16 (Task-ID-based naming → semantic naming refactor + I-205 範囲内 unwrap() cleanup)**: T15 完了後、I-205 scope cleanup task = (16-a) Audit (task-ID-based naming violation enumerate) + (16-b) Semantic naming proposal table + (16-c) 実装分割再考 + 既存 rule violation cleanup integrate (`src/transformer/classes/members.rs:440 default_expr.unwrap()` 等) + (16-d) Atomic refactor commit

### TODO restore reference

PRD doc は close で削除済 (`backlog/I-205-getter-setter-dispatch-framework.md`)、T14-T16 follow-up spec は **TODO `[I-205-T14-T16-followup]` entry** + **本 archive section** で **single source of truth lesson preservation**。案 γ Phase 2 着手時 (= I-D-main close → I-225 → I-162 → I-205 T14-T16) に本 archive section + TODO entry から restore + 必要なら PRD doc 再起票 (= ad-hoc PRD I-205-followup 起票 vs TODO 経由 inline progression、案 γ Phase 2 着手時 user judgment)。

### Migration to subsequent PRD I-D context preserve

I-205 lesson source の **framework rule integration candidates** は subsequent **PRD I-D (= framework rule integration cohesive batch、別 architectural concern boundary)** に migrate 済 (2026-05-01 user 確定)、I-D parent → I-D-pre + I-D-main split (2026-05-11) で進行中 = I-205 architectural concern (= class member access dispatch with getter/setter framework) と orthogonal な "framework rule level structural enforcement" PRD として cohesive batch 化、I-D-pre close + I-D-main spec stage CONVERGED + Implementation stage T2 phase 完了 (2026-05-18) 達成。本 archive section は I-D-main / I-225 / I-162 / I-205 T14-T16 chain の prerequisite lesson source として **mandatory reference**。

### Lesson source preserved (削除禁止)

PRD doc `backlog/I-205-getter-setter-dispatch-framework.md` (3340 行) は close 2026-05-18 で削除済、本 archive section が **single source of truth lesson preservation**。本 PRD T1-T13 implementation chain + Decision Table A/B/C 81 cells matrix + framework v1.3 → v1.7 revision evidence + Iteration v1〜v19 chain 統合 lessons は将来の class member access dispatch / framework rule integration PRD (例: I-A / I-B / I-201-A / I-201-B / I-202 / I-225 / I-162 等) 起票時の **mandatory reference**。

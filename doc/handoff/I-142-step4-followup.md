...
これらは I-142 PRD の枠組みでは解消せず、**新設予定の spec-driven 開発 framework PRD**
(`plan.prd.md` で計画中) の pilot application として再着手する。現在の「実装 →
review → 修正」ループでは毎 review サイクルで同オーダーの defect が発見される
構造的欠陥が確認されており (I-142 の 3 サイクルで各 6 / 7 / 10 個の Tier 1-2
defect を発見)、同じ方法論で Step 4 を実装しても定常的に未解決 defect が残る
可能性が高いため。

根本原因の分析と新 framework の方針は以下を参照:
- 2026-04-15 セッション log: 「レビューのたびに重大な問題が発見される」議論
- `plan.prd.md`: 新 framework PRD の計画

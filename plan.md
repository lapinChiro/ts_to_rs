...
**Batch 4b: I-312b Phase 4-5（Registry の TsTypeInfo 移行 + 検証）**

Phase 1-3 完了済み（convert_ts_type の 2 ステップ化達成）。
残り: registry 関数の convert_to_ts_type_info 直接呼び出しへの移行、
Option ラップ / PascalCase 除去、旧コードのクリーンアップ、Hono ベンチマーク回帰確認。
...
| **4b** | **I-312b** | **convert_ts_type を TsTypeInfo 経由に統一** | **L2: 設計** | **Phase 1-3 完了（2ステップ化達成）。Phase 4-5 残** |
...
| テスト数 | 1809（ユニット 1627 + コンパイル 3 + スナップショット 3 + E2E 84 + integration 89）※Batch 4b Phase 3 時点 |

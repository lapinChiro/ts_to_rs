...
- **計測結果**: **Cluster 1a 11/11 完全解消** (T2.A 完了条件達成)
- **完了済**: T2.A-i (外部 builtin JSON loader), T2.A-ii (TypeResolver scope push), T2.A-iv (expected type free var 抽出)
- **cargo test 状態**: 2228 passed, 0 failed
- **dangling refs 残存**: 23 件 (DOM 16 + 不明 2 + user 1 = T2.A 対象外、別 cluster で対応)

**次タスク**: T-5 (SyntheticTypeRegistry dedup first-write-wins の guard rail) 実装で Cluster 1a 解消を構造的に lock-in、その後 T2.A2 → T1.B (PRD-B 起票) → T2.B → I-382 本体実装。

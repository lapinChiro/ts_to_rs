...
## 現在のフェーズ: Batch 25 (I-392) Phase 0-6 完了、Phase 7 待ち
...
### Phase 7 進行中 — P7.0 完了、P7.1 待ち

P7.0 return wrap infrastructure 完了 → **次: P7.1 delegate method + inner 呼び出し** →
P7.2 inner arg wrapping → P7.3 async 伝搬

### P7.0 完了内容 (2026-04-13)

- **設計判断**: Option B (variant wrap + TypeResolver 型情報) を empirical 検証で採用。
  二相分離アプローチ (SWC 型収集 + IR wrapping)。変換パイプライン・IR ともに無変更
- **`collect_return_leaf_types`** 新規作成 (return_wrap.rs): SWC arrow body の全ブロック
  構造を depth-first walk し、return leaf 式の型を `expr_types` から収集
- **`wrap_leaf`** signature 変更: `ast_arg` → `expr_type + span`。TypeResolver 型を
  variant 決定 priority 3 として追加
- **`wrap_body_returns` / `wrap_expr_tail`** を Iterator ベースに変更。全ブロック構造を
  再帰 walk。iterator 枯渇時は即 Err (positional invariant 安全ネット)
- **`convert_callable_trait_const`** に wrapping 統合
- **`#[allow(dead_code)]` 削除**: return_wrap.rs + overloaded_callable.rs
- **レビュー対応**: 全ブロック構造の走査追加 (C-1)、iterator 枯渇即 Err (I-1)、
  finally collect 除外、SeqExpr collect 除外
- **fixture**: callable-interface-inner で divergent return が variant wrap
- **unit test**: 7 件 (collect 5 件 + wrap_leaf 2 件)

### 現在の状態 (2026-04-13 session 終了時)

- **Test count**: 全テスト pass (lib 2356, integration 93, compile 3, E2E 88)
- **Quality**: clippy 0, fmt 0
- **Compile test skip**: callable-interface, call-signature-rest, interface-mixed,
  callable-interface-param-rename, callable-interface-inner (P8.2 で復帰予定)

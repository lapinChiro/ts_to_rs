...
## /start 再開時の手順 (= PRD I-D-main Implementation stage T1 phase 完了、T2 phase rule wording strengthening 着手 ready)

### Step 1: Empirical sanity check (= post T1 phase + /check_job Round 1+2 + /check_problem G-1 fix state preservation verify)
...
# 2. Path E 0 drifts (strict byte-exact mode、7 axes 全 PASS)
...
# Expected: i_d_main_audit_extensions_test 29 passed / i_d_pre_audit_extensions_test 6 passed
...
### Step 2: Primary references for next action (= T2 phase rule wording strengthening 着手 or C11 fix preempt)
...
6. **TODO entries (= 本 session 最新化、C12 + C13 NEW 追加 reflected)**:
   ...
   - **`[I-D-future-audit-extensions-hardening]` C11 (案 γ Phase 1.5 active priority、I-205 T14-T16 prerequisite)** = Path E `expand_cell_list` Pattern 2/4 hardcoded `<= 30` filter bug fix
   ...
   - `[I-D-future-audit-extensions-hardening]` 残 candidates (C1-C7 + C9 + C10 reduced R4) = L4 latent
   - `[I-D-future-self-applied-symmetry-audit]` = framework v1.9 candidate (L4 latent)
   - `[I-205-retroactive-cell-numbering-section]` = 案 γ Phase 2 T15 batch 化候補
7. **Bootstrap utilities (= post-T1 phase state、framework v1.9 Axes 5/6/7 含む 7 axes full leverage)**:
   - [`scripts/verify_line_refs.py`](scripts/verify_line_refs.py) (Method A)
   - [`scripts/verify_prd_self_audits.py`](scripts/verify_prd_self_audits.py) (Path E、7 axes、11 functional regression tests in `tests/i_d_pre_path_e_test.rs`)
...
### Step 3: 次 action options (= T2 phase 着手 or 案 γ Phase 1.5 C11 fix preempt)

**Option (a): PRD I-D-main Implementation stage T2 phase = rule wording strengthening (13 sub-tasks、案 γ Phase 0 残 work、recommended next step)**:
...
**Option (b): 案 γ Phase 1.5 = TODO C11 fix 直接着手 (= I-205 T14-T16 prerequisite preempt)**:

1. TODO `[I-D-future-audit-extensions-hardening]` C11 entry 参照、Resolution direction R1 (= Pattern 2/4 `<= 30` filter を `<= 99` に修正で Pattern 1/3 consistency 確立) 実施
2. `scripts/verify_prd_self_audits.py` `expand_cell_list` 関数 Pattern 2/4 filter修正 + 新 fixtures (cells N > 30 case)
3. I-205 PRD audit baseline 更新 coordinated implementation
4. Path E utility own correctness improvement = I-D-pre Cell 2 v5-1 latent limit 解消

### Step 4: 後続 prerequisite chain (案 γ Phase 1〜、option (a) 採用時)

PRD I-D-main close 完了後、案 γ Phase 1 (= I-225 → I-162) → 案 γ Phase 1.5 (= C11 fix) → 案 γ Phase 2 (= I-205 T14-T16) chain 着手。詳細 = 下記「実行順序」section + 「次の作業 table」。**Note**: C11 fix は I-205 T14-T16 着手前 mandatory prerequisite、I-225/I-162 完了 OR I-D-main close 後 immediate 着手 candidate。
...
[次] PRD α-3 = I-162 (constructor synthesis `Self::new()` for no-explicit-constructor classes)
   ↓
═════ 案 γ Phase 1.5: TODO C11 fix (= I-205 T14-T16 prerequisite、2026-05-15 /check_problem 由来) ═════
   ↓
[次] **TODO `[I-D-future-audit-extensions-hardening]` C11 = Path E `expand_cell_list` Pattern 2/4 hardcoded `<= 30` filter bug fix** (= Pattern 2 `CELL_STANDALONE_RE` + Pattern 4 `TABLE_FIRST_COL_NUM_RE` の `if 1 <= n <= 30` を `<= 99` に修正で Pattern 1/3 consistency 確立 + 新 fixtures (cells N > 30 case) + I-205 PRD audit baseline 更新 coordinated implementation。**Resolution direction (R1) minimal-invasive structural fix 採用 expected**、scope = `scripts/verify_prd_self_audits.py` + `tests/i_d_pre_path_e_test.rs` + `tests/fixtures/i_d_pre/*` + `backlog/I-205-getter-setter-dispatch-framework.md` baseline 更新 + INV-4 baseline declaration update。**Empirical motivation**: I-205 has cells 1-81 (= 81 cells matrix with gaps)、"Cell 38" / "| 38 |" 等 wordings が Pattern 2/4 で silently skip = audit Axis 1 reporting inflated missing list)。詳細 = TODO `[I-D-future-audit-extensions-hardening]` C11 entry
   ↓
...
| **次優先 (案 γ Phase 1.5、I-205 T14-T16 prerequisite、2026-05-15 /check_problem 由来)** | L3 (Path E utility own correctness) | **TODO `[I-D-future-audit-extensions-hardening]` C11 = Path E `expand_cell_list` Pattern 2/4 hardcoded `<= 30` filter bug fix** | Path E utility own correctness (= Pattern 2 `CELL_STANDALONE_RE` + Pattern 4 `TABLE_FIRST_COL_NUM_RE` `<= 30` filter を Pattern 1/3 consistency `<= 99` に修正 + 新 fixtures + I-205 PRD audit baseline 更新 coordinated)。**Architectural concern**: utility cell number range filter consistency、I-205 T14-T16 着手前 fix 必要 (= I-205 PRD doc audit baseline 更新と coordinated implementation 必要性 = highest priority active TODO item)。詳細 = TODO C11 entry |
| 次優先 (案 γ Phase 2) | L2 | **I-205 T14〜T16** | Class member access dispatch with getter/setter framework 完了 (e2e green-ify + naming refactor)、C11 fix 完了 prerequisite |
...
- **TODO 由来 deferred entries (I-D-pre 起源、L4 latent、案 γ Phase 0 完了後再評価) + 2026-05-15 本 session 由来 elevated active entry**:
  - **`[I-D-future-audit-extensions-hardening]` C11 (NEW 2026-05-15 /check_problem 由来) = 案 γ Phase 1.5 active priority** = Path E `expand_cell_list` Pattern 2/4 hardcoded `<= 30` filter bug fix (= I-205 T14-T16 prerequisite、L3 elevated)。残 candidates (C1-C7/C9/C10 reduced R4) は引き続き L4 latent
  ...
  - `[I-D-future-audit-extensions-hardening]` 残 candidates (C1-C7/C9/C10 reduced R4): audit script extensions structural hardening (L4 latent、C8 IMPLEMENTED via I-D-main v20 cohesive batch、C10 R1+R2+R3 done via L1-1 in-session-fix)
  ...
  - `[I-205-retroactive-cell-numbering-section]`: 案 γ Phase 2 T15 batch 化、I-205 PRD doc に `## Cell Numbering Convention` + `## Spec→Impl Mapping` section 追加 (= audit scope 内自動 promote、C11 fix と同 batch 候補)

...
- **`load_builtin_types` の戻り値**: `Result<(TypeRegistry, SyntheticTypeRegistry)>` に変更済み。`TranspileInput.base_synthetic` で pipeline にシード
- **オーバーロード解決**: `resolve_overload` ヘルパー（5 段階: 単一 → 同一戻り値 → 引数数 → 引数型互換 → フォールバック）。`lookup_method_sigs` で registry lookup を一元化

...
**Phase A〜D の大部分が完了。** 残りは Phase 3（型解決統一、3-1 完了・3-2 以降）〜4、D5（reg パラメータ統合）、Phase E（最終検証）。
...
#### 次に着手すべき作業 — Phase 3-2（`resolve_expr_type` 関連関数の削除）

Phase 3-2: `resolve_expr_type` とその内部関数（heuristic, bin_expr, call_return, new_expr, field_type）を削除する。`resolve_method_return_type` は残す。

詳細: `tasks.type-resolution-unification.md` Phase 3-2 セクション

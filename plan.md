...
**次に着手すべき作業 — D-TR〜D4: 型解決の統一（`tasks.type-resolution-unification.md` 参照）:**

D-TR-1 調査で根本的な設計問題を特定: TypeResolver（pre-pass）と runtime fallback（ExprContext / heuristic / TypeEnv narrowing）が同一機能を並行実装している。TypeResolver の expected_types が 3 パターンしかカバーしておらず、ExprContext が 26 箇所で補完している状態。TypeResolver を完全化し、全 fallback を削除する。

実施順序:
1. D-TR〜D4 (Phase 1〜4): 型解決の統一（詳細は `tasks.type-resolution-unification.md`）

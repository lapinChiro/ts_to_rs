...
1. **[I-211-c](backlog/i-211-c-verification.md)**: 検証 + E2E テスト + ベンチマーク効果測定
...
| 順序 | イシュー | 解消見込み | 理由 |
|---|---|---|---|
| 1 | **I-211: ECMAScript 標準型追加** | 11 件 | ECMAScript 標準型の TypeRegistry 追加。I-211-a/b 完了済み。残り I-211-c（検証 + E2E テスト + ベンチマーク） |
...
- **外部型ローダーの API**: `load_builtin_types` と `load_types_json` の両方が `Result<(TypeRegistry, SyntheticTypeRegistry)>` を返す。`TranspileInput.base_synthetic` で pipeline にシード
...
- **JSON ファイル分割**: `src/builtin_types.json` → `src/builtin_types/web_api.json` + `ecmascript.json`。`web_api.json` の再抽出は不要（JSON は全シグネチャを保持済み、ローダーの `first()` 除去で全シグネチャがロードされる）

### I-211-b 完了済み（設計判断の記録）

- **JSON ファイル分割**: `src/builtin_types/web_api.json`（106 型） + `src/builtin_types/ecmascript.json`（58 型）に分割済み
- **ローダー**: `load_types_into` 関数で既存 TypeRegistry にマージ。ECMAScript → Web API の順で読み込み（後勝ちで Web API 版が優先）
- **抽出スクリプト**: `--ecmascript` フラグで ECMASCRIPT_TYPES ルートリストによるフィルタリング。`libMap` に ES5 + ES2015 サブモジュール 9 エントリを追加

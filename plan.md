...
Batch 4d-A（クリーンアップ）+ T-5（テスト信頼性向上）完了。次は 4d-B でユーティリティ型変換（Partial/Required/Pick/Omit/NonNullable）の二重実装を解消し、type_converter の convert_utility_* を resolve_* への委譲に置き換える。
...
|-------|---------|---------|-------|------|
| **4d-B** | — | **ユーティリティ型変換の統一（convert_utility_* → resolve_* 委譲）** | **L2: 設計** | |
...
| テスト数 | 1899（ユニット 1720 + コンパイル 3 + スナップショット 3 + E2E 84 + integration 89）※Batch 4d-A + T-5 完了時点 |

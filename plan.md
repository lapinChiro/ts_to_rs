...
- **計測結果**: Cluster 1a 11 件のうち **4 件解消** (M, Status, OutputType, U)、残 **7 件**
- **残 7 件の分類** (probe 再投入で確定): 3 サブクラスタに分解
  - **T2.A-i**: 外部 builtin JSON ローダの scope 欠如 — `TNext`, `TResult`, `TResult1`, `TResult2` (4 件)
  - **T2.A-ii**: interface call signature overload merge 漏れ — `E` (1 件)
  - **T2.A-iii**: utility type / typedef resolve 経路の scope 引き継ぎ漏れ — `P`, `S` (2 件、追加 probe 要)
- **進行順序**: T2.A-i → T2.A-ii → T2.A-iii (詳細根拠は `report/i382/master-plan.md` 該当節)
- **cargo test 状態**: 2225 passed, 0 failed

**次タスク**: T2.A-i = 抽出器 (`tools/extract-types`) の signature 型パラメータ抽出追加 → JSON 再生成 → Rust ローダの scope push 実装。

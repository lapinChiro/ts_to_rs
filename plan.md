...
- **計測結果**: Cluster 1a 11 件のうち **10 件解消** (M, Status, OutputType, U, TNext, TResult, TResult1, TResult2, P, S)、残 **1 件** (E)
- **完了済**: T2.A-i (外部 builtin JSON loader scope push), T2.A-ii (TypeResolver パスの scope push, P/S を解消)
- **残 1 件**: T2.A-iv = expected type 経由の free type param 伝搬 (`E` via SSGParamsMiddleware)。別 root cause。
- **cargo test 状態**: 2227 passed, 0 failed

**次タスク**: T2.A-iv の設計検討 (3 案候補は `report/i382/master-plan.md` 参照)。

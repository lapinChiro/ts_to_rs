...
Batch 4d-C 完了。レビューで発見された残存課題を PRD 化（`backlog/4e-typedef-error-propagation-and-type-alias-migration.md`）。調査レポート: `report/post-4dc-review-issues.md`。

タスク順序: T1（resolve_typedef エラー伝播）→ T2（typedef テスト強化）、T3（type_aliases 移行、T1 と並行可能）→ T4（type_aliases テスト強化）→ T5（品質確認）。

**Batch 4e 完了後**: `transformer/functions/params.rs` の `convert_property_signature` 移行（課題 3、小規模タスク）。
...
|-------|---------|---------|-------|------|
| **4e** | — | **resolve_typedef エラー伝播 + type_aliases TsTypeInfo 移行** | **L2: 設計** | |
...
## ベースライン（ベンチマーク計測: 2026-04-04、Batch 4d-C 完了時点）
...
| テスト数 | 1904（ユニット 1725 + コンパイル 3 + スナップショット 3 + E2E 84 + integration 89）※Batch 4d-C 完了時点 |

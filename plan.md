...
## 現在の状態 (2026-04-30 post I-205 T9 Iteration v15 `/check_problem` cleanup + ReceiverCalls struct → enum refactor)

| 指標 | 値 |
|------|-----|
| Hono bench clean | **111/158 (70.3%)** = T7/T8/T9 baseline と同一 (Preservation、`prd-completion.md` broken-fix PRD allowed pattern: Hono が logical compound assign on class instances を主要使用していないため expected) |
| Hono bench errors | **63** (T7/T8/T9 baseline と同一、no new compile errors、本 PRD scope 外への regression 0 件) |
| cargo test (lib) | 3308 pass / 0 fail / 0 ignored (I-205 T9 = 3274 baseline + 34 T9 = 22 v13 + 12 v14 deep-deep。Iteration v15 cleanup は refactor 主体で test 数変化なし、struct → enum dispatch refactor + span chain cleanup + matrix doc 7 cells 追加) |
...
[PRD 2.7: I-198 + I-199 + I-200 cohesive batch — framework Rule 改修 (Rule 3/4/10/11/12) + TypeResolver coverage extension + ast-variants.md Prop/PropOrSpread/Decorator section 新規追加 + audit scripts CI 化] (完了 2026-04-27)
       │
       ▼
[PRD 2.75: I-205 — Class member access dispatch with getter/setter methodology framework (PRD 2.8/2.9/I-201-B prerequisite、Tier 2 broken framework → Tier 1 完全変換、L2、Spec stage v7 final 完了 2026-04-28、Implementation Stage T1〜T9 完了 2026-04-29、T10 (this.x dispatch) ← **次着手**)]
       │
       ▼

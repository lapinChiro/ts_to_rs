...
| 1000行超ファイル | 0 |
| コンパイルテストスキップ | 13 件（builtins なし） / 12 件（builtins あり） |

### エラーカテゴリ内訳（79 件、2026-03-29 B-1完了後実測）

| カテゴリ | 件数 | 関連イシュー |
|----------|------|-------------|
| OBJECT_LITERAL_NO_TYPE | 36 | 複数（詳細: report/object-literal-no-type-investigation-2026-03-28.md）。B-1で+1（intersection修正により新たに露出） |
...
- **B-0c**: 低カバレッジ7ファイルのテスト観点補完（81観点）。7ファイルにユニットテスト追加（テスト数 1286→1367、+81）。CI カバレッジ閾値 89% クリア。超過テストファイル2件を凝集度ベースで分割
- **B-1 (I-221)**: intersection メンバー型の網羅的サポート。前処理（空 TsTypeLit 除去 + TsParenthesized アンラップ + identity mapped type 簡約）+ convert_ts_type 汎用フォールバック + union 分配法則（`A & (B | C)` → enum）+ generator struct variant 対応。INTERSECTION_TYPE 9→0件、全体 87→79（-8エラー）

## 次の開発

### Phase B: 型変換範囲の拡張

Phase B-0（テスト基盤 + メソッドロジック統一）完了。B-1（intersection）完了。次は B-2。
...
**87 → 79（B-1完了: -8）→ 残り B-2〜B-4 で 79 → ~66（-13 見込み）**

...
I-112c Phase 1-3 + I-211 + I-224 実装済み（70→54 件）。残り 54 件を 4 つのイシューに分解。

### 開発順序

| 順序 | イシュー | 解消見込み | 理由 |
|---|---|---|---|
| 1 | **I-266: コンストラクタ引数・関数呼出し引数 expected type** | ~20 件 | `resolve_new_expr` のコンストラクタ引数解決バグ修正 + コンストラクタシグネチャ TypeRegistry 登録 |
| 2 | **I-268: ジェネリクスフィールド展開** | ~14 件 | `E extends Env` の制約型からフィールド展開 |
| 3 | **I-269: Optional スプレッド unwrap** | 4 件 | `Option<T>` → `T` のフィールド展開。I-268 と同じ基盤 |
| 4 | **I-267: return/new 型逆引き** | ~10 件 | コンストラクタ引数は I-266 で解消。残りは戻り値型からの逆引き |

### 依存関係

```
I-266（コンストラクタ引数 expected type）───→ I-267（return/new、I-266 の拡張）

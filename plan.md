...
- Phase A: プロダクションコードのシグネチャ変更
  - 14 ファイル、108 関数に `tctx: &TransformContext<'_>` パラメータ追加
  - `transform_module` / `transform_module_collecting` は公開 API のため内部でデフォルト tctx 生成
  - `src/lib.rs` のエントリポイントも対応済み
  - `cargo check --lib` 0 エラー、`cargo clippy --lib` 0 エラー
  - `clippy.toml`: `too-many-arguments-threshold = 10`

**次に着手すべき作業 — Phase B: テストコード修正:**

テストファイル 7 件、462 箇所のコンパイルエラーを修正する。全て `tctx` パラメータの追加。
...
| `src/transformer/tests.rs` | 6 | `transform_module` 呼び出し修正（シグネチャ変更なし — 公開 API） |
...
- Phase C: FileTypeResolution lookup の実装
  ...
  - C2: `resolve_expr_type` の 30 箇所を lookup に置換

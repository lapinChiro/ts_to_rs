...
1. `backlog/ts-indexed-access-type.md` — 型変換の追加。小規模で独立性が高い
2. `backlog/builtin-api-batch.md` — 組み込み API 10 カテゴリの一括追加。既存パターンの拡張
3. `backlog/spread-non-first-position.md` — 既存機能の拡張。既存テストとの整合性を保つ
4. `backlog/inline-type-literal-param.md` — 変換パイプラインの拡張が必要。設計コストが中程度
5. `backlog/conditional-type-tier1.md` — 新しい型構文カテゴリ。IR 拡張を含む大きめの作業
6. `backlog/conditional-type-tier2-fallback.md` — Tier 1 の実装が前提

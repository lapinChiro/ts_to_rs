...
### Phase 9B (P9.2) 完了、P9.3-P9.4 待ち

**次: P9.3 (type substitution) + P9.4 (select_overload Stage 2 修正)**
...
### 現在の状態 (2026-04-13 Phase 9B 完了時)

- **Test count**: 全テスト pass (lib 2366, integration 95, compile 4, E2E 88)
- **Quality**: clippy 0, fmt 0
- **Compile test skip**: callable-interface 系は全て復帰済。async-class-method も復帰済。
  error-case fixture `callable-interface-generic-arity-mismatch` を skip 追加済
- **#[allow(dead_code)]**: production code に 0 件
- **INV-6**: `unwrap_promise_and_unit` / `unwrap_promise_type` 削除済。
  Promise unwrap は `RustType::unwrap_promise()` に統一
- **INV-2 lint**: helpers.rs の violation は `compute_widest_signature` の引数として
  正当な `call_signatures` 参照 (false positive)。lint script exit code 変更は保留

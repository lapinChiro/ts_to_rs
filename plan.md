...
**Batch 4: I-312（registry 責務逸脱）**

TypeDef が RustType を保持 → registry フェーズに型変換ロジックが漏れる構造的問題を解消する。
...
| ~~3~~ | ~~I-319+I-343~~ | ~~配列インデックス安全化（S1 panic 排除）~~ | — | **完了** |
| ~~3b~~ | ~~I-346~~ | ~~safe indexing downstream unwrap（テストインフラ復旧）~~ | — | **完了** |
| **4** | **I-312** | **registry 責務逸脱（TypeDef のTS型メタデータ分離）** | **L2: 設計** | **← 次** |
...
| テスト数 | 1748（ユニット 1569 + コンパイル 3 + スナップショット 3 + E2E 84 + integration 89） |

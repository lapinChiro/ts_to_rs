...
**Phase A〜D の大部分が完了。** 残りは Phase 3-7（`ast_produces_option` 削除）→ Phase 4（TypeEnv 簡素化）→ D5（reg パラメータ統合）→ Phase E（最終検証）。
...
- Phase 3-5: `set_expected_types_in_nested_calls` 廃止（3-1 の step 4 追加で冗長化済み）
- Phase 3-6: `type_env` パラメータの部分的除去（`convert_ts_as_expr` インライン化）

#### 次に着手すべき作業 — Phase 3-7（`ast_produces_option` 削除）

詳細: `tasks.type-resolution-unification.md` Phase 3-7 セクション

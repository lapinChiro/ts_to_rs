...
### 次: Phase 7 (Trait delegate impl)

**Phase 7 開始前に設計判断が必要** (PRD Phase 7 セクション参照):
inner fn body の return 式を variant wrap するアプローチで、`Expr::Ident` の型を
TypeResolver の `get_expr_type` (span ベース) で取得できるか empirical に検証する。
検証結果に基づいて inner fn return type + delegate impl の設計を確定。

P7.1 delegate method + inner 呼び出し → P7.2 inner arg wrapping →
P7.3 async 伝搬

### 現在の状態 (2026-04-12 session 終了時)

- **Test count**: 全テスト pass (lib 2350+, integration 93, compile 3, E2E 88)
...
- **#[allow(dead_code)]**: overloaded_callable.rs, return_wrap.rs,
  arrow_fns.rs (wrap_body_returns/wrap_expr_tail),
  mod.rs (return_wrap_ctx/spawn_nested_scope_with_wrap) — Phase 7-9 で使用開始時に削除

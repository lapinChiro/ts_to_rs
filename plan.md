...
D-2-A〜E + F-0〜F-1 完了。残り: D-2-F（F-2〜F-8）→ G → H → I。

- **D-2-F**: F-1（expressions/ ラッパー全削除 + ヘルパーメソッド化 + F-3b #2,3,5,6,7 対応）完了。残り: F-2（statements/ ラッパー削除）→ F-3（functions/ ラッパー削除）→ F-3b #1,4,8（残りのサブ Transformer 書き換え）→ F-4（classes/ ラッパー削除）→ F-5〜F-8（mod.rs ラッパー削除 + pipeline 更新 + 検証）

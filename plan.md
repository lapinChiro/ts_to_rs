...
#### B-0a: テスト基盤整備（I-287 + I-288 + I-289）

| ID | 項目 | 放置した場合の影響 |
|---|------|-----------------|
| I-289 | コンパイルテスト builtins あり | **ビルトイン依存 fixture の回帰検出不能**。Phase B の I-221 でビルトイン型参照パターンを変換してもコンパイル検証できない |
| I-288 | TypeResolver ビルトイン型テストヘルパー | ビルトイン型メソッド関連の TypeResolver unit test がなく、Vec→Array マッピング等の挙動を 31 メソッド全体で検証できない |
| I-287 | integration_test.rs マクロ化 | I-289 で builtins バリアント追加時、マクロ化されていれば全テストの移行が容易 |

3 つは同じファイル群（`tests/`）を修正するためバッチ化。

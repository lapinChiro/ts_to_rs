...
| 指標 | 値 |
|------|-----|
| Hono クリーン | 104/158 (65.8%) |
| エラーインスタンス | 91 |
| コンパイル(file) | 98/158 (62.0%) |
...
OBJECT_LITERAL_NO_TYPE: I-112c Phase 1-3 + I-211 + I-224 + I-266 + I-268 + I-269 で 70→48 件に削減済み。I-195 修正で 48→51 件に増加。I-194+I-35 後 50 件。
I-281 で INDEXED_ACCESS 6→3、TYPE_ALIAS_UNSUPPORTED 13→10（合計 106→100、-6件）。
I-267 Phase A（TypeRegistry merge バグ修正）で OBJECT_LITERAL_NO_TYPE 50→41（合計 100→91、-9件）。Phase B（フォールバック空オブジェクト）で -1件。

## 次の開発: エラーインスタンス効率削減（91→<80 目標）

根本的・基盤的な修正を先行し、波及効果で全体コストを最小化する。

| 順序 | イシュー | 直接削減 | 波及効果 | 概要 |
|------|---------|---------|---------|------|
| 1 | I-281 | ~~6件~~ 6件（実測） | I-200 の前提充足、keyof/typeof 基盤 | typeof ローカル変数の型解決（TypeCollector 拡張）✅ 完了 |
| 2 | I-267 | ~~10件~~ 10件（実測: merge バグ9+フォールバック1） | — | OBJECT_LITERAL_NO_TYPE 削減 ✅ 完了 |
...
**合計削減見込: 91 → ~70（残り -21件、I-221/I-285/I-200/I-284）**

...
1. Transformer のプロダクションコードに `convert_expr_with_expected` の呼び出しが存在しない（`convert_expr` のみ使用）
2. `convert_expr_with_expected` 関数が削除されている（または `#[cfg(test)]` のみ）
...
- `TctxFixture` に TypeResolver 経由の構築メソッドが追加されている
- 新メソッドを使用したテストが 3 件以上ある（既存テストの書き換え）
...
- `convert_expr_with_expected` を `Some(&expected)` で呼ぶテストが存在しない
- 全テストが TypeResolver 経由の expected type、または FileTypeResolution への手動挿入で動作する
- テストの検証内容（assert）は変更しない
...
- Transformer プロダクションコードに `convert_expr_with_expected` の呼び出しが存在しない
- `convert_expr_with_expected` が削除されている（またはテスト専用）
- 全テスト GREEN
...
#### 2.5-D-2: `convert_expr_with_expected` の削除

**ファイル**: `src/transformer/expressions/mod.rs`

**修正**: `convert_expr_with_expected` 関数を削除。`convert_expr` が直接 `tctx.type_resolution.expected_type(span)` を読むようにする（現在 `convert_expr_with_expected` 内部で行っているロジックを `convert_expr` に統合）。

**検討**: Option\<T\> の unwrap + Some 付与ロジック（mod.rs:84-100）は `convert_expr` に残す必要がある。これは expected type の「読み取り」であり「伝搬」ではないため。
...
- `convert_expr_with_expected` は存在しない

...
| `scripts/verify_line_refs.py` | exists | — | 264 | 2026-05-10 (Iteration v12 bootstrap) | ✓ verified (Method A utility、formal lock-in target) |
| `scripts/verify_prd_self_audits.py` | exists | — | 368 | 2026-05-10 (Iteration v16 bootstrap) | ✓ verified (Path E utility 4 axes、formal lock-in + F6/F7 fix target) |
| `scripts/audit-handoff-doc-line-refs.py` | NOT EXISTS | — | — | — | ✗ to be created (Cell 3 / v11-5) |
...
- **/check_job deep deep review (2026-05-11) で initial review が missed の妥協発見**:
  ...
  - **NEW Implementation gap I5 (Medium、別 PRD candidate)**: Closed PRD test 不在 (= `is_active_prd == False` で新 functions invoke されない動作の明示 verify)。既存 audit logic test 責務 = Phase 3 scope outside、TODO `[I-D-future-audit-extensions-hardening]` C4 candidate として後続追加検討
...
- **Convergence criterion application**: 本 v3 entry = Phase 3 着手前 design decision lock-in + Phase 3 完了 + /check_job 4-layer review (= I1 fix) + /check_job deep deep review (= I3 + I4 即時 fix、I5 別 PRD candidate) retroactive embed (= 8 findings classified、I1+I3+I4 fix 済 / I2+I5 別 PRD / R1-3 別 PRD)、Critical = 0 + High = 0 維持
- **Spec stage 完了判定**: ✓ Phase 3 完了 (= 3 NEW audit functions + helper + formatter + proper top-level library import (= sys.path.insert + `# noqa: E402` 排除) + DRY 達成 + 6 tests (= True branch 4 + False branch 1 + dual verify 1) + 7 fixtures (= positive 3 + negative 3 + out-of-scope 1) + INV-4 baseline preserve + /check_job 4-layer + deep deep review pass + Action Items L4 latent TODO lock-in)、Phase 4 (= `scripts/audit-handoff-doc-line-refs.py` 新設 + CI integration) 着手 ready。Final third-party `/check_job` re-invocation は Phase 6 T7-pre で実施

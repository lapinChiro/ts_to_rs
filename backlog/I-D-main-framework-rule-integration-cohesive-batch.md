...
| `scripts/audit-prd-rule10-compliance.py` | exists | 79933 (~1811 行) | 2026-05-15 (I-D-main T1 phase 完了 + /check_job Round 1+2 review fix + /check_problem G-1 fix で T1-5 non-tuple arm counting bug structural fix = generic_arm_pattern で all non-`_` arms count、count-based fallback path coverage 達成、INV-4 3-tuple baseline preserve、+35322 bytes / +778 行 cumulative drift sync。pre-T1-1 baseline 44451 / 1033 行は historical preserve、Iteration v20 cascade sync log entry) | ✓ verified (41 top-level `^def` declarations = 26 `verify_*` audit functions + 13 helpers/utilities + 1 `main()` + 1 `_format*` formatter helper、I-D-main T1 phase commit で empirical sync) |
...
3. **Existing PRD docs compliance maintenance**: 本 PRD で establish する新 audit verify mechanisms に対し、active backlog/ PRD docs (= I-050 baseline FAIL preserve / I-205 PASS / I-D-main PASS = **3-tuple post-close INV-4 baseline**、Path B split 後 I-D-pre は close で active backlog/ scope 外、close-deleted PRD は audit out-of-scope) の compliance 維持 verify run で structural integrity 維持
...
### INV-4: Existing PRD docs compliance preservation (delta-based regression lock-in、3-tuple post-close baseline post Path B split + I-D-pre close)

- **(a) Property statement**: 本 PRD で establish する新 audit verify mechanisms を active backlog/ PRD docs (= backlog/I-050-any-coercion-umbrella.md + backlog/I-205-getter-setter-dispatch-framework.md + backlog/I-D-main-framework-rule-integration-cohesive-batch.md、closed PRDs (= I-D-pre 含む) は close 時 file 削除されて active backlog/ scope 外 = audit target excluded) に対し run、**delta-based regression 0** (= pre-I-D-main baseline state を preserve、新 audit functions が active backlog/ PRD docs を新たに invalid 化しない)。**Active backlog/ baseline (Path B split 2026-05-11 で 3-tuple → 4-tuple intermediate state を経て、2026-05-11 I-D-pre close で 3-tuple post-close に最終化、Iteration v20 で sync)**: I-050 = FAIL (legacy partial-framework umbrella、`## Rule 10 Application` heading 不在 = pre-existing) / I-205 = PASS / I-D-main (本) = PASS。本 PRD 完了時の post-I-D-main state も同 baseline preserve (= I-050 baseline failure 不変、I-205 + I-D-main PASS 維持)
...
- **(c) Verification method**: 本 PRD T6 task で active backlog/ PRD docs に対する audit run + delta-based regression 0 確認、CI で active backlog/ 全 PRD doc に対する audit を merge gate 化。test fn `test_invariant_4_existing_prds_baseline_preservation` (`tests/i_d_main_invariants_test.rs`) で **baseline-aware assertion**: I-050 = pre-existing FAIL state preserve (= audit script exit code 1 + violation message が "missing `## Rule 10 Application` heading" 一致) + I-205 = PASS preserve (= exit code 0) + I-D-main = PASS (= exit code 0) を **3-tuple post-close assertion logic** で verify (= I-050 baseline failure を test loop から exclude with "pre-existing baseline annotation" 方式、Iteration v6 F7 fix base + Path B split 2026-05-11 拡張 + I-D-pre close 2026-05-11 で 3-tuple final、Iteration v20→v22 systematic sync)
- **(d) Failure detectability**: audit script fail (= 既存 PRD doc が新 verify mechanism で reject、regression detect)
- **(e) Path E utility own correctness independence note (PRD I-D-c11 close 2026-05-18)**: 本 INV-4 baseline 3-tuple は **`scripts/audit-prd-rule10-compliance.py` のみ track**、Path E utility (`scripts/verify_prd_self_audits.py`) result は INV-4 baseline tuple に **direct 影響しない independent dimension**。PRD I-D-c11 (= Path E utility `expand_cell_list` Pattern 2/4 hardcoded `<= 30` filter fix to `<= 99` uniform) close 後の Path E utility own correctness improvement で I-205 PRD doc 自身の Path E result Axis 1 missing list は acknowledged pre-existing state を preserve (= I-205 PRD doc の Scope / Spec→Impl Mapping sections に Cell # 1-81 全 enumerate 不在 = TODO `[I-205-retroactive-cell-numbering-section]` per 案 γ Phase 2 T15 batch、本 INV-4 baseline は audit-prd-rule10 dimension のみで preserve)。Path E utility own correctness ceiling improvement は I-D-c11 で達成 (= filter uniform `<= 99`、regex `\d{1,2}` upper-bound と integral consistency)、本 INV-4 baseline preservation 不変。
...
**Approach**:
- `.claude/rules/spec-stage-adversarial-checklist.md` (461 行 post 2026-05-12 Rules 改善 batch / 13 rules) に Rule 5/6/8/9/10/13 sub-rule 拡張 = 6 rules 改修 (= cell 28 v13-5 Rule 9/13 cell numbering convention は I-D-pre Cell 5 で完成、本 I-D-main は他 sub-rule のみ extend)
- `.claude/rules/check-job-review-layers.md` (346 行 post 2026-05-12 Rules 改善 batch / 4 layers) に Layer 3/4 sub-rule 拡張 = 2 layers 改修 (= cell 19 v11-7 Layer 1 factual accuracy semantic check は I-D-pre Cell 4 で完成、本 I-D-main は Layer 3/4 sub-rule のみ extend)
...
- `.claude/rules/spec-stage-adversarial-checklist.md`: 461 行 (post 2026-05-12 Rules 改善 batch = Versioning section 削除 + PRD-agnostic 化 + sub-rule numeric 統一 等で -57 行) → ~620 行見込み (= Rule 5/6/8/9/10/13 sub-rule 拡張、Versioning section は git history へ delegation 済のため新 entry 不要)
- `.claude/rules/check-job-review-layers.md`: 346 行 (post 2026-05-12 Rules 改善 batch = +8 行 net) → ~480 行見込み (= Layer 3/4 sub-rule 拡張、Layer 1 sub-step (4) factual accuracy は I-D-pre Cell 4 完成)
...
**T1 共通 completion criteria** (Path B split 後): 全 10 sub-tasks 完了 + (= 9 new verify functions in `audit-prd-rule10-compliance.py` + 1 existing function strengthening = 10 total audit改修、I-D-pre 5 sub-tasks = T1-pre-1/2/3a/3b/4 が別 PRD scope) PASS for 本 PRD doc + active backlog/ PRDs に対し **INV-4 baseline-aware delta-based regression 0** (= I-050 = pre-existing FAIL state preserve、I-205 + I-D-main = exit code 0、**3-tuple post-close INV-4 spec satisfy** = Path B split 後 I-D-pre close で audit out-of-scope)。**Path B split 2026-05-11 cascade sync + I-D-pre close 2026-05-11 + Iteration v22 baseline sync**: I-D parent v10 F2 fix の "12 new + 1 strengthening = 15 total (incl. NEW script + CI)" を本 I-D-main scope reduce で "9 new + 1 strengthening = 10 total" に再 systematic sync (Path B split 由来 5 sub-tasks I-D-pre migration excluded)、4-tuple intermediate state → 3-tuple post-close final state に v22 で final sync (= v21 F1 finding 由来 systematic correction)
...
- 対応 test contract `tests/i_d_main_rule_wording_test.rs::<test_fn>` を grep-based assertion で実装
...
- **Work**: 本 PRD T1-T5 で establish した新 audit verify mechanisms に対し、active backlog/ 全 PRD docs (= `backlog/I-050-any-coercion-umbrella.md` + `backlog/I-205-getter-setter-dispatch-framework.md` + 本 PRD doc) を audit run、**INV-4 baseline-aware delta-based regression 0** 確認 (= I-050 = pre-existing FAIL state preserve、I-205 = exit code 0 preserve、I-D-main = exit code 0、I-D-pre は close 後 audit out-of-scope、3-tuple post-close baseline assertion、INV-4 evidence)。違反発見時の判定 = (a) I-050 が pre-existing FAIL state を維持 (= violation message が `missing '## Rule 10 Application' heading` 一致) → baseline preserve PASS / (b) I-050 が新 violation type で FAIL transition、または I-205 / I-D-main が exit code 1 に regress → **delta-based regression**、本 PRD scope 内 fix mandatory / (c) その他 (= 新 active PRD doc 追加で baseline 拡張) → INV-4 spec 更新で対応
- **Completion criteria**: `for prd in backlog/*.md; do python3 scripts/audit-prd-rule10-compliance.py "$prd"; done` の **delta-based regression 0** 達成 (= I-050 baseline FAIL state 維持 + I-205 + I-D-main exit code 0、3-tuple post-close INV-4 spec satisfy = Path B split 後 I-D-pre は close で active backlog/ scope 外、active 3-tuple = {I-050, I-205, I-D-main}。**Iteration v8 F3 fix で baseline-aware 化 + Iteration v20 で Path B split sync (= I-D → I-D-main rename + I-D-pre close)**: 旧 wording "全 exit code 0" は INV-4 (a) baseline-aware spec と direct contradict だったため、3-tuple post-close assertion logic に correct)、CI で active backlog/ 全 PRD doc に対する audit を merge gate 化 (= `.github/workflows/ci.yml` integration verify、INV-3 evidence)
...
4. **Existing PRD docs compliance preservation (INV-4 baseline-aware delta-based regression 0、Path B split 後 + I-D-pre close 後 3-tuple post-close)**: active backlog/ 全 PRD docs (= {I-050, I-205, I-D-main}、closed PRDs (I-D-pre 含む) は audit out-of-scope) に対する新 audit verify mechanisms run が **3-tuple post-close baseline assertion satisfy** (= I-050 = pre-existing FAIL state preserve [violation message `missing '## Rule 10 Application' heading` match] + I-205 = exit code 0 preserve + I-D-main = exit code 0 = 3-tuple post-close INV-4 spec satisfy、INV-4 evidence、Iteration v22 で final sync)

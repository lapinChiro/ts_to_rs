...
//   - has_top_level_await: bool (= Axis C1 partition、module top-level に到達可能な
//     **任意の Expr::Await sub-node 存在** で true。Spec stage 逆戻り (2026-05-07、I-228 fix)
//     により AST shape direct → recursive walk に extend = top-level Stmt::Expr の expr tree
//     全体 / Decl::Var.init expression tree (全 declarator の init) / ExportDecl-wrapped
//     Decl::Var.init を `Visit::visit_await_expr` で walk、nested function bodies は skip
//     (= 関数内部の await は別 async context、本 axis trigger に contribute しない)。
//     旧 spec "Stmt::Expr(Expr::Await) or Decl::Var with Expr::Await init" は AST shape 直接
//     のみ enumerate していたため `console.log(await fetch())` 等 nested await を miss、
//     T3 sync `fn main` 誤選択で Rust E0728 compile error の原因。`is_executable_mode` 同様
//     に ExportDecl-wrapped Decl::Var with await init / multi-declarator with await init も
//     正しく detect する。)
//
...
///
/// **Init kind classification 仕様** (Spec stage 逆戻り 2026-05-07、I-228-b/d fix を反映):
/// VarDecl の **全 declarator** をiter で walk し、以下の precedence で集約 (= ANY-rule):
/// - `InitKind::AwaitInit`: 任意の declarator の init が `Expr::Await(_)` を含む (= Axis C1
///   partition の Decl::Var)。recursive walk で nested Await も検出 (= I-228 main fix と
///   一致)。awaitable は executable trigger、async dispatch trigger 双方を発火。
/// - `InitKind::SideEffect`: 任意の declarator の init が Lit でも Unary-Lit でもない
///   一般 expression (= Call / Ident / New / Member etc.、Axis A3 partition trigger)。
///   `Lit::Regex` は Rust 上 `Regex::new("...")` runtime call = const 不可能のため SideEffect 扱い
///   (I-228-b fix、design intent "Rust `const` 適合" との整合)。
/// - `InitKind::Lit`: 全 declarator の init が以下のいずれか (Rust `const` 適合):
///     - `Expr::Lit(Lit::Num/Lit::Bool/Lit::Str/Lit::Null/Lit::BigInt)` (= Lit::Regex / JSXText 除外)
...
/// - None init (`let x;`) は callers の defensive guard で filter (= classify_decl_var_path /
///   has_side_effect_init で precondition violation を avoid)
///
/// **Multi-declarator semantics (I-228-d fix)**: 旧 spec `var.decls.first()` only check は
/// `const a = 1, b = compute();` 等 mixed init を Lit と誤分類 → ToplevelConst routing →
/// `pub const b = compute();` Rust compile error の原因。新 spec は ANY-rule で全 declarator
/// が check 対象、ANY が SideEffect/AwaitInit なら全 declarator が FnMainBodyCapture (= 全て
/// fn main body 内 `let` として capture、Lit declarator も local binding に変化、`pub const`
/// 化を放棄するが TS module-load semantic と Rust execution semantic は byte-exact 等価)。
fn classify_init_kind(var: &VarDecl) -> InitKind {
    let mut has_await = false;
    let mut has_side_effect = false;
    for decl in &var.decls {
        let Some(init) = decl.init.as_deref() else {
            // None init = caller precondition violation (= declare-marked Var or `let x;`),
            // filtered by has_side_effect_init / classify_decl_var_path defensive guards.
            continue;
        };
        ...
        match init {
            ...
            _ => has_side_effect = true,
        }
    }
    if has_await { InitKind::AwaitInit }
    else if has_side_effect { InitKind::SideEffect }
    else { InitKind::Lit }
}
...
| `ModuleDecl::Import/Export*` (Lit init / no init / type-only export) | 既存 path 維持 (Axis E E1 partition、`pub` modifier preserve) | 同上 (executable mode でも Module export presence は orthogonal、INV-5 整合 = `__ts_main` rename target は private fixed) |
| `ModuleDecl::ExportDecl(Decl::Var)` with side-effect / await init (Spec stage 逆戻り 2026-05-07、I-228-c fix) | (unreachable per `is_executable_mode` definition: side-effect init は executable trigger、ExportDecl wrap 内も recursive scan 対象) | `DeclVarPath::FnMainBodyCapture` = `MainStmt::Let` / `LetAwait` に capture、`pub` modifier は **drop** (= TS export semantic は Rust 単一 binary 出力では cosmetic、source-order preservation + side-effect runtime evaluation を優先)。User が export 公開を維持したい場合は `function init() { return compute(); } export const c = init();` ではなく `function init(): number { return compute(); } export const c = 0;` (= Lit init で型形だけ宣言) 等の refactor が必要。Spec gap origin: I-228-c (= `export const c = compute();` を旧 spec で library mode 誤分類 → Rust E0265 / E0277 compile error) |
...
#### T1-3: INV-5 invariants test fill-in + 4-layer review ✓ (完了 2026-05-07)
...
- **Result (2026-05-07)**: 11 reachable B4 cells (matrix # 9/19/20/29/39/40/49/59/69/79/80) を table-driven integration test として実装、各 cell で (1) `transpile()` Err with `__ts_main` + `is reserved` wording assert + (2) `transpile_collecting()` `unsupported[0]` collision wording assert (= structural priority lock-in)。**Cells 49/59/69 priority sub-check** で A-axis 代替 reject (`Stmt(If(`、`Stmt(Empty(`、`Stmt(Debugger(` envelope 込み substring) が unsupported list 内に存在するが index 0 ではない (= **non-trivial collision arm priority empirical proof**)。**Cell 59 forward-compat note**: 現在 (post T1-2 / pre T2/T3) Stmt::Empty は `transform_module_item` 末尾の `_ => Err(format_module_item_kind(...))` arm で reject、alternative wording = `Stmt(Empty(`。T4-2 で A5a silent-skip 実装が landed すると alternative 消失 → 当該 alternative-reject 行を vacuous-priority 形式 (`unsupported len == 1` 検出) に切替する intended test evolution。Production code 0 LOC change (test-only)、cargo test --lib 3372 pass / cargo test full 24 binaries all green / cargo fmt 0 diff / cargo clippy 0 warning / check-file-lines OK (test file 428 行)。**`/check_job` 4-layer self-applied review (敵対的 deep review)**: 初回 review で Layer 3 / Layer 4 finding = cell 59 を `cells_with_alternative_a_axis_reject` から除外していた compromise (= forward-compat 名目で structural strength を犠牲にしていた、`ideal-implementation-primacy.md` 違反) を発見 → empirical probe で current state cell 59 unsupported[1] = `Stmt(Empty(EmptyStmt {...})` を構造的 lock-in 可能と確認 → cell 59 を non-trivial priority sub-check に追加 + cell 49/69 substring を `Stmt(<Variant>(` envelope 込み form に tighten + docstring に forward-compat note 追加で **structural strengthening fix**。Re-review で Layer 1 Mechanical / Layer 2 Empirical (11 cells × 2 API verify) / Layer 3 Structural cross-axis (全 A-axis A0-A6 + 全 reachable C-axis C0+C1 直交 enumerate) / Layer 4 Adversarial trade-off (test-only、no production trade-off、structural fix) 全 **0 findings**。**Defect Classification 5 category (final)**: Grammar gap / Oracle gap / Spec gap = 0、Implementation gap = 1 (= self-applied fix で resolved)、Review insight = 1 (= L1 substring tightening、self-applied fix で resolved)。

### T2: `MainStmt` IR + `UserMainKind` enum + `collect_top_level_executions` helper (3 sub-tasks bundled into 1 commit per user instruction 2026-05-07)

✓ **T2 完了 (2026-05-07)**: User 指示「次のコミットは T2 完了時とします」により T2-1 / T2-2 / T2-3 を 1 commit に bundle (PRD 当初は 3 sub-commits decomposition だったが user override で集約)。

#### T2-1: IR enums + predicates 新規 (src/transformer/main_synthesis/ 新規 directory module) ✓

- **Work**: 新規 directory module `src/transformer/main_synthesis/{mod.rs, tests.rs}` 作成 + `MainStmt` enum (4 variants: `Expr(IrExpr)` / `ExprAwait(IrExpr)` / `Let { name, init }` / `LetAwait { name, init }`) + `UserMainKind` enum (5 variants: None / FnSync / FnAsync / NonFn / Collision) + `InitKind` enum (3 variants: Lit / SideEffect / AwaitInit) + `DeclVarPath` enum (3 variants: LibraryMode / ToplevelConst / FnMainBodyCapture) + `DispatchArm` enum (13 variants for Rule 9 (a) 1-to-1 mapping) + `classify_init_kind` / `has_side_effect_init` / `classify_decl_var_path` predicates with **declare/no-init defensive guards** + `is_executable_mode(module: &Module) -> bool` predicate (Stmt 全 variants explicit enumerate per Rule 11 (d-1) self-applied compliance) + `has_top_level_await` AST-only predicate (DRY counterpart of `collect_top_level_executions`'s flag) + `detect_user_main` + helpers (`detect_user_main_from_decl` / `detect_user_main_from_default_decl` / `classify_main_identifier` / `DeclShape`、Rule 11 (d-1) compliant Decl 全 variants enumerate) + `classify_dispatch_arm(is_executable_mode, user_main_kind, has_top_level_await) -> DispatchArm` (13 reachable arms + 4 structurally-unreachable arms via `(false, _, true)` exhaustive match per `tests/swc_parser_top_level_await_test.rs` AST-level mutual exclusion lock-in)。
- **Completion criteria** (achieved): cargo check pass、`audit-ast-variant-coverage.py --files src/transformer/main_synthesis/mod.rs` で `_` arm violation 0 件 (Rule 11 (d-1) self-applied compliant)、まだ呼び出し不在で behavioral change なし。
- **Result**: 850-line `mod.rs` + 631-line `tests.rs` (file-line check OK、< 1000 threshold)。`pub mod main_synthesis;` 公開 (external integration test 経由 verify access、6 items pub: UserMainKind / DispatchArm / classify_dispatch_arm / is_executable_mode / detect_user_main / has_top_level_await; 4 items pub(crate) with per-item `#[allow(dead_code)]` for T3/T4 staging: MainStmt / DeclVarPath / classify_decl_var_path / collect_top_level_executions、TS_MAIN_RENAME precedent pattern)。

#### T2-2: collect_top_level_executions shared helper + 64-test representative unit tests + dispatch arm 1-to-1 mapping integration test ✓

- **Work**: `Transformer::collect_top_level_executions(&mut self, module: &Module) -> Result<(Vec<MainStmt>, UserMainKind, bool)>` shared helper 実装 (Rule 11 (d-1) Stmt 全 variants explicit enumerate、ambient/no-init Var defensive skip、`let-else + unreachable!` macro pattern for `.expect()` rule compliance per testing.md "unwrap()/expect() only in test code")。`tests/i224_helper_test.rs::test_dispatch_arm_one_to_one_mapping_per_in_scope_cell` を `#[ignore]` 解除 + fill-in (43 reachable cells × DispatchArm 1-to-1 mapping × 13-variant DispatchArm coverage sanity)。
- **Completion criteria** (achieved): 64 unit tests in `src/transformer/main_synthesis/tests.rs` 全 pass (Equivalence partitioning + Boundary value + Decision Table + AST variant exhaustiveness + 4 #[should_panic] for unreachable arms + 80-cell representative collect_top_level_executions cells)、`test_dispatch_arm_one_to_one_mapping_per_in_scope_cell` green。
- **Result**: cargo test --lib transformer::main_synthesis = 64 pass / 0 fail / 0 ignored。

#### T2-3: INV-3 (C0 partial) + INV-6 invariants test fill-in + 4-layer review ✓

- **Work**: `tests/i224_invariants_test.rs::test_invariant_3_sync_async_dispatch_consistency_4_subcases` C0 partial fill-in = Trigger 1 only (B2+C0、6 cells: 5/15/25/35/55/75) + Sync no-trigger (11 cells: 3/11/13/17/23/31/33/37/71/73/77) で is_async_required = (user_main_kind == FnAsync) || has_top_level_await の computation verify (C1 cells deferred to T8)。`test_invariant_6_type_resolver_layer_unaffected` fill-in = main_synthesis/mod.rs source content audit で 7 forbidden tokens (TypeResolver / type_resolver / FileTypeResolution / type_registry / expr_type / narrowing / expected_type) 不在を structural verify (= INV-6 (c) verification (2) sufficient condition、(1) cargo test subprocess invoke は build directory deadlock risk のため代替に main test suite の global green state を rely)。
- **Completion criteria** (achieved): 2 invariants test green、Layer 1-4 全 0 findings (post .expect→unreachable! refactor、initial review で 1 finding 発見 → self-fix で resolved)。
- **Final quality**: cargo test 3774 pass / 0 fail / 86 ignored / cargo fmt 0 diff / cargo clippy --all-targets --all-features -- -D warnings 0 warning / check-file-lines OK。
- **`/check_job` 4-layer self-applied review (final state)**:
  - **Layer 1 (Mechanical)**: TODO/FIXME 0 / unwrap/expect/panic in production 0 (post-fix) / clippy 0 / fmt 0 diff / test name + assertion message 適切。
  - **Layer 2 (Empirical)**: 64 unit tests + 1 helper test + 2 invariants tests 全 pass。Hono bench Preservation (production 0 LOC change for transform_module path、main_synthesis is staging foundation)。
  - **Layer 3 (Structural cross-axis)**: Axis A (A0/A1/A2/A3/A4/A5a/A5b/A6) + Axis B (B0/B1/B2/B3/B4) + Axis C (C0/C1) + Init shape (Lit/SideEffect/AwaitInit) + Decl variant (Fn/Var/Class/Interface/TypeAlias/Enum/Module/Using) + DispatchArm (13 reachable + 4 unreachable) 全 dimensions 直交 enumerate。Spec gap 0 (PRD Design section #2 + #3 spec を 1-to-1 reflect)。
  - **Layer 4 (Adversarial trade-off)**: pre/post matrix = 全 reachable cells が DispatchArm leaf に 1-to-1 mapping (Rule 9 (a) compliant)。Trade-off statement: 4 items per-item `#[allow(dead_code)]` for T3/T4 staging (TS_MAIN_RENAME precedent、structural foundation pattern、not interim patch)。
- **`/check_job` adversarial deep review **2 round** (2026-05-07)**:
  - **Round 1 (5 findings → 4 self-fix + 1 TODO defer)**:
    - L1-1 (self-fix): 6 public items に `#[doc(hidden)]` 付与 (internal API public exposure 明示)
    - L1-2 (self-fix): INV-6 forbidden_tokens に `type_resolution` 追加 (TransformContext field accessing FileTypeResolution)
    - L2-1 (TODO defer): `has_top_level_await` AST shape direct interpretation が nested top-level await (`f(await x)` 等) を miss、Spec gap (= framework 失敗 signal) → TODO `[I-228]` 起票
    - L2-2 (self-fix Critical): `detect_user_main` が `declare`-marked decls を filter せず → `is_ambient_decl(decl: &Decl)` helper 新設 + 9 NEW unit tests
    - L3-1 (defer rationale): 12 orthogonality-merged out-of-scope cells (A4/A5a/A5b + B0..B3) は dispatch tree に到達しない dead path、PRD mapping table も in-scope cells のみ enumerate、追加 test value なし
  - **Round 2 (3 additional findings → 3 self-fix + 3 TODO sub-entries)**:
    - L1-3 (self-fix): INV-6 forbidden_tokens の `"narrowing"` が一般語 ("Lit narrowing the match" 等) で false-positive trigger 発覚 → specific identifier `"narrowed_type"` + `"narrowing_analyzer"` に narrow + main_synthesis docstring section title を "Lit variant filtering" に rename
    - L2-3 (self-fix Critical): `classify_init_kind` の `Some(Expr::Lit(_)) => InitKind::Lit` が PRD design intent ("Rust `const` 適合"、line 970 prefix) と矛盾 (Lit::Regex は Rust 上 `Regex::new()` runtime call = const 不可、PRD bullet enumeration line 970-971 の Regex inclusion は intra-PRD inconsistency) → match を `Lit::Num \| Bool \| Str \| Null \| BigInt` に narrow + Regex/JSXText を SideEffect routing per design intent + 2 NEW tests (Regex → SideEffect + `#[should_panic]` precondition lock-in)
    - 3 sibling Spec gap candidates (TODO `[I-228]` sub-entries 集約):
      - **I-228-b**: PRD InitKind::Lit definition の Lit::Regex 包含 inconsistency (本 T2 R2 で design-intent narrow fix 適用、PRD spec cleanup は Spec stage 逆戻り 時)
      - **I-228-c**: ExportDecl-wrapped Decl::Var with side-effect init の Axis A trigger detection 漏れ (`export const c = compute();` を library mode 誤分類 → existing path `pub const c = ...;` Rust compile error)
      - **I-228-d**: Multi-declarator VarDecl with mixed init shape の first-only check 限界 (`const a = 1, b = compute();` を Lit 誤分類 → ToplevelConst routing → `pub const b = compute();` Rust compile error)
  - Final re-review で Layer 1-4 全 **0 findings ✓**。
- **Defect Classification 5 category (final、2 round 累積)**: Grammar gap / Oracle gap = 0、Spec gap = **1 entry × 4 sibling sub-cases** (= L2-1 + I-228-b/c/d、TODO `[I-228]` 集約 escalate = framework 失敗 signal の Spec stage iteration trigger)、Implementation gap = 3 (= L1-2 INV-6 token + L2-2 declare-marked filter + L2-3 Lit::Regex narrow、全 self-fix で resolved)、Review insight = 2 (= L1-1 doc(hidden) + L1-3 token specificity + Round 2 `#[should_panic]` precondition、全 self-fix で resolved)。
- **Final test count**: 75 unit tests = 64 baseline + 9 declare-marked (R1 L2-2) + 2 R2 fix (Regex SideEffect verify + `#[should_panic]` precondition lock-in)。**Final forbidden_tokens**: 9 specific identifiers (TypeResolver / type_resolver / FileTypeResolution / type_resolution / type_registry / expr_type / narrowed_type / narrowing_analyzer / expected_type)。
- **Commit (proposed)**: `[WIP] I-224 T2 完了: main_synthesis foundation module + 75 unit tests + dispatch arm 1-to-1 mapping integration test + INV-3 (C0 partial) / INV-6 invariants test fill-in + adversarial deep review 2 round fix (declare-marked filter + doc(hidden) + INV-6 token narrowing + Lit::Regex narrow + #[should_panic] precondition + I-228 Spec gap 4 sub-entries escalate) + 4-layer review pass`
...
### T7: Test harness ESM upgrade permanent integration (Option β cohesive batch infra) (2 sub-commits、Iteration v12 で spec revise = harness 側 ESM mode write 拡張 + observe-tsc.sh permanent retention)

#### T7-1 (revised v12): tokio dep verify (既存 = N/A) + e2e_test.rs harness 側 ESM mode write 拡張

- **Work**: 
  1. **tokio dep verify (既存 = N/A note)**: `tests/e2e/rust-runner/Cargo.toml` の `tokio = { version = "1", features = ["full"] }` (= "macros" + "rt-multi-thread" を含む superset、`tests/compile-check/Cargo.toml` と sync 維持中) を retention verify。**追加 work 不要** = N/A re-classify。
  2. **e2e_test.rs harness 側 ESM mode write 拡張**: `tests/e2e_test.rs::E2eRunnerInstance::prepare_single_file_ts_exec` + `prepare_multi_file_ts_exec` で **ts-exec dir に `package.json {"type":"module"}` を unconditional 書き出し** (= tsx を ESM mode で起動可能にする infra)。Iteration v12 design choice rationale: unconditional write は detection logic 導入を avoid (= I-397 substring-based detection risk と symmetry)、105 existing fixtures + new top-await fixtures 全て non-breaking (= empirical probe 完了)、modern Node.js trend と整合。
  3. **Verify existing fixtures regression 0**: harness ESM mode write 実装後、cargo test --test e2e_test の **既存 185 active tests 全 GREEN 維持** (= e2e_test scope baseline、`tests/e2e_test.rs` 内 active test count) + **#[ignore]'d 93 tests 状態 unchanged** + ESM-mode 動作の lock-in unit test or assertion 追加 (= ts-exec dir 内 `package.json` content が `{"type":"module"}` であることを harness internal で empirical verify)。
- **Completion criteria**: cargo test --test e2e_test で **185 baseline (existing active) + 3 NEW (ESM lock-in tests) = 188 active GREEN + 93 ignored unchanged = 計 281 tests 全 PASS** (= post-fix mean ~111s baseline 維持、I-399 INV-T3 -22% improvement preservation)、harness 側 ESM mode write が `prepare_single_file_ts_exec` + `prepare_multi_file_ts_exec` の **両 path で symmetric 適用** されている (= structural lock-in)、ESM mode write の unit / integration test が新規 1 件以上で empirical lock-in
- **Commit**: `[WIP] I-224 T7-1 (Iteration v12 revise): tokio dep N/A note + e2e_test.rs harness 側 ESM mode write 拡張 (= prepare_single_file_ts_exec + prepare_multi_file_ts_exec で ts-exec dir に package.json {"type":"module"} unconditional 書き出し)`

#### T7-2 (revised v12): observe-tsc.sh --esm flag permanent retention verify + 4-layer review

- **Work**: 
  1. **observe-tsc.sh `--esm` / `--no-auto-main` flag retention verify**: TS-5 trial implementation の permanent retention (= `scripts/observe-tsc.sh` line 32-72 ESM_MODE flag block + line 138-145 package.json write block が iteration v3〜v12 を通じて削除されていないこと verify)。**CI workflow からの invoke は N/A** (= observe-tsc.sh は spec stage developer tool 性質上 CI runtime path に参与しない、Iteration v12 で誤った "CI から正式 invoke" wording を訂正)。
  2. **`/check_job` 4-layer review**: Layer 1 (Mechanical) / Layer 2 (Empirical) / Layer 3 (Structural cross-axis) / Layer 4 (Adversarial trade-off) 全実施。harness 側 ESM mode write の non-breaking property、observe-tsc.sh permanent retention、cells 14-18/30/32-40 etc. の Rust 側 transpile 出力 (= T8 未実装でも `#[tokio::main] async fn main()` 部分は既存) と TS 側 oracle 取得 path の **両 side で ESM mode 整合性** が成立することを Layer 3 cross-axis review で verify。
- **Completion criteria**: Layer 1-4 全 0 findings (or 全 fix 後 0 findings)、observe-tsc.sh `--esm` flag retention 維持 verify、`audit-prd-rule10-compliance.py` PASS 維持
- **Commit**: `[WIP] I-224 T7 完了 (Iteration v12 revise): observe-tsc.sh --esm flag permanent retention verify + 4-layer review pass`
...
| 17 | T7-1 (Iteration v12 revise) | T7 | cargo test --test e2e_test 185 baseline + 3 NEW = 188 active / 93 ignored unchanged + harness ESM mode write unit/integration test | (T7 完了 commit で実施) |
| 18 | T7 完了 (Iteration v12 revise: observe-tsc.sh --esm permanent retention + 4-layer review) | T7 | observe-tsc.sh `--esm` retention verify + audit-prd-rule10-compliance PASS | T7-2 commit で実施 |
...
### Iteration v8 (2026-05-07、Implementation Stage T2 完了 + `/check_job` adversarial deep review 3 round + `/check_problem` light review で発見の Spec gap 4 sub-entries + Layer 3 extension 2 sibling + light review 2 corrections に対する **Spec stage 逆戻り batch fix**)

**経緯 (`spec-first-prd.md` 「Spec への逆戻り」 trigger 発動)**:
- Implementation Stage T2 完了 (2026-05-07) 直後の `/check_job` adversarial deep review (R1+R2) + `/check_problem` light review で TODO `[I-228]` Spec gap (= main entry + 3 sibling sub-entries b/c/d) を発見、全 4 件が **同 root cause "AST shape direct interpretation vs recursive walk" pattern** で集約。
- User 判断 (2026-05-07): `ideal-implementation-primacy.md` "妥協なし" mandate 観点で **Option (A) Spec stage 逆戻り** を採用、4 sub-entries 全 fix を 1 batch で適用。Option (B) defer (= I-224 完了後の cleanup PRD 起票) は `prd-completion.md` Tier-transition compliance 観点で "true closure 達成" を犠牲にする pragmatic compromise = reject。
- Iteration v8 batch 適用後 `/check_job` R3 で **Layer 3 extension 2 件発見**: (1) Object Prop computed property keys (`{ [await x]: 1 }`) が walker で miss、(2) Decl::Class / Expr::Class の outer-context sub-Exprs (super_class / member computed keys) が walker で miss。両者とも I-228 root cause "AST shape direct vs recursive walk" の sibling extension で同 batch 内で structural fix 適用。
- 続く `/check_problem` light review で **2 件の semantic correctness 修正**: (P-1) `class C { p = await x; }` は invalid TS だが walker が ClassProp.value を walk し over-detection、TS spec で await invalid な context は walker scope 外と判明 → walker の ClassProp.value walk 削除 + test を skip behavior の structural lock-in に flip、(P-2) Class.decorators / ClassMember.*.decorators walks は ts_to_rs `parse_typescript` の `TsSyntax::default()` config で **decorators=false**、parser-unreachable dead code と判明 → decorator walks 全削除 (probe で empirical 確認 = SWC parser が `@dec` を "Unexpected `@`" で reject)。
- 4 sub-entries (I-228) + 2 Layer 3 extension (Object/Class) + 2 light review correction (ClassProp.value / decorators) = 計 8 件 fix を 1 batch (= Iteration v8) に統合、real production reachability の Spec gap を全 close。

**主要 spec revision (4 sub-entries + Layer 3 extension 2 + light review correction 2 = 計 8 件 fix)**:

1. **I-228 main (Axis C definition: AST shape direct → recursive walk)**: `has_top_level_await` を Design section #2 line 762 で **module top-level に到達可能な任意の `Expr::Await` sub-node 検出** に extend。Hand-rolled recursive walker (新規 `await_walker.rs` 287 行 → light review 後 279 行、38 SWC Expr variants 全 enumerate per Rule 11 (d-1)、no `_ =>` arms) で top-level Stmt::Expr / Decl::Var.init / ExportDecl-wrapped Decl::Var.init の expr tree 全体を walk、nested function bodies は skip。`console.log(await fetch())` / `const c = process(await fetch())` 等 nested await source が正しく Axis C1 (= async dispatch trigger) として detect される。

2. **I-228-b (InitKind::Lit definition: `Lit::Regex` 削除 + design intent 整合)**: Design section #3 line 970-971 bullet list から `Lit::Regex` 削除、prefix "Rust `const` 適合" との整合確立。Lit::Regex は `Regex::new("...")` runtime call = const 不可能のため `InitKind::SideEffect` partition に分類、ToplevelConst routing 不在で FnMainBodyCapture (= `let r = Regex::new(...)` Rust-compilable emission) routing。Lit::JSXText も同様 (structurally unreachable in VarDecl init position だが defensive)。

3. **I-228-c (Axis A definition: ExportDecl wrap 内 Decl::Var with side-effect/await init を A3 trigger に extend)**: Design section #3 dispatch table line 1065 (= ExportDecl entry) を分割、Lit init / no init / type-only export は existing path 維持 (Axis E orthogonality merge)、ExportDecl-wrapped Decl::Var with side-effect / await init は **`DeclVarPath::FnMainBodyCapture`** に routing、`pub` modifier は **drop** (= TS export semantic は Rust 単一 binary 出力では cosmetic、source-order preservation + side-effect runtime evaluation を優先)。

4. **I-228-d (multi-declarator semantics: first-only check → ANY-rule iter)**: Design section #3 line 977-985 の `classify_init_kind` spec を `var.decls.first()` only → **`var.decls.iter()` 全 declarator walk + ANY-rule precedence (AwaitInit > SideEffect > Lit)** に extend。`const a = 1, b = compute();` 等 mixed init で ANY が SideEffect なら全 declarator が FnMainBodyCapture routing (= 全て fn main body 内 `let` capture、Lit declarator も local binding 化、`pub const` 化を放棄)。TS module-load semantic と Rust execution semantic は byte-exact 等価。

5. **Layer 3 extension 1 (Object Prop computed property keys)**: `expr_contains_await_recursive` の `Expr::Object` arm で `ast::Prop::KeyValue(kv)` が `kv.value` のみ walk していたが、`kv.key` (PropName::Computed) も outer-context で評価されるため walk 必要。新規 `prop_name_contains_await_recursive` helper で PropName 全 variants (Ident / Str / Num / BigInt / Computed) enumerate、KeyValue / Method / Getter / Setter 全 arm で key walk 追加。`{ [await x]: 1 }` / `{ [await x](): void {} }` 等 computed key with await が正しく detect される。

6. **Layer 3 extension 2 (Class shape outer-context await detection)**: `Expr::Class` boundary を再評価、function body は依然 boundary だが super_class / member computed keys は class-definition time の outer context (= async-aware) と判明。新規 `class_contains_await_recursive` helper で Class.super_class + Class.body iter、`class_member_contains_await_recursive` で全 ClassMember variants enumerate (Method.key / ClassProp.key / PrivateMethod / PrivateProp / Constructor / StaticBlock / TsIndexSignature / Empty / AutoAccessor)。bare Decl::Class + ExportDecl-wrapped Class + Expr::Class 全 path で同 walker 経由 (= silent divergence risk 解消)。`class C extends f(await x) {}` (empirical reachable per `/tmp/i228-observations/class-super-await.ts`、tsx stdout="c.tag=1") が正しく Axis C1 として detect される。

7. **Light review correction 1 (P-1: ClassProp.value over-detection 削除)**: 初期 v8 batch で `class_member_contains_await_recursive` の `ClassProp` arm が `p.value` を unconditionally walk、`class C { p = await x; }` を await detect していたが、TS は class field initializer 内の await を **invalid syntax として reject** (instance-construction sync context for non-static / class-definition sync context for static、両方とも非 async)。Walker scope を valid TS source 限定に narrow、ClassProp.value walk を削除。test を `_class_skips_property_initializer` に rename + assert false に flip で skip behavior の structural lock-in。

8. **Light review correction 2 (P-2: parser-unreachable decorator walk 削除)**: 初期 v8 batch で `class_contains_await_recursive` が `Class.decorators` + `class_member_contains_await_recursive` が `ClassMember.*.decorators` を walk していたが、ts_to_rs の `parse_typescript` は `TsSyntax::default()` で **decorators=false** 設定、`@dec` syntax は SWC parser が "Unexpected `@`" で reject (empirical 確認済)。decorator walks 全削除 (= dead code per parser config、`decorators_contain_await_recursive` helper 削除含む)、walker docstring に "ts_to_rs parser doesn't enable decorator syntax" 明示で future parser config update 時の re-extension 候補を documented。

**Iteration v8 findings count**: 4 sub-entries (= I-228 main batch、Critical user 承認済) + Layer 3 extension 2 (Object computed key + Class super_class、R3 review) + light review correction 2 (ClassProp.value + decorators dead code、light review) = 計 8 件 fix。Critical = 0 (全 user 承認 / structurally derived)、High = 0、Medium = 0、Review insights = 0 (final post-correction state)。

**Iteration v8 Spec stage 完了判定 (true closure restoration)**: ✓ **iteration v7 で claim した "真の Spec stage true closure" は I-228 系 4 sub-entries により実は incomplete だった (= compromise 状態) → iteration v8 で structural fix で真の true closure 達成**。Implementation Stage T2 module の predicate impl + tests が revised spec に合わせて update (本 iteration v8 commit に含む)、T3 着手再開可能。Layer 3 extension 2 件 + light review correction 2 件は同 iteration v8 batch 内で同 root cause "AST shape direct vs recursive walk + parser/spec scope correctness" として一体化、structural cohesion を保持。

**Resolved TODO entries**: `[I-228]` (main) + `[I-228-b]` + `[I-228-c]` + `[I-228-d]` = 全 4 sub-entries close (TODO entry 削除、本 PRD doc spec revision で structurally resolved)。

**新規起票 TODO**: `[I-229]` (= JSX containing await + Destructuring pattern await default、I-229-a/b sub-entries、reachability extreme low で defer per real production 0 hits 想定)。decorators は parser-unreachable のため I-229 には含めず、parser config update 時に separately 再評価。

**Convergence pattern (iteration v2 → v8、updated)**:
...
| v7 (stub convention compliance) | 0 | 0 | stub creation closure |
| **v8 (Implementation T2 完了後 R1+R2+R3 deep review + light review で発見の 8 件 fix、Spec stage 逆戻り batch)** | **0 (= 4 sub-entries user 承認済 + 2 Layer 3 extension structurally derived + 2 light review correction、全 batch fix で resolved)** | **0** | **true closure restoration、I-228 系全 close + Layer 3 sibling fix + parser/spec scope correctness 確立** |
...
### Iteration v9 (2026-05-08、Implementation Stage T5-1 着手中の cell-12 / cell-24 e2e empirical probe で発見した **NonTrigger Object/Array literal Var Decl の executable mode silent-drop** Tier 1 silent semantic loss 解消の Spec stage 逆戻り = `InitKind` 4 → 5 variant split structural fix)

**Lesson source / Discovery context**: T5-1 (= cell-09/10/11/12/13/21-24/27a/27b/28/29/31 e2e green-ify) 着手中、cell-12 (matrix # 17 = A1+B3) と cell-24 (matrix # 37 = A3+B3) を transpile + cargo build で empirical probe した結果、両 fixture で `const v: T = { ... };` (= Object literal init) が silently dropped され、後続の `console.log(v.x);` (= consumer) が undefined `v` を参照 → E0425 "cannot find value `v` in this scope" downstream を発見。

**Root cause analysis**:

1. `expr_init_kind` (init_classifier.rs) で `Expr::Object` / `Expr::Array` を function/class definitions (`Arrow` / `Fn` / `Class`) と同じ `InitKind::NonTrigger` partition に merge していた。
2. `classify_decl_var_path` の routing table で `(true, NonTrigger) → DeclVarPath::LibraryMode` (= `convert_var_decl_module_level` 経由)。
3. `convert_var_decl_module_level` の `_ => continue` fallback (arrow_fns.rs:62) が `Expr::Object` / `Expr::Array` init を silent drop (= pre-existing I-016 owner、library mode で許容されていた挙動)。
4. **欠陥**: executable mode (= 他 source で `Stmt::Expr` / SideEffect Var が triggered) で Object/Array literal Var Decl の binding が後続 top-level statement から参照される場合、silent drop は **Tier 1 silent semantic loss** (= 構造的に矛盾する状態を Rust 出力)。

**Spec gap 分類**: framework 失敗 signal = `problem-space-analysis.md` 「partition の emission path consistency」設計時 verify 不足。`InitKind::NonTrigger` partition が runtime semantic 不在 (NonTrigger によく定義) と emission path 不在 (silent drop) の **両者を 1 partition で表現** = "library mode silent drop = OK" の I-016 design 仮定 が executable mode で破れる時の routing 設計が抜けていた。本 Spec gap は本 T5-1 で **架構的解消** (= partition split で別 emission path を確保)、framework 改善 candidate として `problem-space-analysis.md` に "partition consistency invariant verification" を audit script 化する候補を本 entry 末尾に記録。

**Fix design (案 A user 承認 2026-05-08、案 B = fixture simplify は妥協理由 = ideal-implementation-primacy 観点で不採用)**:

1. **`InitKind` enum 4 → 5 variant split**: `Lit / NonTriggerDef (Arrow/Fn/Class) / NonTriggerData (Object/Array) / SideEffect / AwaitInit`。`expr_init_kind` で Arrow/Fn/Class → `NonTriggerDef`、Object/Array → `NonTriggerData`。Type-only wrappers (TsAs / TsConstAssertion / TsTypeAssertion / TsNonNull / TsInstantiation / TsSatisfies / Paren) は recursive walk で inner classification 継承 = TsAs(Object) → NonTriggerData、TsNonNull(Arrow) → NonTriggerDef 等。
2. **`classify_init_kind` precedence 拡張**: ANY-rule precedence を `AwaitInit > SideEffect > NonTriggerData > NonTriggerDef > Lit` に拡張 (multi-declarator で ANY NonTriggerData → NonTriggerData、ANY NonTriggerDef → NonTriggerDef 等、structural honest)。
3. **`classify_decl_var_path` routing 拡張**: 5-column Cartesian product table を以下に更新:

   | is_executable_mode | Lit           | NonTriggerDef | NonTriggerData    | SideEffect        | AwaitInit (AST-impossible) |
   |--------------------|---------------|---------------|-------------------|-------------------|----------------------------|
   | `false` (library)  | LibraryMode   | LibraryMode   | LibraryMode       | LibraryMode       | LibraryMode                |
   | `true` (executable)| ToplevelConst | LibraryMode   | **FnMainBodyCapture** (NEW) | FnMainBodyCapture | FnMainBodyCapture |

4. **`has_side_effect_init` invariant 維持**: SideEffect / AwaitInit のみ match を維持 = NonTriggerData は executable-mode trigger ではない。pure data module (`const Phase = { ... }` のみ source) は library mode に留まる (= pre-existing I-016 owner として library mode の silent drop は本 PRD scope 外、follow-up PRD 候補)。
5. **`capture_var_decl_into_main_stmts` への影響**: 既存 `FnMainBodyCapture` 経路で Object/Array literal init を `MainStmt::Let { name, init: convert_expr(init) }` として capture (= 既存 SideEffect Var capture と同 path)、追加 logic 不要。`convert_expr(init)` は Object literal を struct literal IR に変換 (TypeResolver expected_type 経由 named struct emit)。

**Verification (T5-1 で empirical confirm)**:

- cell-12 transpile output: `struct main { value: f64 } fn main() { let v = main { value: 42.0 }; println!("{}", v.value); }` → cargo run 出力 `42` = expected match。
- cell-24 transpile output: `struct main { kind: String } fn main() { let meta = main { kind: "point".to_string() }; let p = makePoint(1.0, 2.0); println!("{} {} {}", meta.kind, p.x, p.y); }` → cargo run 出力 `point 1 2` = expected match。
- cell-77 (matrix # 77 = A6+B3 = mixed + non-fn main、Object literal Var Decl 含む): 同 fix 経由で GREEN 化 (= structural fix 適用 cell の counterpart in mixed partition)。
- 既存 NonTrigger 概念に依存する E2E test (typeof_const fixture 等): NonTriggerDef / NonTriggerData split 後も `has_side_effect_init` 不変なので library mode 挙動 unchanged、regression 0。

**Production code 修正**:

- `src/transformer/main_synthesis/init_classifier.rs`: `InitKind` enum split + `expr_init_kind` の Arrow/Fn/Class → NonTriggerDef + Object/Array → NonTriggerData + `classify_init_kind` precedence 拡張 + `classify_decl_var_path` routing table 5-column 化 + module-level + variant-level docstring 全 update (5-variant Cartesian product table embed、cell-12 / cell-24 lesson source 記録)。
- `src/transformer/main_synthesis/mod.rs`: `capture_var_decl_into_main_stmts` doc-comment 更新 (NonTriggerData が FnMainBodyCapture 経路を経由する旨明示)。

**Test 修正**:

- `src/transformer/main_synthesis/tests/i224_t5_1_spec_modori.rs` (新規): 13 NEW unit tests = (a) `classify_decl_var_path((true, NonTriggerData)) → FnMainBodyCapture` routing × Object/Array/TsAs(Object) × 3 件 + (b) library mode preservation 1 件 + (c) NonTriggerDef Arrow/Class boundary × 2 件 + (d) `has_side_effect_init` invariant 3 件 + (e) multi-declarator precedence 3 件 + (f) Class expression NonTriggerDef boundary 1 件。
- `src/transformer/main_synthesis/tests/mod.rs`: 既存 NonTrigger 10 tests を NonTriggerDef (5 件) and NonTriggerData (5 件) に migrate (= test 名 + assert variant 改名)。
- `tests/e2e_test.rs`: i-224 cells 40 entries 追加 (14 GREEN + 27 ignored 全件 specific reason) + `should_auto_append_main_call` / `has_top_level_main_call` / `has_main_definition` 自動検出 helper 関数 + 10 NEW unit tests for the detection helper。
- `tests/e2e_test.rs::test_e2e_cell_i205_09_static_getter`: `#[ignore]` 解除 (B2 mechanism dependency 解消)。
- `tests/i224_invariants_test.rs::test_invariant_1_*`: `#[ignore]` 解除 + fill-in (representative cells 9/10/22/28/29/31 で source-order substring positional match)。

**Defect Classification 5 Category (Iteration v9 final)**:

- **Grammar gap = 0**.
- **Oracle gap = 0**.
- **Spec gap = 1** (= NonTrigger merged variant の executable mode silent-drop 不在仕様欠陥、structural fix で resolved)。
- **Implementation gap = 0** (= structural fix で Spec gap 直接 resolve、別途 implementation 不整合不在)。
- **Review insight = 1** (= e2e harness の auto-append convention が i-224 cells 互換性なしを発見、auto-detection helper で backward-compat + i-224 動作達成 = harness improvement)。

**Framework 改善 candidate (本 PRD close 時 I-D PRD 起票候補)**:

- (改善 v9-1 NEW) `problem-space-analysis.md` に "partition consistency invariant verification" 追加 = matrix axis の partition 定義時に「各 partition が単一 emission path を持つか / 複数 path に routing する場合 routing condition が明示されているか」を audit script で auto verify する mechanism。本 Iteration v9 の Spec gap (= NonTrigger partition が library mode silent drop と executable mode capture の **両 emission path を持つべき** だったが 1 partition で表現していた structural defect) を framework rule level で防ぐ。
- (改善 v9-2 NEW) `spec-stage-adversarial-checklist.md` Rule 10 axis enumeration の default check axis として "InitKind partition × executable_mode 軸 Cartesian routing consistency" 追加検討 (= 本 Iteration v9 の Spec gap が Rule 10 dispatch arm enumeration で見過ごされた = framework gap)。
- (改善 v9-3 NEW) e2e harness `should_auto_append_main_call` 検出 helper の rule 化 = `tests/e2e_test.rs` 内 inline 実装を `tests/test_helpers.rs` 等に切り出し、複数 PRD 系列の e2e fixture (i-224 / 将来の matrix-driven PRD) で共通利用可能に。

### Iteration v10 (2026-05-08、Implementation Stage T5-1 完了後の `/check_job` 3 iteration + `/check_problem` 2 round による **post-T5-1 review structural fix 累積**)

**Lesson source / Discovery context**: T5-1 (NonTrigger split + Spec への逆戻り Iteration v9) commit 直前 / 直後の review iteration で、**T5-1 architectural concern の周辺に位置する複数の architectural defect** を継続的に発見・structural fix。各 review iteration が独立した structural concern を surface し、累積 5 件の structural fix を 0 妥協で完成させた。本 Iteration v10 entry は、Spec への逆戻り (v9) 後の Implementation stage post-fix review chain の **最終結果 audit trail** を framework convention (v8 = T2 review fix、v9 = T5-1 silent-drop fix) に symmetric な form で record。

**累積 structural fix 5 件 (review iteration 別)**:

#### `/check_job` initial review fix (Layer 4 Hono bench Tier-transition compliance verify 由来)

- **Discovery**: `/check_job` 初回 invocation 時の Hono bench 実行で **6 file regression** を発見 (clean 108 → 102、errors 68 → 76)。Root cause analysis = T5-1 NonTrigger split で `expr_init_kind` が **type-only AST wrapper** (`TsConstAssertion` / `TsSatisfies` / `TsTypeAssertion` / `TsInstantiation`) 経由で wrapped NonTriggerData (Object/Array) を `FnMainBodyCapture` に routing → capture path 経由で `convert_expr` に流入 → `convert_expr` の wrapper handling 不在 (`_ => Err("unsupported expression")`) で Tier 2 honest error 発火 → 既存 Hono code base の compress / csrf / serve-static / cors 系で compile error 増加。
- **Structural fix**: `convert_expr` に **TsConstAssertion / TsTypeAssertion / TsSatisfies / TsInstantiation passthroughs** を追加 (TsAs / TsNonNull existing pattern symmetric)。Type-only wrapper の inherent passthrough semantic を 4 wrapper に extend。Hono bench: 102 → 106 clean (+4 file recover、compress / csrf / middleware-serve-static / 他 1 件 recovery)。
- **Source file**: `src/transformer/expressions/mod.rs` に passthrough 4 arm 追加。

#### `/check_job deep` review fix (Layer 3 Structural cross-axis 由来 = Mixed Def+Data per-declarator routing 架構欠陥)

- **Discovery**: T5-1 NonTrigger split は VarDecl-level routing (`classify_decl_var_path`) で ANY-rule precedence (NonTriggerData > NonTriggerDef) を採用、mixed Def+Data multi-declarator VarDecl (`const f = () => 1, x = { a: 1 };`) で Arrow が NonTriggerData precedence に絡めとられ FnMainBodyCapture 経由で capture path に routing → Arrow が **closure literal `let f = || 1.0;` inside fn main** として emit、top-level `Item::Fn` 経路を **architecturally lost**。
- **Structural fix**: **Per-declarator routing への migration**:
  1. `classify_per_decl_path(decl: &VarDeclarator, is_executable_mode: bool, parent_var_declare: bool) -> DeclVarPath` 新規 helper を `init_classifier.rs` に追加 = 各 declarator が独立に routing path を decide。
  2. `all_decls_captured(var, is_executable_mode_flag) -> bool` 新規 aggregator = mixed VarDecl で `Ok(false)` 経由 transform_module_item 併走 dispatch 判定。
  3. `capture_var_decl_into_main_stmts` を per-declarator filter に変更 = `classify_per_decl_path` で各 declarator 独立判定、FnMainBodyCapture-bound のみ MainStmt::Let 化 (NonTriggerDef / Lit declarators は skip)。
  4. `try_capture` の `Decl::Var` arm + `ExportDecl(Decl::Var)` arm で `all_decls_captured` 経由 dispatch 整合 = mixed Def+Data 時 `Ok(false)` 経由 transform_module_item 併走、Arrow → top-level `Item::Fn` AND Object → fn-main-body `let` 同時実現。
- **Reachability note**: Hono codebase に mixed Def+Data multi-declarator pattern 不在 (verified by `grep -rE "^\s*const\s+\w+\s*=\s*\(.*\).*,\s*\w+\s*=\s*[\[\{]"` = 0 hits in /tmp/hono-clean)。fix は **architectural correctness driver** であり reachability driven ではない (= per `ideal-implementation-primacy.md` 構造的正しさが reachability rationalization に勝る)。
- **Source files**: `init_classifier.rs` (helper 2 件追加)、`mod.rs` (capture filter + dispatch 整合化)。

#### `/check_job deep deep` review fix (Layer 1 + Layer 3 由来 = legacy code retention + satisfies-type-aware emission)

- **Discovery 1**: 前 `/check_job deep` で `classify_decl_var_path` を `#[cfg(test)]` gate に下げて test-only retention としたが、これは **legacy code 維持の妥協** (= production には不要、test contract preservation のためのみ存在 = "test-only legacy artifact"、`ideal-implementation-primacy.md` 観点で no-dead-code / single-canonical-classifier の妥協)。
- **Structural fix 1**: `classify_decl_var_path` 完全削除:
  1. `tests/mod.rs::classify_decl_var_path_*` 5 件を `classify_per_decl_path_*` に migrate (helper `first_decl(m)` 経由 single-decl access pattern を共通化)。
  2. `tests/i224_t5_1_spec_modori.rs::classify_decl_var_path_*` 6 件を InitKind partition assertion 5 件 + 既存 per-decl routing 6 件で coverage replacement。
  3. Production code から関数本体 + cfg(test) re-export を完全削除。
- **Discovery 2**: 前 review で convert_expr に追加した TsSatisfies / TsTypeAssertion passthrough は **type info を expected_type として inner conversion に propagate せず**、cors-style `const opts = { ... } satisfies CORSOptions` で inner Object literal が "object literal requires a type annotation" Tier 2 honest error 残存。
- **Structural fix 2**: TsSatisfies / TsTypeAssertion で `t.type_ann` を `convert_ts_type` で resolve + `convert_expr_with_expected(&t.expr, Some(&ty))` 経由 inner conversion の expected_type に flow。Hono bench 106 → 107 clean (+1 file recover、cors/index.ts)。
- **Source files**: `init_classifier.rs` (legacy classifier 削除)、`mod.rs` (re-export 削除)、`tests/mod.rs` + `tests/i224_t5_1_spec_modori.rs` (test migration 11 件)、`expressions/mod.rs` (TsSatisfies / TsTypeAssertion expected_type propagation)。

#### `/check_problem` round 1 review fix (session 全体 hidden compromise scan 由来)

- **Discovery 1**: `classify_decl_var_path` 完全削除後、**stale doc references が 9+ 件残存** (init_classifier.rs / mod.rs / tests に intra-doc link broken の form)。
- **Structural fix 1**: production / non-historical context は `classify_per_decl_path` に rename (intra-doc link 整合)、historical / migration-context comments は legacy 名前を意図的に保持 (= 削除履歴の audit trail 維持)。
- **Discovery 2**: 前 deep deep で TsSatisfies / TsTypeAssertion (own type_ann 持ち) のみ expected_type propagation したが、TsAs (own type_ann 持ち、F64/Bool 以外の non-cast case) は passthrough without expected で **architectural asymmetry** 残存。
- **Structural fix 2**: TsAs の `Ok(target_ty)` (non-F64/Bool) arm に `convert_expr_with_expected(&ts_as.expr, Some(&target_ty))` 追加 (3 wrapper symmetric pattern 確立)。Hono bench 0 regression (TsAs `(... as T)` 形 reachability=0)。
- **Discovery 3**: `capture_var_decl_into_main_stmts` の destructuring guard (`Pat::Object` / `Pat::Array`) が **silent skip** = T5-1 silent → honest 化方針との不整合。
- **Structural fix 3**: destructuring pattern を **Tier 2 honest reject** に structural fix (`UnsupportedSyntaxError::new("module-level destructuring var decl with executable trigger requires destructuring pattern conversion; rewrite as ...")`)。Hono bench 107/70 → 107/72 (= +2 honest error, 0 file transition)。bun/ssg.ts (`const { write } = Bun`) + deno/serve-static.ts (`const { open, lstatSync, errors } = Deno`) で silent → honest Improvement 確認。
- **Discovery 4**: `prd-completion.md` の Tier-transition compliance wording が **"Tier 1 silent → Tier 2 honest" classification を enumerate せず**、本 PRD T5-1 の Hono bench 結果 (silent → honest 化複数 file) の compliance 判定が ambiguous。
- **Structural fix 4**: `prd-completion.md` に **`Improvement (Tier 1 silent → Tier 2 honest)` (allowed)** classification 追加 + **`New compile errors on previously-clean files` (prohibited)** に tighten (= clean だった file が transition した時のみ regression、既 not-clean file への error 追加は honest 化として allowed)。
- **TODO 起票**: I-395 (Class expression conversion = prepared-router 残 1 file regression)、I-396 (destructuring proper conversion)、I-397 (auto-append helper edge cases) を `todo-entry-standards.md` format で record。

#### `/check_problem` round 2 review (再 critical scan 由来 = nothing new)

- 全 session-discovered issue が round 1 + 累積 structural fix で対応済みを confirm。
- TsNonNull / Paren expected_type propagation 等の **hypothetical consistency improvement** は session 中で actual reachability や具体的失敗例の発見が無いため起票見送り (= session-discovered issue ではない)。
- 唯一の判断要 item = 本 Iteration v10 entry 追加判断 (user 確定 = A 採用 → 本 entry 起票)。

**Defect Classification 5 Category (Iteration v10 累積 final、v9 → v10 net)**:

| Category | v9 only | v10 累積 (post-/check_job ×3 + /check_problem ×2) |
|----------|---------|------------------------------------------------|
| Grammar gap | 0 | 0 |
| Oracle gap | 0 | 0 |
| **Spec gap** | 1 (NonTrigger merged variant) | **5** (+ /check_job initial: convert_expr type-only wrapper passthroughs不在 / + /check_job deep: VarDecl-level ANY-rule precedence の per-decl emission consistency 破綻 / + /check_job deep deep: classify_decl_var_path test-only retention 妥協 + TsSatisfies expected_type propagation 不在 / + /check_problem: TsAs expected_type asymmetry + destructuring silent drop + prd-completion.md Tier-transition wording gap) |
| Implementation gap | 0 | 1 (= module_items.rs test fixture migration to Class expression、`/check_job` deep deep 中 fix) |
| Review insight | 1 | 4 (= cors-style satisfies-type-aware emission ↑ resolved / mixed Def+Data per-declarator routing ↑ resolved / TsAs expected_type symmetry ↑ resolved / + 残 active 2 件 (Tier-transition wording gap ↑ resolved + Iteration v10 entry 判断 ↑ resolved) — net 0 active) |

**Framework 改善 candidate (本 PRD close 時 I-D PRD 起票候補、v9 改善 + v10 追加)**:

- (v9-1〜v9-3) 既存 v9 改善 candidate を継承
- (改善 v10-1 NEW) `convert_expr` の type-only wrapper passthrough を **expected_type propagation 含む standardized pattern** で文書化 = TsAs / TsNonNull / TsConstAssertion / TsTypeAssertion / TsSatisfies / TsInstantiation / Paren の 7 wrapper 全てで「outer expected_type を inner conversion に propagate する責務」を明示 (= Round 2 で TsNonNull / Paren / TsConstAssertion / TsInstantiation の expected propagation が pre-existing gap として残存、follow-up scope 候補)。
- (改善 v10-2 NEW) `classify_decl_var_path` 完全削除 lesson から、**legacy code の `#[cfg(test)]` retention は temporary compromise** = 設計判断 rule に「test-only retention は production migration 完了の即時実施 = 永続的 retention は no-compromise 違反」を framework rule level に追加検討。

### Iteration v11 (2026-05-08、Implementation Stage T5-2 着手時の B2 + executable-mode `__ts_main()` substitute call .await wrap **Tier 1 silent semantic loss** 発見 = `UserMainSubstitution` enum 化による Spec stage 逆戻り + `from_dispatch` constructor DRY 解消 + `UserMainKind` / `UserMainSubstitution` / `detect_user_main` 三者の `user_main.rs` 同居による cohesion 向上 + INV-7 fill-in)

**Lesson source / Discovery context**: T5-2 着手時 (cell-41 control-flow Tier 2 + cell-75 + cell-77 + cell-79 e2e green-ify scope) に各 cell の現状を empirical probe した結果、**cell-75 (matrix # 75 = A6+B2+C0)** に **Tier 1 silent semantic loss** を発見:

- 現状の cell-75 transpile output: `__ts_main()` (= sync substitute call) inside `#[tokio::main] async fn main` body
- 期待: `__ts_main().await` (= async substitute call、`.await` wrap で renamed async user main を完了まで実行)
- 結果: TS の `async function main()` body 内 `console.log("from async main")` が Rust 実行で **silently dropped** (= returned `Future` が drop されて async body が走らない) → user-visible output が消失 = Tier 1 silent semantic change

**根本原因 (= T-task 分割の Spec gap)**: T5-1 commit message + cell-23/cell-11 の `#[ignore]` reason は「T8 .await wrapping required」と記述していたが、T8 sub-task split (T8-1: Stmt::Expr(Expr::Await) emission / T8-2: Decl::Var(await init) emission + cell 14/34/74 sync user main edge case / T8-3: INV-3 full coverage) に **B2 substitute call .await wrap が明示的に enumerate されていなかった**。Code comment (`expressions/calls.rs:148-158`) でのみ "T8 will additionally wrap" と記述、T-task spec doc には記載なし = framework 失敗 signal (= Spec stage で T-task 分割の cell coverage を verify する mechanism が不在)。

**該当 cells (B2 + executable mode 全件、`UserMainSubstitution::AsyncRename` dispatch arm)**:

- cell-11 (matrix # 15 = A1+B2+C0): `function main` async + Stmt::Expr top-level — T5-1 deferred
- cell-23 (matrix # 35 = A3+B2+C0): `function main` async + Decl::Var SideEffect init — T5-1 deferred
- cell-75 (matrix # 75 = A6+B2+C0): `function main` async + mixed top-level — T5-2 NEW C0 scope

**Spec gap 分類**: framework 失敗 signal — `spec-stage-adversarial-checklist.md` Rule 9 (Dispatch-arm sub-case alignment) は **type dimension** の dispatch arm enumerate を要求するが、本 case は **B-axis dispatch arm の substitute behavior 拡張** = Rule 9 が capture できる範囲外の subtle gap。framework 改善 candidate として「**substitute / rewrite logic** の dispatch arm に対しても Rule 9 を symmetric 適用 (= sync substitute / async substitute / no substitute の 3 arm 全てが test cell coverage を持つ)」を提案 (本 v11 entry 末尾に記録)。

**Resolution (T5-2 で実施した Spec への逆戻り fix)**:

1. **`UserMainSubstitution` enum 新設** (`src/transformer/main_synthesis/user_main.rs`): substitute 状態を `None` / `SyncRename` / `AsyncRename` の 3 variant で明示化。`is_active()` + `is_async()` predicate を提供。`UserMainSubstitution::from_dispatch(exec_mode, user_main_kind)` constructor で dispatch table を **single source of truth 化** (= `transform_module` / `transform_module_collecting` で重複していた match block を DRY 解消)。
2. **`Transformer.user_main_substitution` field type 変更**: `bool` → `UserMainSubstitution` (= async vs sync vs none を field 型で表現)。
3. **`expressions/calls.rs` substitute logic 拡張**: `is_main_substitute && self.user_main_substitution.is_async()` で `Expr::Await(Box::new(call))` wrap を発火 → IR emission 段階で `__ts_main().await` 化。
4. **`functions/mod.rs` + `functions/arrow_fns.rs` rename 判定**: bool 参照を `is_active()` predicate 呼出に置換 (= 意味は同じだが enum-based dispatch に統一)。
5. **Cohesion 向上**: `UserMainKind` enum を `main_synthesis/mod.rs` から `main_synthesis/user_main.rs` に移動 (= `detect_user_main` と同居)。`UserMainSubstitution` も同 file に追加。
6. **INV-2 拡張**: B1 sync substitute は `__ts_main();` (no `.await`) を assert、B2 async substitute は `__ts_main().await` を assert + 同 cell に bare `__ts_main();` (no `.await`) leak 不在 sanity check。
7. **e2e harness 解除**: cell-11 / cell-23 / cell-75 の `#[ignore]` 解除 + 解除理由 (= "T5-2 Iteration v11 2026-05-08 Tier 1 silent-loss fix") を comment 化。
8. **INV-7 fill-in**: `scripts/audit-no-init-call-site.sh` 新規作成 (= `audit-no-pub-fn-init.sh` symmetric design、free-function `init()` call site detection) + `tests/i224_invariants_test.rs::test_invariant_7_pub_fn_init_external_api_audit_post_t4` を `#[ignore]` 解除 + fill-in (= subprocess audit + independent grep verifier の two-verifier structural lock-in)。

**File-line resolution rule 新設 (= 並行する meta-improvement)**: `transformer/mod.rs` が `UserMainSubstitution` enum 追加で 1005 行に超過 → 単純な「末尾切り出し」ではなく `.claude/rules/file-size-resolution.md` を新設 (= 機械的切り出し禁止 + 周辺ファイル調査 + DRY/凝集度問題点 enumerate + 関連実装も含めた再構成 plan の 4-step procedure 必須化)。本 file の `UserMainKind` + `UserMainSubstitution` + `detect_user_main` の `user_main.rs` 同居は本 rule の Worked Example として記録。

**Verification (T5-2 で empirical confirm)**:

- cell-11 e2e: stdout `top-level\nfrom async main\n` (= TS observation byte-exact match) ✓
- cell-23 e2e: stdout 一致 ✓
- cell-75 e2e: stdout `got 100 42\nfrom async main\n` (= TS observation byte-exact match) ✓
- cell-77 e2e: 既に green、変化なし ✓
- cell-41 (Tier 2 reject): `src/transformer/statements/tests/loops.rs::i224_a4_top_level_if_rejected_with_control_flow_wording` 既存 lock-in で coverage ✓
- cell-79 (Tier 2 reject): INV-5 (`test_invariant_5_ts_main_namespace_reservation_with_collision_priority`) 既存 lock-in (cell 79 in fixture list) で coverage ✓
- INV-2 + INV-7 invariants test全 pass ✓
- cargo test --lib: 3542 passed / 0 failed ✓
- cargo clippy / fmt / file-line: 0 warnings / 0 diffs / 全 .rs file < 1000 行 (transformer/mod.rs 931 / main_synthesis/mod.rs 974 / user_main.rs 389) ✓
- audit-prd-rule10-compliance.py / audit-no-pub-fn-init.sh / audit-no-init-call-site.sh: 全 PASS / exit=0 ✓
- Hono bench: clean **107** / errors **72** = 0/0 vs T4 baseline (= **Preservation** classification per `prd-completion.md` Tier-transition compliance) ✓

**Framework 改善 candidate (本 PRD close 時 I-D PRD 起票候補、v11 NEW)**:

- (改善 v11-1 NEW) `spec-stage-adversarial-checklist.md` Rule 9 (Dispatch-arm sub-case alignment) を **substitute / rewrite logic** の dispatch arm にも symmetric 適用する候補 (= sync substitute / async substitute / no substitute の 3 arm 全てが test cell coverage を持つ verify mechanism)。Lesson source: 本 v11 で B2 substitute call .await wrap が T-task 分割の Spec gap として発覚した case。
- (改善 v11-2 NEW) `.claude/rules/file-size-resolution.md` を新設 (= 機械的末尾切り出し禁止 + 周辺ファイル調査 + DRY/凝集度問題点 enumerate + 関連実装も含めた再構成 plan の 4-step procedure)。本 v11 の `UserMainKind` + `UserMainSubstitution` + `detect_user_main` 同居 + `from_dispatch` constructor DRY 解消を Worked Example として lock-in。
- (改善 v11-3 NEW) **Layer 3 deep review で発見した double-await structural bug = `convert_expr` の substitute-time `.await` wrap が source-level `await main();` (cells 16/30/36 + nested `await main()` inside user-defined async fn body) で outer `Expr::Await` と二重に作用する**。Layer 3 直交軸 review 不在では「substitute logic の context-awareness」次元が enumerate されず、本 PRD の最初の T5-2 commit message までこの bug が latent (= cells 16/30/36 が `#[ignore]` 済 = T7-T8 scope だったため visible でなかった)。Framework 改善: `spec-stage-adversarial-checklist.md` Rule 10 axis (i) "AST dispatch hierarchy" に "rewrite / substitute logic の caller-supplied wrap context awareness" を default check axis として追加する候補。Resolution: `Transformer::suppress_main_await_wrap` flag + `Transformer::convert_expr_in_await_context` helper で context-aware suppression 実装、3 entry sites (`convert_expr` Expr::Await arm / `try_capture_module_item_into_main_stmts` Stmt::Expr Await arm / `capture_var_decl_into_main_stmts` Await branch) で uniform 適用。INV-2 拡張で cells 16/36 patterns の `__ts_main().await` 単一 emission + `__ts_main().await.await` 不在 を IR-text-level lock-in。
- (改善 v11-4 NEW) **2nd /check_job review (Layer 1) で発見した missing decision-table unit test coverage** — 1st review が "test coverage adequacy" の Layer 1 check を実施せず、`UserMainSubstitution::from_dispatch` (10-cell 決定 table = 2 × 5 UserMainKind) と `is_active()` / `is_async()` predicates に対する direct unit test の不在を 2nd review で発覚。Indirect coverage (INV-2/INV-3/INV-5/e2e) では存在したが、dispatch table 自体の 1-to-1 cell coverage が未 lock-in。Framework 改善: `check-job-review-layers.md` Layer 1 (Mechanical) sub-rule 追加 — 「新 public API / dispatch table / decision table を導入する PRD で、各 decision table cell に対する **直接 unit test** が存在することを Layer 1 で verify」。Resolution: 4 unit tests (`from_dispatch_covers_full_decision_table` / `is_active_returns_true_for_rename_variants_only` / `is_async_returns_true_only_for_async_rename` / `is_async_implies_is_active`) を `src/transformer/main_synthesis/user_main.rs` の `#[cfg(test)] mod tests` に追加 (2nd review in-fix)、framework rule level の Layer 1 sub-rule 追加は別 PRD I-D scope。

**T5-2 deep review (=`/check_job` Layer 3 cross-axis) Spec gap classification**: 本 v11 entry の double-await bug は **Spec gap** (= framework 失敗 signal、`post-implementation-defect-classification.md` 5 category 分類)。reference doc (= `ast-variants.md` `Expr::Await` entry) に variant 記載あり、SWC parser empirical (= source-level `await x;` shape) も confirm 済、observation log も実体確認可能 (= cell-16/30/36 fixture content)。それでも matrix が "substitute logic の caller-supplied await context" 直交軸を enumerate せず double-wrap の symmetric coverage が欠落 = framework が capture できない領域の発見 → 改善 v11-3 で framework rule level に lift 候補。

**T5-2 /check_problem (light review) で out-of-matrix hypothetical scenarios を TODO [I-398] 起票** (2026-05-08): B2 substitute call `.await` wrap の以下 3 sub-cases は matrix 内 cell として enumerate されず、現 fixture 0 reachability。

- (I-398-a) B2 multi-call mid-stream output divergence (`main(); foo(); main();` で TS event-loop microtask defer vs Rust `.await` block の出力順序乖離、Tier 1 silent semantic divergence)
- (I-398-b) Deeply-nested `Expr::Await` arg over-suppression (`await foo(main())` で suppress flag inheritance により内側 main() Future drop 可能性)
- (I-398-c) Decl::Var bare main() init type divergence (`const r = main();` で TS r=Promise vs Rust r=() = type 乖離)

Resolution direction: PRD I-224 close 後の future PRD で matrix 拡張 (option A) または Tier 2 honest reject reclassify (option B) を choice する。詳細 = TODO `[I-398]`。

### Iteration v12 (2026-05-08、Implementation Stage T7 着手前 prerequisite 調査で発見の **T7 spec wording vs 実態 infra work 乖離** に対する **Spec への逆戻り** = T7-1 / T7-2 work scope revise + tokio dep N/A note + harness ESM mode write 実装方針確定 + `## Sub-commits 一覧` row 17/18 update)

**Lesson source / Discovery context**: I-399 PRD close 完了 (commit `4c53714`) 後の T7 chain 再開時に、`/start` skill prerequisite 調査として T7-1 / T7-2 work spec を実態と empirical cross-check した結果、**T7 spec wording と実体 infra work の乖離** を発見:

1. **T7-1 spec wording**: `tests/e2e/rust-runner/Cargo.toml に tokio = { version = "1", features = ["macros", "rt-multi-thread"] } 等の runtime 依存追加 (= #[tokio::main] macro 用) + tests/e2e_test.rs runner template を ESM-mode に拡張 (= top-await を含む Rust binary を build / cargo run で execute、tokio runtime context で正しく実行) + 既存 e2e harness が新 ESM-mode template と互換動作することを verify`
   - **Reality 1**: `tests/e2e/rust-runner/Cargo.toml` は既に `tokio = { version = "1", features = ["full"] }` を保持 (= "macros" + "rt-multi-thread" を含む superset、`tests/compile-check/Cargo.toml` と sync 維持中)。tokio dep 追加は **不要** = N/A
   - **Reality 2**: rust-runner template (= edition 2021 Rust crate) は既に `#[tokio::main] async fn main() { let value = Promise::<f64>::resolve(42.0).await; println!(...); }` を build / cargo run できる。Empirical probe (rust-runner template clone + cell-14 Rust output exact-form synth + `cargo run` → `got 42` 出力) で confirm。**追加 work 不要**
   - **Reality 3**: cells 11/12/22/23/77 (= production e2e tests with `#[tokio::main]` async fn main) が GREEN status 維持 (T5-2 commit `20d719b` 以降)、これが T7-1 completion criteria (= "rust-runner で `#[tokio::main]` build pass + cargo run 成功") の **structural lock-in artifact** として既に機能
   - **Spec wording の "runner template ESM-mode 拡張"**: "runner template" 指示先が曖昧 (= rust-runner = Rust 側 / ts-exec template = TS 側)。Rust 側 work は既に done のため、charitable reading では "ts-exec template" を指していると解釈可能だが、**spec wording 自体が ambiguous**

2. **T7-2 spec wording**: `scripts/observe-tsc.sh --esm --no-auto-main を CI workflow から正式 invoke (= Spec stage TS-5 trial implementation の permanent integration)`
   - **Reality**: `scripts/observe-tsc.sh` は **spec stage developer tool** (= PRD author が Oracle Observation 取得用に invoke する script、本 PRD spec stage iteration v3 で trial implementation 完了 + line 32-72 + line 138-145 改修済 2026-05-01)。CI runtime path には参与しない (= CI が observe-tsc.sh を invoke する design intent が不明、設計不整合)。"CI から正式 invoke" は誤解を招く wording

3. **実際に必要な infra work** (= cells 14-18 / 30 / 32-40 等の top-await fixture を T9 で e2e green-ify するための prerequisite): `tests/e2e_test.rs::prepare_single_file_ts_exec` / `prepare_multi_file_ts_exec` の **ts-exec dir に `package.json {"type":"module"}` を書き出す** こと。Empirical probe で confirm:
   - **ESM mode なし** (現状): `cell-14.ts` を bare tsx 実行 → `Top-level await is currently not supported with the "cjs" output format` エラー (esbuild transform failure、tsx default = cjs format)
   - **ESM mode あり** (`{"type":"module"}` 書き出し): `cell-14.ts` を tsx 実行 → `got 42` 出力成功
   - **既存 fixtures regression check** (105 fixtures): ESM mode 化は non-breaking (= ESM は CJS の strict superset for normal TS code、`grep -rln "import = require\|export ="` で codebase 内 CJS-only patterns 0 hits 確認、empirical probe `arithmetic.ts + main(); + ESM` → 期待 stdout 出力成功)

**Spec gap 分類**: framework 失敗 signal — `spec-first-prd.md` 「Spec への逆戻り」 trigger に該当 (= Implementation 中に Spec が曖昧で 2 通り以上の実装が可能 + Spec の wording が実体と矛盾)。`post-implementation-defect-classification.md` 5 category では **Spec gap** (= reference doc + tooling から derivable だったが spec wording に enumerate されていなかった infra work)。

**Resolution (本 v12 entry での Spec stage 逆戻り batch fix)**:

1. **T7-1 spec revise** (Sub-commits 一覧 row #17): `tokio dep verify (既存 = N/A) + tests/e2e_test.rs harness 側 ESM mode write 拡張 (= prepare_single_file_ts_exec + prepare_multi_file_ts_exec で ts-exec dir に package.json {"type":"module"} を unconditional 書き出し) + 既存 fixtures regression 0 verify`。Completion criteria: harness ESM mode write 実装後、cargo test --test e2e_test で **185 baseline active (existing) 全 GREEN 維持 + 3 NEW (ESM lock-in tests) 全 GREEN = 188 active + 93 ignored unchanged**、ESM-mode 動作 unit-test or assertion で empirical lock-in。
2. **T7-2 spec revise** (Sub-commits 一覧 row #18): `scripts/observe-tsc.sh --esm flag permanent retention verify (= TS-5 trial implementation の permanence、CI invoke は spec stage tool 性質上 N/A、permanent feature retention のみ) + Layer 1-4 4-layer review`。Completion criteria: observe-tsc.sh `--esm` / `--no-auto-main` flag が retention 維持 (= 削除されないこと verify)、Layer 1-4 全 0 findings。
3. **Unconditional ESM design choice rationale**: "always write `package.json {"type":"module"}`" を採用 (= conditional detection logic を導入しない)。Rationale: (a) **DRY + KISS**: detection logic 導入は code path 増加 (= I-397 と同じ substring-based detection risk)、(b) **Empirical safety**: 105 existing fixtures + new top-await fixtures 全て ESM mode で正常実行 (= probe 完了)、(c) **Future-proof**: 新 fixture 追加時に ESM mode 判定を意識する必要なし、(d) **Modern Node.js trend**: ESM が default 化方向。
4. **Sub-commits 一覧 row 17/18 update**: row #17 description を `T7-1 (revise)` + row #18 description を `T7-2 (revise)` に変更。Quality gate per commit + 4-layer review timing は変化なし。
5. **TS-5 status note**: 既存「iteration v3 で trial implementation 完了」を保持、追加で「Implementation stage T7 で permanent retention (= CI invoke は N/A、observe-tsc.sh は spec stage tool 性質上 CI runtime path に参与しない)」と integration target を明確化。

**Verification (本 v12 entry 時点 empirical confirm)**:

- T7-1 prerequisite empirical: rust-runner template (clone + clean rebuild) で `#[tokio::main] async fn main() { let value = Promise::<f64>::resolve(42.0).await; println!(...); }` build/cargo run 成功 → `got 42` 出力 ✓
- T7-2 prerequisite empirical: `scripts/observe-tsc.sh --esm --no-auto-main` が iteration v3 から retention 維持 (`scripts/observe-tsc.sh` line 32-72 ESM_MODE flag + line 138-145 package.json write block 維持) ✓
- T7 actual infra work empirical: bare tsx with cell-14.ts → "Top-level await is currently not supported" エラー / tsx with `package.json {"type":"module"}` and cell-14.ts → `got 42` 出力 ✓
- Existing fixtures regression check: `arithmetic.ts + main(); + package.json {"type":"module"}` → 期待 stdout 出力 (`add: 3`, etc.) ✓
- Codebase CJS-only patterns: `grep -rln "import = require\|export =\|namespace " tests/e2e/scripts/` で実 CJS pattern 0 hits (cell-12 の `namespace` hit は code comment 内のみ) ✓

**13-rule self-applied re-verify (iteration v12)**:

| Rule | Sub-rule check | iteration v11 verdict | iteration v12 verdict | Notes |
|---|---|---|---|---|
| 1 | (1-1)〜(1-4) | ✓ | ✓ | matrix 80 cells 維持、変更なし |
| 2 | (2-1)〜(2-3) | ✓ | ✓ | Oracle Observations section 維持 |
| 3 | (3-1)〜(3-3) | ✓ | ✓ | NA cells (25) 維持、SWC parser empirical lock-in tests 維持 |
| 4 | (4-1)〜(4-3) | ✓ | ✓ | doc-first dependency order 維持 |
| 5 | (5-1)〜(5-4) | ✓ | ✓ | Stage tasks 2-section split 維持、本 v12 で revise する row 17/18 は Implementation Stage Tasks 内 |
| 6 | (6-1)〜(6-4) | ✓ | ✓ | Scope 3-tier + matrix Scope 列 cross-reference 維持 |
| 7 | Control-flow exit | N/A | N/A | 本 PRD scope 外、変更なし |
| 8 | (8-5) `## Invariants` 独立 section | ✓ | ✓ | INV-1〜INV-7 独立 section 維持、本 v12 で invariant 追加なし |
| 9 | (a)〜(c) | ✓ | ✓ | Dispatch arm mapping 維持、本 v12 は infra work scope の reword で IR enum / dispatch arm 変更なし |
| 10 | Cross-axis matrix completeness | ✓ | ✓ | Axis A × B × C × E 維持、本 v12 で axis 追加なし |
| 11 | (d-1)〜(d-6) | ✓ | ✓ | `_` arm 全廃 維持、`ast-variants.md` single source of truth 維持 |
| 12 | Rule 10/11 Mandatory + structural | ✓ | ✓ | `## Rule 10 Application` section 維持 |
| 13 | (13-1) skill workflow Step 4.5 | N/A | N/A | 本 v12 は Implementation stage 中の Spec への逆戻り、Spec stage skill workflow 不発動 |
| 13 | (13-2) `## Spec Review Iteration Log` record | ✓ | ✓ | 本 v12 entry 自身が record |
| 13 | (13-3) Critical findings 全 fix | N/A | N/A | 本 v12 は Critical/High/Medium/Low classification 不要 (= scope reword + spec gap classification、defect ではない) |
| 13 | (13-4) audit verify | ✓ | ✓ | `audit-prd-rule10-compliance.py` PASS 維持 (本 v12 で Problem Space matrix / Oracle Observations / Scope / Invariants 等 audit-target section に変更なし) |
| 13 | (13-5) Self-applied integration | partial | partial | 本 v12 で発見の framework gap (= "実装 stage で発覚した spec wording と実体の乖離 を Spec への逆戻り procedure で resolve する standardized flow" + "infra work scope の妥当性を Implementation stage 着手直前に empirical cross-check する mandatory step") を本 PRD close 時 I-D PRD 起票候補として記録 (improvement v12-1 / v12-2 NEW) |

**Iteration v12 完了判定**: Spec への逆戻り procedure 完了 (= Iteration log 記録 + T7-1 / T7-2 spec revise + Sub-commits 一覧 update + 13-rule re-verify pass)。Implementation stage T7 (revised spec) 着手可能。

**Framework 改善 candidate (本 PRD close 時 I-D PRD 起票候補、v12 NEW)**:

- (改善 v12-1 NEW) `spec-first-prd.md` 「Spec への逆戻り」procedure に **"Implementation stage 着手直前 prerequisite 調査"** mandatory sub-step 追加候補。Lesson source: 本 v12 で T7 spec wording と実体 infra work の乖離が `/start` skill prerequisite 調査時に発覚、もし `/start` 時点で勝手な解釈で T7-1 を実装着手していたら spec gap が記録されないまま PRD close iteration に持ち越され、後続 review で再発覚 risk があった。Resolution direction: 各 T task 着手直前に「spec wording / completion criteria が現実と整合するか empirical cross-check」を mandatory step として挿入、不整合発見時は Spec への逆戻り発動。
- (改善 v12-2 NEW) `check-job-review-layers.md` Layer 3 (Structural cross-axis) sub-rule 追加候補 — 「**Spec wording と実体 infra work の cross-check**」を Layer 3 default check axis に追加。Lesson source: 本 v12 で T7 spec wording (= "rust-runner tokio dependency 追加" + "ESM-mode runner template 拡張" + "observe-tsc.sh CI から invoke") と実体 infra work (= harness 側 ESM mode write) の乖離が、Spec stage iteration v3〜v11 の self-review でも検出されず、Implementation stage `/start` prerequisite 調査で初めて発覚。Spec stage / Implementation stage transition 時点で spec wording の実体整合性を第三者視点で empirical verify する mechanism が不在。
...
- **`tests/e2e_test.rs` T7 (Iteration v12 revise) ESM mode write lock-in tests** (T7-1 で追加 2026-05-08、3 NEW tests):
  - `test_single_file_ts_exec_dir_writes_esm_package_json`: single-file path で `prepare_single_file_ts_exec` 経由 ts-exec dir に `package.json {"type":"module"}` (= `ESM_PACKAGE_JSON` const) 書き出し structural lock-in。`reset_ts_exec_dir` 経由の DRY single source of truth 適用を verify
  - `test_multi_file_ts_exec_dir_writes_esm_package_json`: multi-file path も同 `reset_ts_exec_dir` を共有することの structural lock-in (= 非対称 cover regression を防止、Layer 3 (Structural cross-axis) symmetric path coverage compliance)
  - `test_top_level_await_fixture_runs_via_tsx_after_esm_setup`: top-await fixture (`const value = await Promise.resolve(42); console.log("got", value);`) を harness 経由 tsx で実行 → 期待 stdout `got 42` を empirical end-to-end lock-in (= ESM mode 化 が tsx top-level await 受け入れに有効である ことの structural complement、Layer 2 (Empirical) Dual verdict TS-side 確証)
...
- iteration history: v1 (initial draft 2026-05-01) → v2 (TS-1〜TS-4 完了 + Axis C scope narrowing + I-226 起票 2026-05-01) → v3 (Option β cohesive batch、third-party review 21 actions fix、I-226 撤回 2026-05-01) → v4 (third-party adversarial agent re-review 2nd round 5 件 fix 2026-05-01) → v5 (3rd round 13 件 + Compromise audit fix 2026-05-01) → v6 minor (4th round 2 件 fix 2026-05-01) → v7 (stub test files 作成 2026-05-01、Spec stage true closure) → v8 (Implementation T2 完了時 I-228 sub-entries 4 件 Spec 逆戻り 2026-05-07) → v9 (T5-1 cell-12/24 silent-drop fix 用 InitKind 4→5 split 2026-05-08) → v10 (T5-1 完了後 /check_job 3 iter + /check_problem 2 round 累積 structural fix 5 件 2026-05-08) → **v11 (T5-2 着手時 B2 .await wrap Tier 1 silent loss 発見 + UserMainSubstitution enum + from_dispatch DRY 解消 + cohesion 向上 + INV-7 fill-in 2026-05-08)**。Implementation T6a (= I-154 doc + CI integration、2026-05-08) は doc/CI structural fix のみで Spec 逆戻り不在 = Iteration log 追加なし、T6a の 2 round /check_job + /check_problem も同様 (4-layer review framework refinement 候補 v11-5/6/7 を I-D batch へ feedback)

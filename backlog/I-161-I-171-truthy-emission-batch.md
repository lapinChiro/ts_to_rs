...
| A-6 | `&&=` | T6 Option<union enum> | `match x { Some(U::F64(v)) if <v truthy> => x = <wrap(y)>, Some(U::String(s)) if <s truthy> => x = <wrap(y)>, _ => {} }` | `x = x && y;` ✗ | ✗ | 本 PRD |
...
| O-5 | `\|\|=` | T5 Option<F64> | `if x.map_or(true, \|v\| *v == 0.0 \|\| v.is_nan()) { x = Some(y); }` | `x = x \|\| y;` ✗ | ✗ | 本 PRD |
| O-5s | `\|\|=` | T5 Option<String> | `if x.as_ref().map_or(true, \|v\| v.is_empty()) { x = Some(y); }` | `x = x \|\| y;` ✗ | ✗ | 本 PRD |
| O-6 | `\|\|=` | T6 Option<union enum> | `match x { None => x = <wrap(y)>, Some(U::F64(v)) if <v falsy> => x = <wrap(y)>, Some(U::String(s)) if <s falsy> => x = <wrap(y)>, _ => {} }` | `x = x \|\| y;` ✗ | ✗ | 本 PRD |
...
- narrow alive: `get_expr_type` が narrow 適用型を返すため、effective 型に dispatch すれば narrow は自動継承
...
| A-6x | A-6 | Option<union enum> | !Copy (Named enum, may contain String variant) | `{ match x { ... }; x.clone() }` (clone で ownership 保持) |
...
| B.1.8 | `Lit(Num(NaN))` | const-fold → `true` | `!NaN = true` |
...
| B.1.16 | `OptChain` | unwrap to `chain.is_none() \|\| <inner falsy>` 展開 | OptChain `x?.v` falsy は `x == null \|\| <v falsy>` |
...
| B.1.19 | `Unary(!)` (double negation) | `!!x` → truthy_predicate_for_expr(x, ty) | De Morgan fold |
...
| B.1.23 | `Bin(LogicalAnd)` (`x && y`) | De Morgan: `!<x falsy> \|\| !<y falsy>` ... see note | result 型は union |
...
| B.1.32 | `Await` | tmp bind + falsy predicate on awaited type | |
| B.1.33 | `Assign` | tmp bind + falsy predicate on value type (assignment returns RHS) | |
...
| B.1.36 | `Update` (`i++`) | tmp bind + falsy predicate on F64 | side effect |
...
| B-T5 | Option<primitive> Copy | `!<e>.is_some_and(\|v\| *v != 0.0 && !v.is_nan())` など | `is_some_and` Rust 1.70+ |
| B-T5s | Option<String> | `!<e>.as_ref().is_some_and(\|v\| !v.is_empty())` | `.as_ref()` で borrow |
| B-T6 | Option<union enum> | `match &<e> { Some(U::X(v)) if <v truthy> => false, ..., _ => true }` | per-variant |
...
- **I-161 complete**: Matrix A の T1-T8 × {Ident, Member} × {Stmt, Expr} × {narrow alive, not alive} 全セル
...
1. **Matrix 全セルカバー (最上位条件)**:
   - [ ] Matrix A 全 in-scope cell (A-1〜A-8 × {Ident, Member} × {stmt, expr} + O-1〜O-8) の実出力が ideal 仕様と一致
   - [ ] Matrix B 全 in-scope cell (B.1.1〜B.1.34 × T1-T8) の実出力が ideal 仕様と一致

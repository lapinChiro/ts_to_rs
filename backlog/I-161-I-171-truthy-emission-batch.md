...
| C-5a | Ident | Option<primitive> | non-exit | present non-exit | bare ElseBranch: `match x { Some(v) if truthy => { else_body }, _ => { then_body } }`。post-if reachable from both branches、union narrow = Option<T> = no useful materialization | ✗ fall-through | ✗ **本 PRD** |
| C-5b | Ident | Option<primitive> | exit | present exit | bare ElseBranch (両 arm exit、post-if 到達不能、materialization 不要、ElseBranch shape で functionally equivalent) | ✗ fall-through | ✗ **本 PRD** |
| C-5c | Ident | Option<primitive> | non-exit | present exit | bare ElseBranch (post-if reachable only via then = x falsy。Option<T> stays Option<T>、weak narrow で primitive 用途には不要) | ✗ fall-through | ✗ **本 PRD** |
| C-5d | Ident | Option<primitive> | exit | present non-exit | **EarlyReturnFromExitWithElse**: `let x = match x { Some(v) if truthy => { else_body; v }, _ => { then_body } };` で post-if narrow `T` を outer let に rebind。post-if reachable only via else branch (then exits)。primitive arithmetic / Some-wrap-coerce 等 type-driven post-if use の compile error 解消必須。**SG-T5-DEEP1 retroactive enumeration (2026-04-24)**: 旧 PRD は C-5 を `body any × else any` で lump しており、(T, F) sub-case を explicit enumerate していなかった (RC-A 違反、framework 失敗 signal)。この cell 分解後、ideal output が一意に確定 | ✗ fall-through | ✗ **本 PRD** (Deep-Fix-1 で実装) |
...
| C-15 | Member (Ident/Computed prop) | Option<T> | always-exit | — | type-aware falsy predicate via Layer 1 dispatch (e.g. Option<F64> → `if !<e>.is_some_and(\|v\| v != 0.0 && !v.is_nan()) { exit }`、Option<String> → `if !<e>.as_ref().is_some_and(\|v\| !v.is_empty()) { exit }`、Option<Named other> → `if <e>.is_none() { exit }`)。**SG-T5-1 訂正 (2026-04-24)**: 旧 ideal text `if <e>.is_none() { exit }` は Option<String> で `Some("")` falsy ケースを取り逃す under-specification。実装は Layer 1 falsy_predicate_for_expr 経由で全 inner 型を網羅 | ✗ fall-through `!Option` | ✗ **本 PRD (Layer 1 only)** |
...
### Cross-cutting Invariants (RC-B retroactive enumeration、SG-T5-DEEPDEEP1/2 由来)

Matrix cell に展開できない transversal property は invariant として明示する (`spec-first-prd.md` framework 改善提案 — I-178 で正式化想定):

- **INV-1 (TypeResolver-IR cohesion)**: TypeResolver narrow event が push されたなら、対応する emission path の IR は narrow を materialize する (Let-wrap + tail expr 等)。この invariant が満たされないと type-driven coercion (Some-wrap、variant-wrap、unwrap 等) が IR と乖離して compile error を生む。
  - **T5 PRD scope verification**:
    - Bang `!x` Layer 2 EarlyReturnFromExitWithElse: ✓ (Deep-Fix-1 + Deep-Deep-Fix-1 で cohesive)
    - `=== null` if-let path with then-exit + else-non-exit: ✓ (Deep-Deep-Deep-Fix-1 で symmetric Let-wrap branch 追加)
    - その他 narrow path (typeof / instanceof / OptChain): pre-existing latent gap、本 PRD scope 外 (I-177 narrow-emission-v2 で一括対応想定)

- **INV-2 (parallel narrow-path symmetry)**: 並列 narrow path (Bang Layer 2 / `=== null` if-let / typeof match / instanceof match / OptChain) は同 sub-case (then_exits × else_exits) 入力に対して symmetric な ideal output を持つ。一方の path が narrow materialization をする sub-case では、他 path も同等な materialization が必要。
  - **T5 PRD scope verification**: Bang + `=== null` の symmetric handling 完了。typeof / instanceof / OptChain は本 PRD scope 外 (上記 INV-1 と同じ I-177 依存)
  - empirical test (post-T5): typeof + then-exit + else-non-exit + primitive arithmetic post-if → E0369 fail (pre-existing、T5 fix 前後で変化なし)
  - **I-177 集約 (2026-04-24 user 判断)**: 残存 INV-2 違反 (typeof / instanceof / OptChain Let-wrap 不在) + 関連する collect_expr_leaf_types narrowed_type query 順序 inconsistency + 反対方向 narrow (`!== null` + (F, T) / Truthy `if (x)` symmetric) は **I-177 PRD に sub-item I-177-A / I-177-B / I-177-C として集約決定**。本 PRD scope 外の T5 latent gap 全件は I-177 PRD で一括対応する設計。詳細は TODO の `[I-177]` entry 内 sub-item 記載を参照。

- **INV-3 (Layer 2 closure-reassign suppression cohesion、P1 fix で確立)**: `try_generate_option_truthy_complement_match` (Layer 2) は `try_generate_narrowing_match` の `=== null` swap branch と同じく、`is_var_closure_reassigned(var_name, if_stmt_position)` 判定で closure-reassign 検出時に None 返却し Layer 1 predicate form に fall-through する。outer `Option<T>` binding が closure-reassign target として preserve される。
  - **T5 PRD scope verification**: Bang `!x` Layer 2 (P1 fix) ✓ / `=== null + then_exit + else_non_exit` Let-wrap branch (Deep-Deep-Deep-Fix-1) ✓ / 既存 `=== null` early-return swap branch ✓ — 全 narrow materialization path で suppression 一貫
  - unit test lock-in: `bang_with_closure_reassign_falls_through_to_predicate_form` + `null_check_then_exit_else_non_exit_with_closure_reassign_falls_through`
...
- [x] **Body-exit sub-case completeness (RC-A 対策、SG-T5-DEEP1 retroactive)**: `body × else` dimension を持つ cell は (then_exits × else_exits) の 4 sub-case を explicit enumerate (C-5a/b/c/d 4 行に分解、"any × any" lump 禁止)
- [x] **Cross-cutting invariant enumeration (RC-B 対策、SG-T5-DEEPDEEP1/2 retroactive)**: matrix cell に展開できない transversal property を invariant として明示し verification 方法を記述 (INV-1 TypeResolver-IR cohesion / INV-2 parallel narrow-path symmetry)
...
### T5: I-171 Layer 2 実装 (`try_generate_option_truthy_complement_match` 拡張) ✅ 完了 (2026-04-24)

**完了状態 (2026-04-24)**:
- `try_generate_option_truthy_complement_match` を `OptionTruthyShape` 列挙で 2-form 統一: `EarlyReturn` (既存 T6-3 form) と `ElseBranch` (新規、`else_body.is_some()` 路で `match x { Some(x) if truthy => { else_body }, _ => { then_body } }` を Stmt::Match で emit、Some(x) shadow による narrow 材料化)
- 内部 `unwrap_parens` → `peek_through_type_assertions` 置換で C-11/C-12/C-13 narrow path に乗る
- `convert_if_stmt` 末尾に **const-fold dead-code elimination** を追加 (Layer 1 `BoolLit` を Layer 2 で if-wrapper ごと除去、C-7/C-9/C-24 ideal emission)
- `build_union_variant_truthy_arms` も shape-aware 化、`ElseBranch` form では各 variant arm body に `let <var_name> = Enum::Variant(__ts_union_inner);` shadow を inline
- E2E: 残 RED Matrix C cell 15 件のうち **10 cell GREEN 化** (c4/c5/c7/c11/c12/c13/c14/c17/c19/c24)、残 5 cell は T5 emission ✓ / E2E ✗ で blocker 別物 (c15: synthetic-type-unification gap / c16: OptChain field-access closure E0507 / c16b: T6 P3b 依存 / c18: I-177 narrow-emission-v2 依存 / c23: NC `Option<P> ?? <other>` synthetic union coercion gap)
- Unit test: `truthy_complement_match` 4 case + `if_while::const_fold_dead_code_elim` 6 case + 既存テスト input 修正で計 +11 test (lib 3085 → 3096)
- Quality gate: cargo test 3096 lib + 122 integration + 3 compile + 148 E2E + 32 ignored / clippy 0 / fmt 0 / file-lines OK / Hono bench 111/63 (T4 baseline と完全一致、regression 0)
...
- **Work**:
  - `src/transformer/statements/control_flow.rs::try_generate_option_truthy_complement_match` を scope 拡張:
    ...
    - **C-15 (Member, always-exit only)**: type-aware falsy predicate via Layer 1 (Matrix C-15 訂正版、SG-T5-1 参照)。narrow 材料化なし、narrow event も記録しない

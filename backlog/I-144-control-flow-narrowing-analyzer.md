...
| L2 | Union `T \| U` (synthetic enum) | ✓ 特定 variant に narrow (typeof, instanceof, DU switch) |
...
| E4 | `match` exhaustive | `match x { Some(v) => ..., None => ... }` | 両 arm 明示 narrow |
| E5 | Implicit None tail | Option return の else 欠落に None 暗黙付加 | I-025 現行 |
...
| E10 | Type-specific truthy predicate | `!x.is_empty()` / `x != 0.0 && !x.is_nan()` / `x` | T4c/T4d/T4e truthy narrow 述語 |
...
|----|-------------|---------------------------|---------------------|
| RC1 | **Expect-T-value** (直接 inner T 読取) | #1/#3/#6/#7/#9(arith)/#12/#13/#27-31/#33-35/#42/#45/#46/#48/#50 | narrow alive: 直接 T binding / stale: `.unwrap_or(coerce_default(T))` |
...
|-------|---------|-------------|----------------|----------------|---------------|---------|-----------------|--------------------|
| L1 Option→T | reset → Option に戻す | reset → None (Option 戻し) | NA (non-numeric) | NA | ✓ preserved (observed) | ✓ elide (no-op, observed) | ✓ preserved (observed R6) | **✗ C-2 silent-compile (E2 経路要)** |
| L1 Option→T (T=F64) | reset | reset (None) | **✗ C-1 false-positive** | **✗ C-1 false-positive** | ✓ preserved | ✓ elide | ✓ preserved | **✗ C-2** |
...
| L4 String (non-empty narrow) | reset | NA | NA (string concat?) | NA | ✓ preserved | NA | ✓ preserved | closure reassign 稀 |
...
| I-025 Option return implicit None | L1 | alive (tail None) | E5 implicit None tail |
| I-025 complex (multi exit) | L1 | alive (multi branch) | E4 match (両 arm 明示) |
...
| F4 Loop body | **reassign 有無で分岐**: reassign なし → narrow 維持 / reassign あり → loop head で widen (fixpoint, observed) | 要確認 | **E1/E2 切替** (reassign あり → E2 `let mut Option`) |
| F5 Switch case arm | arm scope 内 narrow | ✓ DU emission / union variant | 維持 |
| F6 Try body | try 内 narrow、catch 到達で widen (observed) | 要確認 | Lock-in (I-149 連動で catch scope widen 実装) |
...
- [x] E (emission) 11 pattern (v2 で E2 を E2a/b/c 分割、E10 追加) を Rust AST pattern として純化
...
| 5 | **E2E readiness**: 各 ✗ cell の E2E fixture が (red 状態で) 準備 | ⏸ T1 着手時に実施 | T1 task 内で C-1/C-2a/b/c/I-024/I-025/Cell#14/T4d/T7 の fixture 作成 |

**Outstanding**: #5 は T1 実施時に green 化。#1-#4 は本 v2 で全 [✅]。
...
#### T1: Per-cell E2E fixture 作成 (red state)

- **Work**:
  - Matrix ✗ cell の細分化 (v2):
    - **C-1**: compound narrow 維持 (`x += 1` 後も narrow)
    - **C-2a**: `??=` + closure capture → RC3 E2a
    - **C-2b**: closure reassign + RC1 arithmetic read → E2b unwrap_or(0.0)
    - **C-2c**: closure reassign + RC6 string concat → E2b unwrap_or("null")
    - **C-2d**: closure reassign + RC1 return → scope out (signature widen 別 PRD)
    - **I-024**: truthy narrow complex case (multi-level)
    - **I-025**: Option return multi exit → E4 match
    - **I-142 Cell #14**: `??=` narrowing-reset structural
    - **T4d-NaN**: truthy number NaN predicate
    - **T7**: OptChain compound narrow
  - 各 ✗ cell + 代表 ✓ cell (regression lock-in) に `tests/e2e/scripts/i144/cell<N>-<desc>.ts` 作成
  - `scripts/record-cell-oracle.sh` で tsc runtime stdout を oracle として記録
  - 現時点で Rust emission が failing (red state) であることを confirm
  - `tests/e2e_test.rs` に `run_cell_e2e_tests("i144")` 呼び出し追加 (現状 red、T7 完了で green 化予定)
- **Completion criteria**: Matrix ✗ cell 全て (C-1, C-2a, C-2b, C-2c, I-024, I-025, I-142 Cell #14, T4d, T7) + 代表 ✓ cell の E2E fixture が red 状態で存在
- **Depends on**: T0

#### T2: Spec-Stage Adversarial Review Checklist 完走

- **Work**:
  - `.claude/rules/spec-first-prd.md` の 5 項目 checklist を 全て [x] にする:
    1. Matrix completeness: 全 cell に ideal output 記載
    2. Oracle grounding: ✗/要調査 cell の ideal output が tsc observation と cross-ref
    3. NA justification: NA cell の理由が spec-traceable
    4. Grammar consistency: matrix variant が reference doc に全て存在
    5. E2E readiness: 各 ✗ cell の E2E fixture 準備済
  - `/check_job` を Spec Stage 版で実施、発見事項を解消
- **Completion criteria**: 5 項目 checklist 全 [x]、`/check_job` Spec Stage で defect 0
- **Depends on**: T1
...
4. ✅ Transformer emission が CFG analyzer + RC context 連動、E1/E2a/E2b/E2c/E3/E4/E10 経路選択可能

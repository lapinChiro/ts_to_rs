...
- **Work**:
  ...
  - **(12-d) E2E fixture green-ify**: cells 70/71/72/74 (4 fixtures、各々 `tests/e2e/scripts/i-205/cell-{70,71,72,74}-*.{ts,expected}`) の `#[ignore]` 解除 + cargo test --test e2e_test で全 pass verify。**cell 74 fixture rename (Iteration v18 fix)**: `class Cache` → `class OptCache` (or similar non-conflicting class name)、Iteration v18 empirical observation で `Cache` が ES2017+ standard built-in `Cache` interface (Service Worker API) と duplicate identifier conflict (TS2300 + TS2339) と判明、fixture content 修正のみで spec / ideal output 維持。
...
- **Completion criteria**:
  - **本 PRD scope cells**: cells 70/72/74 unit test green + E2E fixture green (cell 74 fixture rename 後)
  - **Regression lock-in cells**: cells 71/73/81 unit test green (kind gate / Copy gate / Setter gate skip verify)
  - **Negative cells**: cells 75/76/77/79/80 (別 PRD C2 scope) は本 T12 の no-rewrite gate で正しく skip されること unit test で verify (= helper early return path coverage)
  - **NA cells**: cell 78 (Iteration v18 reclassify) は本 T12 implementation で **emit されない / 影響を受けない** = TS spec で reject される form のため SWC parser を経由した場合の挙動は「getter body without explicit return」path、現状動作維持 (本 T12 で structural change なし)
  - **Hono Tier-transition compliance** (`prd-completion.md` broken-fix PRD pattern、T6-T10 と同形式):
    - **Improvement (allowed)**: existing class Method Getter body non-Copy return E0507 errors transition Tier 1 (clean files count 増加 / errors count 減少 が expected)
    - **Preservation (allowed)**: Hono が C1 pattern (`return self.field;` single-hop) を主要使用していない場合 unchanged
    - **New compile errors (prohibited)**: 本 T12 scope 外 features に新たな compile error 導入は regression
- **Depends on**: T1, T2, T3 (T11 削除済 2026-05-01、本 T12 は T11 dependency なし)
- **Status**: 着手前 Iteration v18 Spec への逆戻り完了 (2026-05-01)、T12 implementation 着手 ready

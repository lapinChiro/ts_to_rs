...
### Iteration v8 (TBD、Implementation stage 着手後 polish + 4-layer review)

- **Planned**: T1〜T15 Implementation Stage Tasks 実装、`/check_job` 4-layer review (Mechanical / Empirical / Structural cross-axis / Adversarial trade-off) で Implementation stage 完了 verification、Defect classification 5 category trace、Iteration v7 で F-deep-deep-7/8 deferred polish (PRD bloat / framework convergence metric) を別 framework PRD として起票検討。
...
### T1: `doc/grammar/ast-variants.md` ClassMember section update + MethodKind tracking 言及 (doc-first)
...
### T2: MethodSignature / TsMethodInfo に kind field 追加 (infrastructure)
...
### T3: collect_class_info で method.kind propagate + `_ => {}` arm 排除 (Rule 11 d-1 fix)
...
### T4: TsTypeLit / convert_method_info_to_sig で kind propagate

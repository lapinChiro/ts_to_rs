...
- ✅ `ast_produces_option` 関数が存在しない（3-7 で削除）
...
**当初の設計**: `convert_expr` から `type_env` を除去し（Phase 4 依存）、その結果として `convert_ts_as_expr` からも除去する。

**実際の実装と設計変更理由**: `convert_ts_as_expr` を調査したところ、`type_env` は `convert_expr` への純粋なパススルーで直接使用していなかった。3-6 の完了条件（「`type_resolution.rs` から `type_env` 依存が消える」）はインライン化で達成可能。関数を呼び出し元 `mod.rs` の TsAs match arm に展開し、`type_resolution.rs` から `type_env` / `TypeEnv` への依存を完全に除去した。

なお、`convert_expr` 自体は引き続き `type_env` を取る。Phase 4 は narrowing 用の push_scope/pop_scope を削除するタスクであり、`convert_expr` のシグネチャ変更は含まない（変数宣言の型追跡用途が残るため）。

#### 3-7: `ast_produces_option` ヘルパー削除 ✅

TypeResolver を強化し、`ast_produces_option` を削除:

1. **Cond 処理強化**: 一方のブランチが null/undefined の場合、非 null ブランチの型 T を `Option<T>` として返す。一方が既に `Option<T>` の場合（ネストされた三項演算子）もそのまま `Option<T>` を返す
2. **OptChain 処理強化**: `ResolvedType::Unknown` を返していたのを、`resolve_member_type` ヘルパーでフィールド型を解決し `Option<T>` で wrap して返すよう変更。メソッド呼び出しの場合は `resolve_method_return_type` で戻り値型を解決
3. `resolve_member_expr` から `resolve_member_type` ヘルパーを抽出（OptChain と共有）
4. OptChain で内側の型が Unknown の場合に `Option<Any>` を返すよう修正（旧 `ast_produces_option` は AST 構造だけで Optional を検出していたため、Unknown でも安全だった。TypeResolver ベースでは Unknown → Unknown だと二重ラップのリスクがある）
5. `ast_produces_option` と `is_null_or_undefined` を mod.rs から削除。Option wrapping は `get_expr_type` の結果のみで判定

...
- heuristic: Phase 3-1〜3-4 完了。`resolve_expr_type` と関連 heuristic 関数群を削除済み。`resolve_field_type`（TypeRegistry フィールド参照）と `get_expr_type`（FileTypeResolution lookup）のみ残存。3-5〜3-7 で副作用分離・TypeEnv 除去・`ast_produces_option` 削除を予定
...
- [x] **Phase 2.5**: Expected Type 伝搬の一本化 — `tasks.expected-type-unification.md` に詳細設計。TypeResolver と Transformer の二重伝搬を解消
...
- [ ] **Phase 3** (3-1〜3-7): Heuristic 削除 — 3-1〜3-4 完了（`resolve_expr_type` 置換・削除）。残り: 3-5 副作用分離、3-6 type_env 除去、3-7 `ast_produces_option` 削除

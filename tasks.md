...
- heuristic: Phase 3-1〜3-5 完了。`resolve_expr_type` と関連 heuristic 関数群を削除済み。`set_expected_types_in_nested_calls` も廃止。`resolve_field_type`（TypeRegistry フィールド参照）と `get_expr_type`（FileTypeResolution lookup）のみ残存。3-6〜3-7 で type_env 除去・`ast_produces_option` 削除を予定
...
- [ ] **Phase 3** (3-1〜3-7): Heuristic 削除 — 3-1〜3-5 完了（`resolve_expr_type` 置換・削除、`set_expected_types_in_nested_calls` 廃止）。残り: 3-6 type_env 除去、3-7 `ast_produces_option` 削除

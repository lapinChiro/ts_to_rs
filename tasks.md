...
**残存する実装不足:**
- expected type 二重伝搬: TypeResolver の `propagate_expected` と Transformer の手動伝搬（`convert_expr_with_expected` 経由 19 箇所）が重複（Phase 2.5 で解消予定）
...
D6 (files.clone 解消)       ─┼─→ 型解決統一と独立。先に実施
                             │
D-TR〜D4 (型解決の統一)     ─┤
  Phase 1: TypeResolver      │  ← 完了
  Phase 2: ExprContext 削除  │  ← 完了
  Phase 2.5: 伝搬一本化     │  ← 次の作業
  Phase 3: Heuristic 削除   │
  Phase 4: TypeEnv 簡素化   │
                             │
D5 (reg パラメータ削除)     ─┘─→ Phase 2.5 完了後が効率的（シグネチャ安定後）
```
...
- [ ] **Phase 2.5**: Expected Type 伝搬の一本化 — `tasks.expected-type-unification.md` に詳細設計。TypeResolver と Transformer の二重伝搬を解消
  - [x] **Phase 2.5-A**: TypeResolver ギャップ埋め（5 パターン追加）+ `visit_var_decl` 再構成 + `resolve_arrow_expr`/`resolve_fn_expr` expected type 読み取り
  - [ ] **Phase 2.5-B**: テストヘルパー整備（`TctxFixture::from_source`）
  - [ ] **Phase 2.5-C**: unit test の TypeResolver 経由移行（50+ テスト書き換え）
  - [ ] **Phase 2.5-D**: Transformer の手動伝搬削除（19 箇所）
- [ ] **Phase 3** (3-1〜3-5): Heuristic 削除 — `resolve_expr_type` の約 30 呼び出しを `FileTypeResolution.expr_type` に置換（**依存: Phase 2.5 完了**）

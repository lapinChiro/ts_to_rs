...
**残存する実装不足:**
- expected type 二重伝搬: Phase 2.5 で解消済み。`convert_expr_with_expected` は private（Option unwrap 再帰専用）。プロダクションコードからの呼び出しなし
...
- [ ] **Phase 3** (3-1〜3-7): Heuristic 削除 — `resolve_expr_type` の約 30 呼び出しを `FileTypeResolution.expr_type` に置換 + `ast_produces_option` 削除（**依存: Phase 2.5 完了**）

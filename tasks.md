...
- heuristic: Phase 3-1〜3-6 完了。`type_resolution.rs` に残存するのは `get_expr_type`（FileTypeResolution lookup）と `resolve_field_type`（TypeRegistry フィールド参照）のみ。`type_env` 依存なし。3-7 で `ast_produces_option` 削除を予定
...
- [ ] **Phase 3** (3-1〜3-7): Heuristic 削除 — 3-1〜3-6 完了。残り: 3-7 `ast_produces_option` 削除

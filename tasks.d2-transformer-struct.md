...
- [ ] **F-2**: `statements/mod.rs` のラッパーを削除（take+restore 22 箇所 + clone 約 15 箇所が解消される）。**F-1 で導入された過渡的パターン**: statements/ free function 内の `crate::transformer::Transformer { tctx, type_env: type_env.clone(), synthetic }.convert_expr(expr)` 11 箇所が解消される（free function 自体が削除されるため）
- [ ] **F-3**: `functions/mod.rs` のラッパーを削除。dead code 3 件（`convert_var_decl_arrow_fns`, `extract_fn_return_type`, `extract_fn_param_types`）も同時に削除される。**追加対応**: `convert_var_decl_arrow_fns` メソッド内の `arrow_type_env.clone()`（line 1209）を排除する — insert 時のマッピングを `Vec<(String, RustType)>` に記録し、TypeEnv は move で渡し、line 1242 の参照は Vec から行う
- [ ] **F-4**: `classes.rs` のラッパーを削除。dead code 2 件（`pre_scan_classes`, `transform_class_with_inheritance`）も同時に削除される
...
| 3 | `classes.rs:795` | `convert_constructor_body` | `convert_expr` | `type_env`（ローカル） | **F-1 で暫定対応**（`type_env.clone()` + サブ Transformer）。F-2 完了後にループ全体をサブ Transformer で統一し clone を排除: `let mut sub_t = Transformer { tctx, type_env, synthetic }; for stmt { sub_t.convert_expr() / sub_t.convert_stmt() }` |
| 4 | `classes.rs:803` | `convert_constructor_body` | `convert_stmt` | `type_env`（ローカル） | #3 と同時に解消（ループ全体のサブ Transformer 統一） |

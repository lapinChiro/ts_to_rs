...
- [ ] **F-3**: `functions/mod.rs` のラッパーを削除。dead code 3 件（`convert_var_decl_arrow_fns`, `extract_fn_return_type`, `extract_fn_param_types`）も同時に削除される。**追加対応**: `convert_var_decl_arrow_fns` メソッド内の `arrow_type_env.clone()`（line 1209）を排除する — insert 時のマッピングを `Vec<(String, RustType)>` に記録し、TypeEnv は move で渡し、line 1242 の参照は Vec から行う。**clippy 違反の解消（F-2 で発見）**: ラッパー6個（`convert_fn_decl` line 1319, `convert_ts_type_with_fallback` line 1336, `convert_object_destructuring_param` line 1351, `convert_var_decl_arrow_fns` line 1368, `extract_fn_return_type` line 1383, `extract_fn_param_types` line 1398）に `type_env: type_env` の redundant field names がある（clippy::redundant_field_names）。ラッパー削除で解消されるが、削除後に `cargo clippy --all-targets --all-features -- -D warnings` で 0 エラーを確認すること
- [ ] **F-4**: `classes.rs` のラッパーを削除。dead code 2 件（`pre_scan_classes`, `transform_class_with_inheritance`）も同時に削除される。**clippy 違反の解消（F-2 で発見）**: ラッパー4個（`extract_class_info` line 1198, `convert_class_decl` line 1214, `pre_scan_classes` line 1229, `transform_class_with_inheritance` line 1247）に `type_env: type_env` の redundant field names がある（clippy::redundant_field_names）。ラッパー削除で解消されるが、削除後に `cargo clippy --all-targets --all-features -- -D warnings` で 0 エラーを確認すること

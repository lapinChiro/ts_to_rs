# Synthetic PRD: T1-pre-4 cell numbering drift clean (negative fixture)

**Status**: Spec stage Iteration v1 (synthetic fixture for T1-pre-4 audit script extension)

PRD I-D-pre Phase 3 T1-pre-4 `verify_cell_numbering_drift_detection` の negative case
fixture。All cell references use canonical "Cell N" / matrix # form, no identifier-level
vocabulary fork = audit pass.

All axes (pending verdict / cross-reference / cell numbering) clean.

## Background

T1-pre-4 audit を **negative case** (= no violation) で verify する目的。
...
Verify T1-pre-4 audit passes when canonical naming consistently applied.

### Verifiable success criteria

audit script exits zero (no "T1-pre-4 violation" in stderr).
...
Cell # canonical = Problem Space matrix # 1. Single-source-of-truth principle 適用、
Spec→Impl Mapping table cell # と 1-to-1 sync。
...
- PRD I-D-pre T1-pre-4 (= negative case).

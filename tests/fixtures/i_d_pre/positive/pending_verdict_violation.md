# Synthetic PRD: T1-pre-1 pending verdict violation (positive fixture)

**Status**: Spec stage Iteration v1 (synthetic fixture for T1-pre-1 audit script extension)

PRD I-D-pre Phase 3 T1-pre-1 `verify_pending_verdict_findings_consistency` を audit
script 側で trigger するための synthetic fixture。TS-pre-X heading 内 Status field に
post-v15 wording 残存 = F7 fix integrated check で flag されるべき。

Other axes (cross-reference / cell numbering / vocabulary fork) は clean。

## Background

T1-pre-1 violation を **positive case** で確実に trigger する目的。
...
Trigger T1-pre-1 violation for audit script regression test.

### Verifiable success criteria

`python3 scripts/audit-prd-rule10-compliance.py <this-fixture>` exits non-zero with
"T1-pre-1 violation (pending verdict)" substring in stderr.
...
- **Completion criteria**: audit detects violation.
- **Status**: IN PROGRESS (Iteration v17 期待 で完成、v18 で convergence target)

### TS-pre-4: Impact Area audit findings record

- **Work**: Record findings.
- **Completion criteria**: section embedded.
- **Status**: PENDING (post-v16 wording で flag されるべき synthetic)
...
- PRD I-D-pre T1-pre-1 (= 本 fixture が trigger する audit function).

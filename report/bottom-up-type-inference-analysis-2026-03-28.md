...
**日付**: 2026-03-28
**Base commit**: 4f1c76a（未コミット変更あり: PRD/TODO/plan.md 更新）
**対象**: エラー（OBJECT_LITERAL_NO_TYPE 40件）＋ **フォールバックで不正確な型を付与している全箇所**
**関連**: `report/fallback-type-inventory-2026-03-28.md` — 全フォールバック箇所の網羅的インベントリ（46+ 生成サイト、カスケード分析付き）
...
| 優先度 | Sink パターン | 改善件数（エラー+フォールバック） | 実装難易度 |
|--------|-------------|-------------------------------|-----------|
| **最高** | S10: コールバックパラメータ型推論 | ~20（エラー:G9 + フォールバック:層2#14,17 + 層4#7） | 中 |
| **最高** | S4: Vec メソッド要素型 | 3（エラー:L3） | 低 |
| **高** | S1: return 文（コールバック戻り値型） | ~12（エラー:G9 と重複 + フォールバック:層2#12） | 中 |
| **高** | S5: 代入 RHS → LHS 型 | ~4（エラー:K1 + フォールバック:層4#8 + 層2#15,16） | 低 |
| **高** | S6: `as T` 型逆伝播 | ~3（エラー:F2 + フォールバック:層4#3） | 低 |
| **中** | S9: 三項演算子対分岐型 | ~3（フォールバック:層2#10 + 層4#4） | 低 |
| **中** | S7: typeof/instanceof ガード | ~4（フォールバック:層5全3 + 層2#1） | 中 |
| **中** | S8: プロパティアクセス | ~5（フォールバック:層2#11,15,16 + 層4#5） | 中 |
| **低** | S2: `\|\|`/`??` ジェネリクス制約解決 | ~8（エラー:H8）| 高（ジェネリクス制約解決が前提） |
| **低** | S3: 型引数推論 | ~15（エラー:D9 + フォールバック:層2#3,4,6 + 層4#7）| 高（TypeScript 相当の推論エンジン）|

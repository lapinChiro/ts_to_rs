...
| T2.A: PRD-A 実装 | **done** | 2026-04-08 | Cluster 1a 11/11 完全解消 |
| T2.A-i: 外部 builtin JSON loader の scope push | **done** | 2026-04-08 | TNext / TResult / TResult1 / TResult2 (4 件) 解消 |
| T2.A-ii: TypeResolver パスの type_param scope push | **done** | 2026-04-08 | P, S (2 件) 解消 — `enter_type_param_scope` 共通化で 5 callers 補完 |
| T2.A-iv: expected type 経由の free type param 伝搬 | **done** | 2026-04-08 | E (1 件) 解消 — `collect_free_type_vars` を resolve_arrow_expr / resolve_fn_expr に投入 |
...
#### Sub-cluster T2.A-i: 外部 builtin JSON ローダの type_param scope 欠如 (4 件) — **解消済 (2026-04-08)**

**解消結果**:
- 抽出器 `tools/extract-types/src/extractor.ts::extractSignature` に `sig.getDeclaration().typeParameters` 抽出を追加
- `tools/extract-types/src/types.ts::ExternalSignature` に `type_params?` フィールド追加
- `web_api.json` / `ecmascript.json` を再生成 (`type_params` 出現数: ecmascript 95, web_api 87)
- Rust ローダ (`src/external_types/mod.rs`) に `ExternalSignature.type_params` deserialize + `convert_external_typedef` で interface scope push + `convert_external_signature` で method scope push を実装
- 副次対応: 抽出器の intersection 型 fallback (line 367-369) を `{ kind: "named", name: typeStr }` から `{ kind: "any" }` に変更 (raw 文字列 leak 防止、constraint 弱化のみで safe)
- 検証: probe 再実行で TNext/TResult/TResult1/TResult2 4 件すべて消失、新規 regression 0、`cargo test --lib` 2226 passed、Hono ベンチ regression 0
- 追加テスト: `external_types::tests::test_promise_then_signature_type_params_propagated_to_synthetic_union` (Promise.then の TResult1/TResult2 が MethodSignature.type_params + 合成 union enum.type_params に伝播することを assert)
...
#### Sub-cluster T2.A-ii: TypeResolver パスの type_param scope push 漏れ — **解消済 (2026-04-08)**

**発見の正体**: 当初 T2.A-iii とした P/S は実は T2.A-ii と同一根本原因 (TypeResolver パスが
generic 型パラメータを `synthetic.push_type_param_scope` に push していなかった) であり、
1 つの修正で同時に解消した。

**解消経緯**:
- `register_union` に backtrace tracer を投入し、E/P/S が含まれる union の登録時 scope を計測
- S は `TypeResolver::visit_class_body` 経由 (scope=[])、P は `TypeResolver::resolve_arrow_expr` の `collect_type_param_constraints` 経由 (scope=[]) で発生していることが判明
- TypeResolver が `type_param_constraints` (HashMap) は管理していたが、`synthetic.push_type_param_scope` を呼ばないため synthetic 側 scope が常に空。dedup first-write-wins の挙動と合わせ、後の Transformer 側の正しい呼び出しは bad first entry を再利用
- 修正: `helpers.rs` の `collect_type_param_constraints` を `enter_type_param_scope` にリネーム + scope push を担当、5 callers (`visit_fn_decl` / `visit_class_body` / `visit_method_function` / `resolve_arrow_expr` / `resolve_fn_expr`) を `(constraints, prev_scope)` を受け取る形に統一
- 副次効果: scope push を constraint 変換**前**に行うため、`<K, V extends Record<K, string>>` のような sibling 参照 constraint も自動的に解決可能 (session-todos T-2 を TypeResolver パスで解消)
- 検証: probe 再実行で P/S 消失、新規テスト `complex_features::test_class_method_generic_propagates_to_synthetic_union_via_type_resolver` 追加 (`Wrap<S> | S` の synthetic enum が type_params=[S] を持つことを assert)、`cargo test --lib` 2227 passed、Hono ベンチ regression 0

#### Sub-cluster T2.A-iv: expected type 経由の free type param 伝搬 (1 件) — **解消済 (2026-04-08)**

**解消結果**:
- `helpers.rs::collect_free_type_vars` 新規追加: 解決済 RustType を walk し、registry に未登録 + 既知 scope に無い + Rust builtin (`HashMap`/`Box`/...) でない + path 修飾無し (`::` 含まない) の `Named { name }` / `DynTrait(name)` を「free type variable」として抽出する構造的判定 (heuristic ではない)
- `resolve_arrow_expr` / `resolve_fn_expr`: expected_types 読み出し直後に `collect_free_type_vars` を実行し、抽出した free var を body 解決中だけ `synthetic.push_type_param_scope` に append-merge
- 副次効果: `RUST_BUILTIN_TYPES` を `pub(crate)` に昇格し、type_resolver から再利用可能に
- 検証: probe 再実行で E が消失 (Cluster 1a 11/11 達成)、新規テスト `complex_features::test_arrow_inheriting_generic_interface_does_not_leak_free_type_var` 追加 (`GenericMiddleware<E>` を annotation に持つ const arrow から E が leak しないことを assert)、`cargo test --lib` 2228 passed、Hono ベンチ regression 0

**Cluster 1a 完全解消後の dangling refs (23 件)**: 以下はすべて T2.A の対象外で別 cluster に属する
- DOM/Web API builtin (16 件): AudioData, BufferSource, CanvasGradient, CanvasPattern, HTMLCanvasElement, HTMLImageElement, HTMLVideoElement, HeadersInit, ImageBitmap, ImageBitmapRenderingContext, MediaSourceHandle, RTCDataChannel, RequestInfo, SVGImageElement, ServiceWorker, TemplateStringsArray, VideoFrame, WebGL2RenderingContext, WebGLRenderingContext, Window — Cluster 1c (PRD-β 想定)
- 不明識別子 (2 件): __type, symbol — Cluster 1b (PRD-γ 想定)
- user 定義型 (1 件): HTTPException — Cluster 2 (= I-382 本体, PRD-B 想定)

#### (旧) Sub-cluster T2.A-iv: expected type 経由の free type param 伝搬 (1 件、未対応)

**対象**: `E` (referencer: `FnSSGParamsOrSSGParams`)

**由来**: `helper/ssg/middleware.ts:43` の `export const ssgParams: SSGParamsMiddleware = (params) => async (c, next) => {...}`

**新たに判明した root cause**: 内側 / 外側どちらの arrow も自身では `<E>` を宣言していない。
`SSGParamsMiddleware` interface の `<E extends Env>(...)` 由来の E は **expected type
propagation 経由** で arrow body に運ばれる。`resolve_arrow_expr` は `expected_types[arrow_span]`
から param 型を取得して使うが、その時点で expected type は既に call signature が
"instantiated" された後の `Fn<...>` であり、原型の type_params 情報は失われている。

このため arrow body 内で `Context<E>` を含む synthetic union が登録される際、E は
TypeResolver 自身の scope にも synthetic registry の scope にも存在せず、leak する。

**T2.A-ii との違い**: T2.A-ii は AST 上の `arrow.type_params` 等から直接抽出可能だが、
T2.A-iv は expected type の元となった interface の generic 情報を辿る必要があり、別経路の
設計が必要。

**推奨対応** (3 案、未決定):
1. **(a) free type variable 抽出**: 内側 arrow 進入時、expected type に含まれる free な
   `Named` 名のうち、現スコープに無く registry にも未登録のものを暫定 type param として
   scope に push する (構造的に最も近いが、誤検出リスクあり)
2. **(b) expected type 由来 type_params の保持**: `expected_types` の値を `Fn` 単体ではなく
   `(Fn, Vec<TypeParam>)` のペアにし、call signature 解決時に type_params を保持。
   resolve_arrow_expr が読む際に scope に push (最も正確だが invasive)
3. **(c) free type param の erasure**: arrow が自身では type_param を宣言しない場合、
   expected type 内の free 名を `Any` に置換してから param/return 型を取り出す
   (silent semantic loss にはならないが、callee 型情報が失われる)

T2.A-iv は別 PRD として切り出すか PRD-B 統合を検討。本 TODO は T2.A 完了基準
(11/11 解消) の障害となるため、`session-todos.md` ではなく master-plan で追跡する。

#### (旧) Sub-cluster T2.A-iii: utility type / typedef resolve 経路の scope 引き継ぎ漏れ (2 件)

**※** 当初 P/S は別 root cause と推定したが、T2.A-ii と同一発生源と判明したため
T2.A-ii に統合済 (上記参照)。

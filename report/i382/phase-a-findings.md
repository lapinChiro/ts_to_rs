...
- DOM 型は `src/builtin_types/web_api.json` に登録済 →
  `src/registry/mod.rs:569-572 register_external` で external HashSet に入る
- **root cause**: external 認識は成功するが、当該 TypeDef variant が
  ...
  `pub struct HTMLCanvasElement;` が生成される
- **Phase B 含意**: `TypeDef::ExternalUnsupported` variant 導入
  (または generate_external_struct の非 Struct variant 対応) が必須。
  TypeVar refactoring 自体とは直交した独立 PRD 候補 (PRD-β)

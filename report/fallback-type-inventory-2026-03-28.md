...
> **⚠️ 注意**: 行番号は base commit 時点のもの。C-1〜C-4 の開発（`d88d75e` まで）で多数のコード変更があり、行番号はずれている。コード参照時は関数名・パターンで検索すること。
>
> **2026-03-31 更新（d88d75e 時点の実測値）**:
> - `RustType::Any` 生成サイト: 26 → **87 箇所**（C-1〜C-4 で intersection/expected type/TypeRegistry 基盤拡張により増加。カスケード構造は不変）
> - `ResolvedType::Unknown` 生成サイト: 30+ → **57 箇所**（call_resolution.rs に集中。2パス解決基盤の拡充による）
> - `todo!()` 生成: patterns.rs の typeof/in/instanceof 3パターン → **大幅改善**。narrowing ロジック改善により patterns.rs から todo!() は除去済み。残存は `generator/mod.rs:306`（メソッド本体プレースホルダ、1箇所のみ）
> - `serde_json::Value` カスケード: 不変（A → B-3 の最終変換経路は同一）
> - `HashMap` フォールバック: identity mapped type（`{ [K in keyof T]: T[K] }` → `T`）は B-2 で解消。P2-P5 パターンは依然 HashMap
